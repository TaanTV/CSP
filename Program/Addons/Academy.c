...
void LandAcademyFortCapture(string _tmp)
{
	...
	for(n = 1; n <= iEnemyCrew; n++)
	{
		iChar = NPC_GenerateCharacterIndep("AcademyLandEnemy_" + n, "pirate_"+1+rand(9), "man", "man", iRank, PIRATE, 0, false);
	...
	Log_InfoS(GetConvertStrWithReplace("Variable_Academy_1", "Logs.txt", "#space#", " "));
}
...
void AcademyLandCheckLoosers()
{
	...
	if(iPlayerCount == iPlayerCrew)
	{
		...
		Log_Info(GetConvertStrWithReplace("Variable_Academy_2", "Logs.txt", "#space#", " "));
	...
	if(iEnemyCount == iEnemyCrew)
	{
		...
		AcademyLandTrainingPause();
		Log_Info(GetConvertStrWithReplace("Variable_Academy_3", "Logs.txt", "#space#", " "));
		LAi_SetFightMode(PChar, false);
...
void AcademyLandTrainingEnd(bool bWinner)
{
	...
	Log_TestInfo(GetConvertStrWithReplace("Variable_Academy_4", "Logs.txt", "#space#", " ") + d + GetConvertStrWithReplace("Variable_Academy_5", "Logs.txt", "#space#", " "));
...
{
	PChar.AcademyLand.Can = true;
	Log_TestInfo(GetConvertStrWithReplace("Variable_Academy_6", "Logs.txt", "#space#", " "));
}
...
void LandAcademyWallOnWall(string _tmp)
{
	...
	LAi_group_SetRelation("EnemyFight", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);
	LAi_group_FightGroups("EnemyFight", LAI_GROUP_PLAYER, true);
	Log_Info(GetConvertStrWithReplace("Variable_Academy_7", "Logs.txt", "#space#", " "));
}
...
void LandAcademyOneOnOne(string _tmp)
{
	...
	ChangeCharacterAddressGroup(chr, PChar.location, "encdetector", "enc02");
	...
	LAi_group_SetRelation("EnemyFight", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);
	LAi_group_FightGroups("EnemyFight", LAI_GROUP_PLAYER, true);
	Log_Info(GetConvertStrWithReplace("Variable_Academy_8", "Logs.txt", "#space#", " "));
}
