...
void Sea_DeckBoatStartNow(ref _iShipsCharacter)
{
	...
	if (!bAbordageStarted)
	{
		...
		pchar.boatinfo = _iShipsCharacter.index;
...
void DeckBoat_Start()
{
	...
	//Настроим интерфейс
	DeleteBattleInterface();
    PlayVoice("interface\"+LanguageGetLanguage()+"_Abandon0.wav");// шлюпки на воду!
	ref mchr = GetMainCharacter();
...
void SetSailorDeck_Ships(ref Chref)
{
	...
	if(characterID == "MaryCelesteCapitan")
	{
	    ...
	    sld.name = GetConvertStrWithReplace("Variable_Cabin_1", "Names.txt", "#space#", " ");
	    sld.lastname = "";
	...
	if(CheckAttribute(pchar,"questTemp.ReasonToFast.canSpeakSailor") || CheckAttribute(pchar,"GenQuest.CaptainComission.canSpeakBoatswain"))
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_Cabin_2", "Names.txt", "#space#", " ");
	    sld.lastname = "";
		sld.Dialog.Filename = "Quest\sailors_dialog.c";
	...
	if(CheckAttribute(pchar,"GenQuest.Hold_GenQuest.canSpeakSailor"))
	{
		model = LAi_GetBoardingModel(PChar, &ani);
		sld = GetCharacter(NPC_GenerateCharacter("saylor_0"+i, model, "man", ani, 10, sti(PChar.nation), 0, true));
		sld.name = GetConvertStrWithReplace("Variable_Cabin_3", "Names.txt", "#space#", " ");
	    sld.lastname = "";
		sld.Dialog.Filename = "Quest\sailors_dialog.c";
    ...
    for (int i=1; i<5; i++)
    {
	    ...
	    sld.name    = GetConvertStr("Variable_quests_reaction_46", "Names.txt");
	    sld.lastname = "";
        sld.Dialog.Filename = "Quest\sailors_dialog.c";
    ...
    for (i=0; i < GetPassengersQuantity(mchr); i++)
    {
        ...
        if(cn != -1)
        {
            ...
            if(CheckAttribute(offref,"prisoned"))
            {
	            if(sti(offref.prisoned)==true && GetRemovable(offref)) // ставим только фантомов
	            {
                    ChangeCharacterAddressGroup(offref, mchr.location, "rld", "aloc0");
                }
            }
        }
    }
