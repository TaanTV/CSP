...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Whisper_china_relic_3":
			dialog.text = "Если бы я это знал, то ни о чём вас не просил бы. Но я не тороплю, приходите, когда найдёте. От меча в ближайшее время я избавляться не собираюсь.";
		...
		case "StepPL5First_3":
			...
			dialog.text = "Да вроде бы собирался на север Мэйна курс держать, к испанцам. А куда точно - не ведаю... Хотя подождите, что-то он говорил о " + XI_ConvertString("Colony" + pchar.questTemp.piratesLine.Q5.city_2 + "Voc") + ". Но сами понимаете, что за достоверность не ручаюсь.";
		...
		case "StepPL5Second_1":
			dialog.text = "Нанимал. Работу он выполнил, долг вернул сполна. Только в таверне стал болтать языком, что, якобы, выбил у должника сумму, многократно превышающую сумму долга, и как провёл меня, умолчав об этом.\n"+
				...
				"В общем, я вздохнул с облегчением, когда Джон Эйвори куда-то уплыл из нашего города. И теперь, слышать о нём не желаю!";
		...
		break;
		case "StepPL5Four_2":
			dialog.text = "Да, да, я о вас слышал... кажется... Так о чём я рассказывал? А да, представляете, врывается эдакая образина, приставляет нож к горлу, и начинает меня грабить! Меня!!! Уважаемого человека, посреди бела дня...";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		case "LoanUsurer":
			...
			if (npchar.city == "villemstad" && !CheckAttribute(pchar, "questTemp.KSM_DVR_Rostovshik"))	//Квест в Виллемстад "Деньги в рост"
			{
				link.l1 = "Есть ли у вас дело, требующее сторонней помощи?";
			...
			link.l10 = "У меня есть драгоценные камни на обмен.";
		...
		case "jewelCancel":
			...
			AddQuestUserData("SeekUsurersJewel", "sSex", GetSexPhrase("","а"));
		...
		case "CapBloodUsurer_2":
			dialog.text = "Что?! О чём вы говорите?";
		...
		case "SlaveGalleon_1":
			dialog.text = "Вы очень близки к истине, " + pchar.name + ". Именно отобрать, а конкретно - свободу. Репартиментос - это своеобразный хитрый способ закабаления индейцев под благовидными псевдо-законными предлогами - мирным путём, так сказать. Применяется испанскими властями по отношению к коренному населению Мэйна. Вам интересно полушать подробности?";
		...
		case "FindRat_6":
			...
			link.l1 = "Хм... Этого негодяя следует вздёрнуть на нок-рее! Сообщите мне более подробную информацию о нём, ну и, конечно, размер гонорара за работу.";
			link.l1.go = "FindRat_7";
		break;

		case "FindRat_7":
			dialog.text = "Насчёт вознаграждения не беспокойтесь. Я щедро заплачу вам за его уничтожение, и не только деньгами. Я не пожалею лучших вещей из своей индейской коллекции. Теперь о нём. Эту крысу зовут Франсуа Гонтьер\nНедавно, чтобы замести следы - ибо он знает, что я за ним охочусь - он продал свой фрегат на одной из верфей архипелага и скрылся. Мне удалось выяснить у своих коллег, что его недавно видели в Панаме. Пожалуй, вам стоит начать поиски именно оттуда.";
		...
		case "Havana_Fort_yes_3":
			dialog.text = "Вот видите... " + pchar.name + ", вы мой лучший агент, и я рассчитываю вести с вами бизнес в дальнейшем. В знак моей признательности, позвольте подарить вам мой коллекционный клинок с талисманом и кирасу которая много раз спасала меня от шальных пуль в прошлом. И может, хватит уже постоянно высказывать мне своё недоверие, да ещё в такой... резкой форме?";
			link.l1 = "Хорошо, " + npchar.name + ". Просто вы и меня поймите: миллион - не маленькая сумма. А за этот палаш примите мою благодарность, выглядит он и вправду роскошно.";
			log_info("Вы получили Кханду");
		...
		case "KSM_DVR_1":
			dialog.text = "Да-да. Некое деликатное дельце... Один человек, Саймон Бартон, взял у меня довольно крупную сумму, под неплохой процент. Да вот только возвращать её не спешит.";
			link.l1 = "Ситуация неприятная. Почему же вы дали деньги столь сомнительному субъекту?";
			link.l1.go = "KSM_DVR_2";
		break;

		case "KSM_DVR_2":
			dialog.text = "Саймон казался вполне преуспевающим дельцом. Только вот всё его состояние в один день ушло на дно вместе с принадлежащим ему судном... А теперь он пропивает занятое у меня золото.";
			link.l1 = "Я могу посетить господина Саймона и попросить его вернуть деньги.";
			link.l1.go = "KSM_DVR_3";
			link.l2 = "Мне не интересно выбивать долги. Прощайте.";
		...
		case "KSM_DVR_3":
			...
			dialog.text = "Он занял " + KSM_DVR_Schitaem + " пиастров... Было бы просто замечательно, если бы вы изъяли хотя бы половину. К тому же, от того, сколько денег удастся вернуть, зависит и ваша доля.";
			link.l1 = "По-моему, всё честно. Где мне искать господина Бартона?";
			link.l1.go = "KSM_DVR_4";
		break;

		case "KSM_DVR_4":
			dialog.text = "Похоже, что Саймон слегка свихнулся от своих бед. Целыми днями сидит в бухте Пальмовый берег, всё ждет, когда его корабль покажется на горизонте...";
			link.l1 = "Я найду его.";
