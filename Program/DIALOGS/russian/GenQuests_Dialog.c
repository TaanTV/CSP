void ProcessDialogEvent()
{
	...
	{
		case "First time":
			dialog.text = "Явный баг. Как он здесь оказался? Сообщите о нём Warship'у и подробно расскажите, как Вы его получили.";
		...
		case "Convict_11_2":
			...
			link.l1 = "Ну хорошо, ждите меня " + XI_ConvertString(pchar.GenQuest.Convict.ShoreLocation + "Voc") + ". Ближе к полуночи будет вам кораблик... И не надейтесь меня обмануть!";
		...
		case "Convict_14":
			dialog.text = RandPhraseSimple(RandPhraseSimple("Эх, капитан, небось сам"+ GetSexPhrase("","а") +" недавно с пустым кошелём бегал"+ GetSexPhrase("","а") +", а как в карманах зазвенело - нос от простого люда воротишь.","Капитан, негоже так с простыми людьми обходиться. Все под одним Богом ходим."),
		...
		case "CaptainComission_51":
			...
			link.l1 = "Пираты, не дождавшись выкупа продали его в рабство. Я пош"+ GetSexPhrase("ёл","ла") +" за ним на Барбадос, но спасти не сумел"+ GetSexPhrase("","а") +"...";
		...
		case "CaptainComission_304":
			...
			dialog.text = "Помогите мне людьми и шлюпками, чтобы вывезти груз с " + XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Gen") + ", а за это заберёте всё, что не войдёт в мой трюм.";
		...
		case "CaptainComission_305":
			string sTempGen = GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name") + "Gen"));
			sTemp = GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name") + "Acc"));//здесь нужен винительный падеж? его нет в коммон ини пока что
			dialog.text = "Ну хорошо, чтоб не водить вас за нос, расскажу всё по порядку.\n" +
				...
				"Мы уже приготовились было к смерти, но, благодаря Пресвятой Деве Марии и взводу мушкетёров, нам удалось перебить пиратов. Каково же было наше ликование, когда обнаружилось, что трюм " + sTempGen + " под завязку набит " + GetStrSmallRegister(XI_ConvertString(Goods[sti(pchar.GenQuest.CaptainComission.Goods)].Name + "Abl")) + ". " +
		...
		case "CaptainComission_308":
			dialog.text = "Тогда давайте встретимся " + XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Voc") + " сразу после полуночи, а то при моём нынешнем положении маячить на рейде слишком опасно. Как бы нас не обнаружил патруль.";
...
//			bQuestDisableMapEnter = true;
		...
		case "CaptainComission_310":
			dialog.text = "Я заберу ваше судно, а вас оставлю " + XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Voc") + ".";
		...
		case "CaptainComission_313":
			...
			AddQuestUserData("CaptainComission2", "sShoreNameDat", XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Voc"));
		...
		case "CaptainComission_Canoneer11":
			...
			dialog.text = "Вот и славно. Встречаемся сегодня после полуночи " + XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore + "Voc") + ". И позаботьтесь, чтоб в вашем трюме было место для " + pchar.GenQuest.CaptainComission.GoodsQty + " шт. " + GetStrSmallRegister(XI_ConvertString(Goods[sti(pchar.GenQuest.CaptainComission.Goods)].Name + "Gen")) + ". Жалко ведь бросать с таким трудом добытое...";
		...
		case "CaptainComission_388":
			...
			dialog.text = "Мне нужно добраться до " + XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore + "Gen") + ", что на " + XI_ConvertString(GetIslandByCityName(sTemp) + "Voc") + " за " + pchar.GenQuest.CaptainComission.iDay + " дней. Там мои люди должны были подготовить сбыт товара.";
		...
		case "ContraDeliverQuest_NoMon":
			dialog.text = "Слушай сюда - мы с тобой ни о чём не договаривались. Вот тот, кто с тобой договаривался - тот тебе деньги и должен дать.";
...
//				link.l1.go = "ChurchGenQuest1_DialogShip_5_2_1"; // Сваливаем, поверили ему
		...
		case "Greedy":
			dialog.text = "О, ты тут камешек драгоценный наш"+ GetSexPhrase("ёл","ла") +"? Отдай его, пожалуйста. С тебя не убудет, а нам поможет штаны поддержать.";
