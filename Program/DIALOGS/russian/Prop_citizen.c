	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
            ...
            if(!CheckAttribute(NPChar, "quest.propositioned") || NPChar.quest.propositioned == "0")
			{
                ...
                if (!CheckAttribute(pchar, "questTemp.different.Church_NightGuard") && pchar.questTemp.different == "free" && !CheckAttribute(pchar, "questTemp.different.FackStreet") && !isBloodPrologue)
                {
