...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	case "rumours_habitue":
		...
		{
			 if (pchar.name != "Виспер" && pchar.sex != "skeleton" && !CheckAttribute(pchar, "OrderHunter"))
			 {
				...
				if (pchar.questTemp.pirateVikingQuest == "4")
				{
					if(ShipsTypes[sti(RealShips[sti(pchar.Ship.Type)].BaseType)].Type.Merchant == true && GetCompanionQuantity(pchar) == 1)
					{
						if (GetSquadronGoods(Pchar, GOOD_EBONY) >= 600 || GetSquadronGoods(Pchar, GOOD_SILK) >= 1000 || GetSquadronGoods(Pchar, GOOD_SILVER) >= 400)
						{
							...
							sld.mapEnc.Name = "шебека 'Драккар'";
							sld.mapEnc.type = "war";
							FantomMakeCoolFighter(sld, 20, 80, 80, "topor_viking", "pistol3", 100 * MOD_SKILL_ENEMY_RATE + 100);
							FantomMakeCoolestSailor(sld, SHIP_REQUIN, "Драккар", CANNON_TYPE_CANNON_LBS24, 80, 80, 60);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        case "TakeChest_2":
            if (GetCharacterItem(pchar, "Chest") >= sti(pchar.GenQuest.LoanChest.Chest))
            {
				...
				{
					Dialog.Text = "Всё это хорошо, и я принимаю у вас золото. Однако, вы не уложились в отведённые вам сроки доставки. Поэтому я вынужден сократить размер вашего гонорара на " + pchar.GenQuest.LoanChest.Money.Discount + " процентов. Прошу прощения, но это обсуждению не подлежит. Извольте получить...";
					link.l1 = "Спасибо!";
    				link.l1.go = "exit";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        case "AscoldTrader_WasSeek":
            dialog.Text = "Послушай, не встречал"+ GetSexPhrase("","а") +" ещё порошка мумии, который я просил тебя разыскать?";
            Link.l2 = "Да, наш"+ GetSexPhrase("ёл","ла") +". И сколько ты мне за него заплатишь?";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		case "RBlueBird":
			dialog.text = "Эх, капитан, не случилось, а случается уже давно. Мы, торговцы Карибского моря, на грани разорения. А во всём виновата проклятая 'Синяя Птица'...";
			link.l1 = "'Синяя Птица'?";
		...
		break;
		case "RBlueBird_10":
			dialog.text = "Паскаль Вуазье. Он содержит лавку в Шарп-Тауне. Так вы берётесь за дело?";
		...
		case "RBlueBird_retMoney_5":
			dialog.text = "Ну хорошо, хотя тон здесь ни при чём. Это вы нарушили условия сделки, а не мы - вот суть вопроса!";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		case "MechanicIsPrison_man":
			...
			link.l1 = "В чём дело? Почему вы ещё не на клипере?... А где Механик?";
...
{
	string sRetRumour = "Явный баг. Сообщите о нём АТ, для Warship'а.";
...
string GetInfoShipYarder(string _sTown)
{
	...
	string sTemp = "Говорят, что он худший корабел в этих водах. Странно, что его корабли вообще на плаву держаться."; 
	string sBestInNation;
	int iBestInNation = sti(refNPCShipyard.reputation);

	if (sti(refNPCShipyard.reputation) > 19) sTemp = "Его не хвалят за мастерство. Выдающихся судов он не строит.";
	if (sti(refNPCShipyard.reputation) > 39) sTemp = "Он вполне нормальный мастер. Выдающихся судов он не строит, но и совсем плохих на воду не спускает.";
	if (sti(refNPCShipyard.reputation) > 59) sTemp = "Его хвалят за мастерство. Его верфь выпускает хорошие корабли.";
	if (sti(refNPCShipyard.reputation) > 79) sTemp = "Его очень хвалят за мастерство. Если вам нужен выдающийся корабль, то смело обращайтесь к нему.";
	if (sti(refNPCShipyard.reputation) > 99) sTemp = "Он в совершенстве освоил кораблестроение. Можете смело обращаться к нему, если хотите самый лучший корабль.";

	string sTemp2 = " Говорят, что у него самые низкие цены на корабли и ремонт.";
	if (stf(refNPCShipyard.shipCostRate) > 0.9) sTemp2 = " Говорят, что у него низкие цены на корабли и ремонт."; 
	if (stf(refNPCShipyard.shipCostRate) > 1.0) sTemp2 = " Говорят, что у него высокие цены на корабли и ремонт."; 
	if (stf(refNPCShipyard.shipCostRate) > 1.1) sTemp2 = " Говорят, что у него безумно высокие цены на корабли и ремонт."; 
	sTemp += sTemp2;
	...
	if (iBetterNationYarders == 0) sTemp += " А ещё ходят слухи, что он лучший корабел в " + GetNationNameX(refNPCShipyard.nation) + " колониях.";
		else sTemp += " А ещё ходят слухи, что в " + sBestInNation + " верфист более умелый, так что наши военные заказывают корабли там.";
	//if (iBetterYarders <= 1) sTemp += " А ещё ходят слухи, что он лучший корабел на всём архипелаге.";
...
string GetNationNameX(string snation)
{
	...
	switch (snation)
	{
		case "0": sTemp = "английских"; break;
		case "1": sTemp = "французских"; break;// в французских...
		case "2": sTemp = "испанских"; break;
		case "3": sTemp = "голландских"; break;
		case "4": sTemp = "пиратских"; break;
	}
	return sTemp;
}
