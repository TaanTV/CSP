...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			{
				link.l4 = "Слушай, Роджерс, тебе ни о чём не говорит слово 'стрела'? Может быть, здесь бывает пират по кличке 'Стрела', или судно пиратское с таким названием заходило к тебе в порт?";
				link.l4.go = "Mushket";
			}
			...
			if (CheckAttribute(pchar, "questTemp.silverfleet") && pchar.questTemp.silverfleet != "over" && pchar.questTemp.silverfleet != "ToHuber" && NPChar.id == "LeFransua_Mayor")
			{
				...
				{
					link.l1 = "Здравствуйте. Вас должны были предупредить обо мне. Я по поводу груза, перевозимого с Брабадоса на Ямайку...";
					link.l1.go = "SF_LeFransuaBeginning";
				}
				if (GetNpcQuestPastDayParam(NPChar, "SilverFleetSeaCheck") > 0)
				{
					link.l1 = "По поводу конвоя...";
		...
		case "SF_LeFransuaBeginning":
			dialog.text = "Да-да, меня предупредили. Но многого я вам не скажу. Моё непосредственное начальство имеет интерес к этому конвою... Поэтому подробной наводки я вам не дам, но некоторую информацию предоставлю.";
			link.l1 = "Спасибо и на этом. Так что вы можете сказать?";
			link.l1.go = "SF_LeFransuaSeaCheck";
			pchar.questTemp.silverfleet.lefransuamayorspeak = true;
		break;

		case "SF_LeFransuaSeaCheck":
			dialog.text = "На данный момент 'Серебряный конвой' не в пути. Завтра может будет новая информация.";
			if (CheckExistQuestEncounter("Head_of_Silver_Squadron")) dialog.text = "Прямо сейчас 'Серебряный конвой' находится в пути к Ямайке. Точнее не скажу, извини. Возможно завтра будут новые сведения.";
			if (GetNpcQuestPastDayWOInit(pchar, "SilverFleetGo") < 3) dialog.text = "Совсем недавно 'Серебряный конвой' видели близ Барбадоса. Куда он направился - сам" + GetSexPhrase("","а") +" знаешь.";
