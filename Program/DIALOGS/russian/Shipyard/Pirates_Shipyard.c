	...
	switch (Dialog.CurrentNode)
	{
        ...
        case "ship_tunning_start":
            ...
            else
            {
				...
				if (S_armor(pchar)) // СФ
				{
					dialog.text = NPCStringReactionRepeat("Посмотрим, что тут у нас... Нет... за это... я не возьмусь.", "Я вообще не понимаю технологий, которые применены на вашем корабле. Как я могу его улучшить?", "Я вам уже всё сказал. За это судно я не возьмусь.", 
	                          		"Всё! Моё терпение вышло! У вас других вопросов нет что ли!?", "block", 2, npchar, Dialog.CurrentNode);
					link.l1 = HeroStringReactionRepeat("Хмм... Ну ладно.", "Действительно...",
                      				"Хм, однако...", "Када пач?..", npchar, Dialog.CurrentNode);
					Link.l1.go = "ship_tunning_not_now";
					break;
    				}
                    ...
                    {
    			        Link.l0 = "Увеличить броню корпуса.";
		...
		case "ship_tunning_cannon_all_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению калибра и количества орудий на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) +
					" мастер-корабел затребовал: "+NewStr()+"чёрного дерева - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"сокровищ - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
		...
		case "ship_tunning_cannon_all_again_2":
			...
			else
			{
				...
				AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: "+NewStr()+"чёрного дерева - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"сокровищ - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_c_quantity_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"чёрного дерева - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"сокровищ - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_Capacity":
			...
			s1 = s1 + " плюс - " + capacityWorkPrice + " пиастров на рабочие расходы. Вроде бы всё. Ах да - деньги вперёд.";
            dialog.Text = s1;
			Link.l1 = "Годится. Я принимаю условия. Всё оговорённое будет доставлено.";
		...
		case "ship_tunning_Capacity_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"сандала - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"золотых слитков - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_Capacity_again_2":
			...
			else
			{
                ...
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: "+NewStr()+"сандала - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"золотых слитков - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_SpeedRate":
			...
			s1 = s1 + " Я стар, и не собираюсь вечно гнить в этой дыре, поэтому за работу возьму: сундуков с золотом - "+ SpeedRateMatherial3 + ", хороших гранёных алмазов - "+ SpeedRateMatherial2 + ", плюс - " + SpeedRateWorkPrice + " пиастров на рабочие расходы. Вроде бы всё. Ах да - и деньги вперёд.";
		...
		case "ship_tunning_SpeedRate_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"шелка - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"алмазов - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_SpeedRate_again_2":
			...
			else
			{
                ...
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText",  "Мне осталось довезти: "+NewStr()+"шелка - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"алмазов - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_MastMultiplier":
			...
			s1 = s1 + " Я стар, и не собираюсь вечно гнить в этой дыре, поэтому за работу возьму: сундуков с золотом - "+ MastMultiplierMatherial3 + ", хороших больших жемчужин - "+ MastMultiplierMatherial2 + ", плюс - " + MastMultiplierMatherialWorkPrice + " пиастров на рабочие расходы. Вроде бы всё. Ах да - и деньги вперёд.";
		...
		case "ship_tunning_MastMultiplier_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"досок - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"больших жемчужин - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_MastMultiplier_again_2":
			...
			else
			{
                ...
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText",  "Мне осталось довезти: "+NewStr()+"досок - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"больших жемчужин - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_MaxCrew":
			...
			s1 = s1 + " В оплату возьму: сапфиров - "+ MaxCrewMatherial2 + ", сундуков с золотом - "+ MaxCrewMatherial3 + ", плюс " + MaxCrewWorkPrice + " пиастров на рабочие расходы. Пожалуй, всё. Ах да - и деньги вперёд.";
            dialog.Text = s1;
			Link.l1 = "Годится. Я принимаю условия. Всё оговорённое будет доставлено.";
		...
		case "ship_tunning_MaxCrew_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"кожи - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"сапфиров - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_MaxCrew_again_2":
			...
			else
			{
                ...
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: "+NewStr()+"кожи - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"сапфиров - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков  - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_TurnRate":
			...
			s1 = s1 + " И за работу я возьму: сундуков с золотом - "+ TurnRateMatherial3 + ", рубинов - "+ TurnRateMatherial2 + ", плюс - " + TurnRateWorkPrice + " пиастров на рабочие расходы. Вы же понимаете, что я не планирую умирать на этой помойке... Вот, пожалуй, всё. Ах да - деньги вперёд.";
            dialog.Text = s1;
			Link.l1 = "Годится. Я принимаю условия. Всё оговорённое будет доставлено.";
		...
		case "ship_tunning_TurnRate_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"полотна - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"рубинов - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_TurnRate_again_2":
			...
			else
			{
                ...
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: "+NewStr()+"полотна - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"рубинов - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_HP":
			...
			s1 = s1 + " За работу возьму: серебряных слитков - "+ HPMatherial2 + ", сундуков с золотом - "+ HPMatherial3 + ", плюс - " + HPWorkPrice + " пиастров на рабочие расходы... Внуки давно зовут меня назад в Европу, не хочется, знаете ли, возвращаться с пустыми руками... Вот, пожалуй и всё. Ах да - деньги вперёд.";
            dialog.Text = s1;
			Link.l1 = "Годится. Я принимаю условия. Всё оговорённое будет доставлено.";
		...
		case "ship_tunning_HP_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"красного дерева - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"серебряных слитков - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых... Говорит, что в Европу собрался - врёт, наверное, старый чёрт.");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_HP_again_2":
			...
			else
			{
                ...
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: "+NewStr()+"красного дерева - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"серебряных слитков - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_WindAgainst":
			...
			s1 = s1 + " а в качестве оплаты возьму: изумрудов - "+ WindAgainstMatherial2 + ", сундуков с золотом - "+ WindAgainstMatherial3 + ", плюс - " + WindAgainstWorkPrice + " пиастров на рабочие расходы. Дело-то непростое... Вроде бы всё. Ах да - деньги вперёд.";
            dialog.Text = s1;
			Link.l1 = "Годится. Я принимаю условия. Всё оговорённое будет доставлено.";
		...
		case "ship_tunning_WindAgainst_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: "+NewStr()+"хлопка - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"изумрудов - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". "+NewStr()+"В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Совсем старик с ума сбрендил от этих побрякушек.");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_WindAgainst_again_2":
			...
			else
			{
                ...
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: "+NewStr()+"хлопка - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"изумрудов - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_HullArmor":
			s1 = "Давайте посмотрим, что можно сделать. Броня корпуса сейчас " + hullArmor;

			s1 = s1 + ". Чтобы укрепить корпус понадобится: железа - "+ HullArmorMaterial1 + ",";
			s1 = s1 + " а в качестве оплаты возьму: маленьких жемчужин - "+ HullArmorMaterial2 + ", сундуков с золотом - "+ HullArmorMaterial3 + ", плюс - " + HullArmorPrice + " пиастров на рабочие расходы. Дело-то непростое... Вроде бы всё. Ах да - деньги вперёд.";
            dialog.Text = s1;
			Link.l1 = "Годится. Я принимаю условия. Всё оговорённое будет доставлено.";
		...
		case "ship_tunning_HullArmor_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
			    ...
			    ReOpenQuestHeader("ShipTuning");
			    AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText",  "За свою работу по укреплению брони корабля " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) +
				" мастер-корабел требует: "+NewStr()+"железа - " + NPChar.Tuning.Matherial1 + ", "+NewStr()+"маленьких жемчужин - "+ NPChar.Tuning.Matherial2+ ", "+NewStr()+"сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Совсем старик с ума сбрендил от этих побрякушек.");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_HullArmor_again":
		    ...
		    {
                NextDiag.TempNode = "ship_tunning_HullArmor_again";
			    dialog.Text = "Броня ждёт. прин"+ GetSexPhrase("ёс","есла") +", что я просил?";
			    Link.l1 = "Да. Кое-что удалось доставить.";
		...
		case "ship_tunning_HullArmor_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_HullArmor_again";
                dialog.Text = "Тебе осталось привезти: железа - "+ sti(NPChar.Tuning.Matherial1) + ", маленьких жемчужин - "+ sti(NPChar.Tuning.Matherial2) + ", сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".";
				link.l1 = "Хорошо.";
				link.l1.go = "Exit";

                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: "+NewStr()+"железа - "+ sti(NPChar.Tuning.Matherial1) + ", "+NewStr()+"маленьких жемчужин - "+ sti(NPChar.Tuning.Matherial2) + ", "+NewStr()+"сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_HullArmor_complite":
            ...
            NextDiag.TempNode = "ship_tunning_again";
			dialog.Text = "... Вроде бы всё... Можешь теперь куда уверенней ловить снаряды бортами.";
			Link.l1 = "Спасибо! Проверю обязательно.";
			Link.l1.go = "Exit";
