	...
	switch (Dialog.CurrentNode)
	{
	    ...
	    case "GhostShip_Speak_1":
            ...
            pchar.GenQuest.GhostShip.lastspeak_date = LastSpeakDate();
   			dialog.text = "Ну... И-ик! Видел я страшное - корабль-призрак, а на нём сплошные мертвецы... Говорят, он тут со времён Колумба плавает...";
		...
		case "GhostShip_Speak_2_2":
			if(Pchar.chr_ai.type != LAI_TYPE_SIT) LAi_SetSitType(pchar);
			dialog.text = "'Летучий'! Так корабль-призрак опять появился в архипелаге. Вроде пропадал на много лет... в преисподней был, не иначе. А то как прокляты все на нём и пуля их не берёт.";
        ...
        case "talk_with_alchogol_song":
			dialog.text = "Ну... И-ик! Бывает всё на свете хорошо... И-ик! В чём дело сразу не поймёшь... И-ик! А просто славно ром пошёл... И-ик! ямайский чёрный ром... И-ик!\n Подставляй, наливай!";
			link.l1 = "А дальше?";
		...
		case "begin_sit":
			...
			dialog.text = LinkRandPhrase("Ик! Вот это я понимаю! Вот это по-нашему! Ну, так чего? Поставишь мне кружечку рома?",
						...
						"Эхе-хе, присаживайтесь, "+ GetSexPhrase("сударь мой","сударыня") +", выпьем... отчего ж не выпить в хорошей-то компании? В нашей портовой таверне иногда очень интересные вещи можно услышать, если иметь терпение и пить маленькими глотками, чтобы оверкиль не свалиться... То кэп какой-нибудь загрузит лишку на борт и начнёт своими похождениями хвалиться, то кто-то знающий чего сболтнёт в нужной кондиции... Тут разный люд обретается, и многим есть что порассказать, поверьте, "+ GetSexPhrase("сударь мой","сударыня") +".");
		...
		case "sit_3":
			...
			switch (iSituation)
			{
                ...
                case 5:
					if(Pchar.chr_ai.type != LAI_TYPE_SIT) LAi_SetSitType(pchar);
                    if (!CheckAttribute(pchar , "GenQuest.GhostShip.LastBattleEnd") && !checkAttribute(npchar, "GhostShip_Speak_Yet") && pchar.GenQuest.GhostShip.lastspeak_date != LastSpeakDate() && pchar.rank >= 10)		//Квест Корабль-Призрак
					{
