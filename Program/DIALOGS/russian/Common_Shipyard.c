...
void ProcessDialogEvent()
{
	...
	if(HasSubStr(attrLoc, "shiporderend2_"))
	{
		...
		else pchar.location.from_sea = rColony.from_sea; 	//корабль берёт ГГ и его в порт ставим
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "shiporderend":
			...
			if (sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok_ship)
			{
				...
				else
				{
					...
					Link.l99 = "Да, точно. Заберу в другой раз.";
					Link.l99.go = "exit";
				}
			}
			else			
			{
				dialog.text = "Хм... Я не вижу вашего флагмана в порту. А забрать свой корабль вы можете только здесь.";
				if (sti(Pchar.Ship.Type) == SHIP_NOTUSED)	//фикс - забираем себе, если нет корабля
				{
					Link.l99	= "Да, я как раз без корабля, так что сам заберу.";
					Link.l99.go = "shiporderend2_-1";
				}
				else
				{
					Link.l99 = "Да, точно. Забыл, что эскадра не в порту.";
					Link.l99.go = "exit";
				}
			}
		break;
		...
		case "shiporder1":
			if (CheckAttribute(npchar, "questTemp.chest"))
			{
				...
				link.l2.go = "exit";
				link.l3 = "Обстоятельства изменились, я хочу отменить заказ.";
				link.l3.go = "shipordercancel";
			}
			else
			{
				...
				case 2: sAdd = " Будет готов послезавтра!"; break;
				}
				if (timeleft < 1)
				{//готов
		...
		case "Blood_Shipyard1":
			...
			link.l3 = "Вы о нём слышали. Он капер... называет себя Роландо Пицарро - гроза Семи Морей и прочая, прочая...";
		...
		case "Blood_Shipyard6":
			dialog.text = "Как вы правильно подметили, я о нём слышал. Не знаю, кем был Роландо Пицарро до того, как объявился в Береговом Братстве. Но я хорошо помню его судно 'Танцующий с Волками'. Красивое название, и красивое судно... было.";
			link.l1 = "Что?! Он ни словом об этом не обмолвился... ну, в нашей с ним беседе.";
			link.l1.go = "Blood_Shipyard7";
		break;

		case "Blood_Shipyard7":
			dialog.text = "Теперь он не распускает язык - жизнь его проучила. Захватив пару богатых призов, он начал рваться в верхушку Братства, пытаясь подсидеть самого Моргана. Ну, знаете - эта идея Шарповской Либертании и всё прочее. Говорил он много и красиво, и, само собой, нашлись люди, которые поверили в эти бредни и пошли за ним... Кто ж знал, что всё так обернется...";
...
string GiveOrderMatherial(ref NPChar)
{
	...
	for (int k=1;k<10;k++)
	{
		...
		if (amount > 0) sLeft += NewStr() + XI_ConvertString(sGood1) + " - " + amount + "шт., ";
		...
		TakeNItemsWithCabin(pchar, sItem2, -amount);
		NPChar.questtemp.(sItem2) = sti(NPChar.questtemp.(sItem2)) - amount;
		amount = sti(NPChar.questtemp.(sItem2));
		if (amount > 0) sLeft += NewStr() + LanguageConvertString(idLngFile, Items[FindItem(sItem2)].name) + " - " + amount + "шт., ";
	}
		sItem2 = "chest";
		amount = GetCharacterItemWithCabin(pchar, sItem2, true) - sti(NPChar.questtemp.(sItem2));
		if (amount < 0) amount = amount + sti(NPChar.questtemp.(sItem2)); else amount = sti(NPChar.questtemp.(sItem2));
		...
		NPChar.questtemp.(sItem2) = sti(NPChar.questtemp.(sItem2)) - amount;
		amount = sti(NPChar.questtemp.(sItem2));
		if (amount > 0) sLeft += NewStr() + "Сундуков с монетами - " + amount + "шт.";
...
bool CheckOrderMatherial(ref NPChar)
{
	...
	for (int k=1;k<10;k++)
	{
		...
		sItem2 = g_ShipBermudeTuningItems[k];
		if (sti(NPChar.questtemp.(sItem2)) > 0 && GetCharacterItemWithCabin(pchar, sItem2, true) > 0) return true;
	}
		sItem2 = "chest";
		if (sti(NPChar.questtemp.(sItem2)) > 0 && GetCharacterItemWithCabin(pchar, sItem2, true) > 0) return true;
