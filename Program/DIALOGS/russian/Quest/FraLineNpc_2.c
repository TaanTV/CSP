...
//>>>>>>>>>>>>========= Разброс диалогов по персонажам =====================
			...
			if (npchar.id == "RockBrasilian" && pchar.questTemp.State == "Fr7RockBras_toSeekPlace")//квест №7, базар с Бразильцем.
            {
				...
				{
					link.l1 = HeroStringReactionRepeat("Я осмелил"+ GetSexPhrase("ся","ась") +". Зовут меня "+GetFullName(pchar)+". Я приш"+ GetSexPhrase("ёл","ла") +", чтобы вытащить тебя отсюда.", "Давай продолжим... Меня зовут "+GetFullName(pchar)+", я приш"+ GetSexPhrase("ёл","ла") +" за тобой.", "Меня зовут "+GetFullName(pchar)+", я приш"+ GetSexPhrase("ёл","ла") +" за тобой.", "Давай закончим, не вопрос. Меня зовут "+GetFullName(pchar)+", я приш"+ GetSexPhrase("ёл","ла") +" за тобой.", npchar, Dialog.CurrentNode);
				...
				{
    				dialog.text = "Кто ты?..";
    				link.l1 = "Я приш"+ GetSexPhrase("ёл","ла") +" за тобой, меня послал д'Ожерон...";
...
//********************************* Похищение донны Анны. Квест №3 **********************************
 		case "Step_F3_1":
			dialog.text = "Хосе мёртв... Видит Господь, я этого не хотела... Он нашёл письмо, которое я писала ему, и запер меня здесь...";
