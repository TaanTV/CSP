...
// ============================================================================
	switch(Dialog.CurrentNode)
	{
		...
		case "TalkInChurch_1":
            ...
            link.l1 = "Постойте, cеньора! Отчего вы так печальны? Кто этот счастливец-капитан, судьба которого так вас тревожит? Быть может, я что-то слышал о нём и смогу вам помочь?";
		...
		case "BrigAbordage_8":
            ...
            link.l1 = "Письмо? Корабль? О чём вы говорите?";
		...
		case "AmbushPortoBello_2":
			...
			link.l1 = "Именно так. Как сказал один из этой парочки: 'придушить по-тихому'. Они выложили мне всё - перепутали с капитаном брига, который вывез вас из Сан-Хуана. Сеньора, это просто счастье, что я вовремя об этом узнал и успел перехватить вас...";
		...
		case "IsabellaInCave_3":
            dialog.text = "Дальше они приволокли меня сюда. Сальватор пытался заставить меня подписать доверенности на распоряжение имуществом бедного Мигеля, ведь после его смерти всё досталось мне по завещанию.";
		...
		case "SalvatorIsDead_3":
            dialog.text = "Сеньор, делайте, что хотите, я же хочу попросту забыть обо всём, что произошло.\n"+
		...
		case "SalvatorIsDead_5":
            ...
            link.l1 = "Сеньора, это серьёзный шаг для меня. Я зарабатываю этим на жизнь. Но ради вас я готов на всё... Я согласен, Изабелла!";
		...
		case "InSanJuanAgain_3":
            dialog.text = "Хорошо, " + pchar.name + ". Тогда я скажу вам, что всё уже приготовлено к венчанию. Завтра утром я жду вас в церкви у алтаря...";
		...
		case "InSanJuanAgain_after":
			...
			link.l1 = HeroStringReactionRepeat("Ах, Изабелла, как же мне выдержать всё это?! Я хочу видеть вас сейчас...", "Изабелла, это проклятие какое-то. Я с ума сойду до завтра!",
		...
		case "NewLife_firstTime_4":
			...
			link.l1 = "Изабелла, я готов отдать всё, что имею!";
...
//************************************ новая жизнь бывшего корсара ********************************
		case "NewLife_hello":
			...
			if (iDay > 40)
			{
				...
				link.l1 = RandPhraseSimple("Изабелла, дорогая, я промотался всё это время по делам.", "Дорогая, навалилось все разом как-то...");
		...
		case "NewLife_JustTalk":
			if (CheckAttribute(pchar, "RomanticQuest.Atilla"))
			{
				switch (pchar.RomanticQuest.Atilla)
				{
					...
					break;
					case "YouAreGood":
						dialog.text = "Дорогой, ты мне ничего не говорил, но мне довелось узнать, что ты отдал своему другу Атилле очень большую сумму. Полагаю, это благодарность за всё то, что он сделал для нас. Дорогой, ты самый лучший человек на свете! Я в этом убедилась очередной раз.";
		...
		case "AngryRepeat_Budget_Pay":
            ...
            if (CheckAttribute(npchar, "angry.terms")) //примиряемся 30 дней.
            {
                if (GetNpcQuestPastDayParam(npchar, "angry.terms") > sti(npchar.angry.terms))
                {
        			...
        			link.l1 = "Хорошо, Изабелла, я сделаю всё, что ты скажешь.";
        			link.l1.go = NextDiag.TempNode;
        			CharacterDelAngry(npchar);
                }
		...
		case "AngryRepeat_KnowPirates":
            ...
            if (CheckAttribute(npchar, "angry.terms")) //примиряемся 80 дней.
            {
                if (GetNpcQuestPastDayParam(npchar, "angry.terms") > sti(npchar.angry.terms))
                {
        			...
        			link.l1 = "Хорошо, Изабелла, я сделаю всё, что ты скажешь. Пиратства больше не будет, дорогая...";
        			link.l1.go = NextDiag.TempNode;
        			CharacterDelAngry(npchar);
