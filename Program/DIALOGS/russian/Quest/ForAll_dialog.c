...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "GiveKeyMan_4":
			...
			link.l1 = "Знаешь, приятель, вот это и есть то самое, о чём я веду речь...";
		...
		break;
		case "GiveKeyMan_11":
			dialog.text = "Нет, "+ GetSexPhrase("приятель","подруга") +". Ты же сам"+ GetSexPhrase("","а") +" понимаешь, о чём просишь...";
		...
		break;
		case "PKInMarigo_1":
			dialog.text = "О нём можете не переживать, вы его уже не найдёте. Позаботьтесь об остальных.";
		...
		case "DeckSharp_2":
			...
			link.l1 = "Проблема в том, что я не знаю, где они промысел ведут. Они же очень скрытные и перемещаются постоянно! Вот, приш"+ GetSexPhrase("ёл","ла") +" к тебе за советом как к специалисту в этом деле.";
		...
		case "SharpSeekSpy_fri_2":
			...
			link.l1 = "Понимаешь, приятель, мне было очень интересно узнать, где же Шарп наводки берёт на тартаны ловцов. Теперь узнал"+ GetSexPhrase("","а") +".";
		...
		case "SCQ_ProverkaZnaniy_2":
			dialog.text = "Я назову вам фразу на латыни, а вы должны без ошибок повторить за мной. На это я дам вам 16 секунд. Если вы сумеете справиться с заданием быстрее чем за 8 секунд, то я увеличу вашу награду. Готовы?";
		...
		case "SCQ_ProverkaZnaniy_4":
			...
			{
				Dialog.text = "Отлично, вы хорошо владеете латынью, капитан. Теперь я точно уверен, что у меня получится сдать зачёт! Вот ваша награда, "+ sti(iMoney) +" пиастров.";
				...
				{
					Dialog.text = "НЕВЕРОЯТНО! Вы прекрасно владеете латынью! Теперь я замотивирован сдать зачёт, и у меня точно всё получится! Спасибо вам, "+GetSexPhrase("капитан","девушка")+", вот ваша награда, "+ sti(iMoney) +" пиастров и одна отмычка.";
					TakeNItems(PChar, "Lockpick", 1);
					Log_info("Вы получили отмычку");
		...
		case "SCQ_Pirates":
			dialog.text = "Сейчас я вам объясню. Дело в том, что мой муж, совершенно обычный человек, был захвачен пиратами! Они убивали всех, никого не щадили...";
		...
		case "FriendCap": //встреча на суше
			...
			link.l1 = TimeGreeting() + " А я наш"+ GetSexPhrase("ёл","ла") +" вас по делу.";
		...
		break;
		case "HasbandCap_2":
			dialog.text = "Ха! Ну, это понятно... Вообще-то я настолько закрутился с работой, что мне не до неё сейчас. Выгодные предложения следуют одно за другим, я не могу упускать возможную выгоду только потому, что моя жена волнуется.";
		...
		case "HasbandCap_inDeck_2":
			dialog.text = "Фу, чёрт! А я, честно говоря, испугался - решил, что это знаменитый охотник за призами - Одноглазый Стид Хантер за мной увязался. Ну, хорошо, что ошибся...\nВы знаете, я настолько закрутился с работой, что мне не до неё сейчас. Выгодные предложения следуют одно за другим, я не могу упускать возможную выгоду только потому, что моя жена волнуется.";
			link.l1 = "Это конечно, но вы могли хотя бы написать ей, что с вами всё в порядке.";
		...
		case "OffM_result":
			...
			{
				link.l1 = "Да, дружище, наш"+ GetSexPhrase("ёл","ла") +". Вот он, двуствольный мушкет, другого такого нет.";
