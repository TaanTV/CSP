...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Tichingitu_11":
			...
			dialog.text = "Отлично! Вот это дело! Сейчас я напишу записку и приложу свою печать, подожди немного\nВот, держи. Отдай это коменданту и можешь забирать своего индейца. Вот только зачем он тебе понадобился - ума не приложу. Разве что на ярмарках за деньги показывать, ха-ха!";
		...
		break;
 		...
 		case "Step_3":
            ...
            {
            	dialog.text = "У-у-у, да ты не просто капер, а грозный капер... Пош"+ GetSexPhrase("ёл","ла") +" вон отсюда!";
        		link.l1 = "...";
		...
		break;
 		...
 		case "No_Powder":
        	dialog.text = "Эй, "+ GetSexPhrase("брат","сестрёнка") +", шалишь! Нет у тебя порошка. Людей, которые пытаются меня обмануть, я не признаю больше.\n"+
                          "Так что, пош"+ GetSexPhrase("ёл","ла") +" вон отсюда!";
		...
		break;
 		...
 		case "No_Work":
        	dialog.text = "Да? Я не люблю, когда мне отказывают, таких я более знать не желаю. Поэтому - пош"+ GetSexPhrase("ёл","ла") +" вон из моего дома!";
		...
		break;
 		...
 		case "Step_10":
    		...
    		link.l1 = "Понял"+ GetSexPhrase("","а") +". Что ж, буду смотреть по сторонам. Соберу все три вещи - буду у тебя. Но кажется мне, тебя беспокоит что-то ещё, не так ли?";
		...
		case "Tichingitu_1":
			dialog.text = "У тебя на удивление хорошее чутьё. Что-ж, расскажу. Неделю назад ко мне вломились в дом несколько бандитов. Куда только стража смотрела...";
		...
		case "Tichingitu_8":
			...
			link.l1 = "Его ведь казнить собираются, а мне он может и пригодится...";
		...
		break;
 		...
 		case "Step_13":
        	dialog.text = "Слушай, ежели желаешь, заскочи ко мне через месяц-другой. Может, ещё работёнка подвернётся.";
		...
		break;
 		...
 		case "Step_15":
        	dialog.text = "Проблема следующая. Испанцы, вот уже сотню лет безжалостно истребляющие континентальных индейцев и уничтожающие их культурное наследие, вдруг озаботились сбором предметов этой самой индейской культуры.\n"+
                          ...
                          "В общем, мне удалось узнать, что в Гаване собрана большая партия предметов культа ацтеков невиданной до сей поры ценности. Я хочу получить эти предметы, понятно, что всё забрать не удастся, но хоть что-нибудь.\n"+
		...
		break;
 		...
 		case "AfterManowarsBattle":
            ...
            {
             	link.l1 = "Аскольд! Я наш"+ GetSexPhrase("ёл","ла") +" твоего друга-информатора! Это испанский офицер по имени Фернандо Гонсалес в Сантьяго. Кстати, он передавал тебе привет...";
        		link.l1.go = "Step_20";
            }
		...
		break;
 		...
 		case "Seek_Grave":
        	dialog.text = "Наш"+ GetSexPhrase("ёл","ла") +" что-нибудь?";
            ...
            if (pchar.questTemp.Ascold == "Ascold_SeekGrave")
            {
            	...
            	link.l1 = "Чёрт возьми, Аскольд, я облазил"+ GetSexPhrase("","а") +" все в округе, но ничего не наш"+ GetSexPhrase("ёл","ла") +". Абсолютно ничего.";
        		link.l1.go = "Step_36";
            }
            if (pchar.questTemp.Ascold == "Ascold_CantSeekGrave")
            {
            	dialog.text = "Что скажешь на этот раз? Наш"+ GetSexPhrase("ёл","ла") +" что-нибудь?";
            	link.l1 = "Нет, не наш"+ GetSexPhrase("ёл","ла") +". Нет здесь никакой гробницы, глупо всё это...";
        		link.l1.go = "Step_37";
            }
            if (pchar.questTemp.Ascold == "Ascold_NotEnterFoundGrave")
            {
            	dialog.text = "Как результаты поиска? Есть что-нибудь новенькое?";
            	link.l1 = "Не знаю точно, что это, но я наш"+ GetSexPhrase("ёл","ла") +" в пещере вход куда-то, но он закрыт.";
        		link.l1.go = "Step_38";
            }
            if (pchar.questTemp.Ascold == "Ascold_SeekRockLetter" && !CheckCharacterItem(pchar, "Rock_letter"))
            {
            	dialog.text = "Наш"+ GetSexPhrase("ёл","ла") +" скрижали?";
            ...
            if (pchar.questTemp.Ascold == "Ascold_SeekRockLetter" && CheckCharacterItem(pchar, "Rock_letter"))
            {
            	...
            	link.l1 = "Точно, я наш"+ GetSexPhrase("ёл","ла") +" их, каррамба!";
        		link.l1.go = "Step_45";
            }
		...
		break;
 		...
 		case "Step_38":
        	dialog.text = "Ага, кажется ты наш"+ GetSexPhrase("ёл","ла") +" то, что ищет де ла Фонтен.\n"+
		...
		break;
 		...
 		case "Found_Grave":
            ...
            {
            	dialog.text = "А, вот и ты, " + GetFullName(pchar) + "! Рассказывай быстрее, что ты наш"+ GetSexPhrase("ёл","ла") +".";
            	link.l1 = "Я сумел"+ GetSexPhrase("","а") +" открыть вход в пещеру. В пещере, в одном из ответвлений был вход в гробницу. Я наш"+ GetSexPhrase("ёл","ла") +" её.";
        		link.l1.go = "Step_46";
            }
		break;

 		case "Step_46":
        	dialog.text = "Тысяча чертей! Вот это да! Что-нибудь наш"+ GetSexPhrase("ёл","ла") +" внутри?";
		...
		break;
 		...
 		case "Step_60":
            ...
            {
            	link.l2 = "Лампу какую-то наш"+ GetSexPhrase("ёл","ла") +" восточную у этого Лейфа. Не знаю, что это такое.";
        		link.l2.go = "Step_63";
            }
		break;
