...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
 		...
 		case "EngLine_quests":
			...
			switch (pchar.questTemp.State)   // Доступ к квестам
            {
	     		case "go_talks_with_pirates":
	    			...
	    			link.l1 = "Я приш"+ GetSexPhrase("ёл","ла") +" к тебе по поручению генерал-губернатора Ямайки, сэра Томаса Мэдифорда.";
	    			link.l1.go = "Step_1";
	      		break;
            }
		break;
        ...
        break;
        case "Step_F8_5":
            dialog.text = "Понимаешь, мне всё одно выгодна вся эта заварушка. Если Мэдифорд попросит меня об одолжении, то, почитай, обвинения с меня сняты. Ну, а если ты сумеешь раздобыть документы, доказывающие, что не я устроил тот беспредел, в котором меня обвиняют - тоже хорошо. Второй вариант мне предпочтительней, так как с голландцами я портить отношения не хочу.";
