void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			link.l1 = "Извини, приятель, дел по горло. Не могу уделить тебе внимание.";
			link.l1.go = "exit";
		...
		case "Andre_Abel_7":
			dialog.text = RandPhraseSimple("Нам повезло с ветром, капитан " + GetFullName(PChar) + ".", "Форт-де-Франс - милый городишко, но за то время, пока я в нём торчал, он успел надоесть мне до чёртиков.");
...
//			LAi_ActorGoToLocatorFunc(NPChar, "reload", "reload6_back", "Andre_Abel_Quest_Delete_Andre", 15);
		...
		case "Andre_Abel_In_Tavern_4":
			dialog.text = "Скажи: так, мол, и так, Андре Абель очень сожалеет, что не принял ваше предложение, раскаивается... Скажи, что бизнес мой практически развалился и интереса для него я уже не представляю...\n"
		...
		case "Andre_Abel_In_Tavern_Quest_Complete_2":
			dialog.text = "Вот и славно. Торговая лицензия " + XI_ConvertString(Nations[sti(NPChar.LicenceNation)].Name + "Gen") + " у меня есть только на 20 дней. Не забудь, что входить в порт нужно под дружественным флагом... Да ты и сам"+ GetSexPhrase("","а")+" это прекрасно знаешь.";
