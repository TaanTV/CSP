...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "FT_2":
			dialog.text = "Верно!.. А откуда вы меня знаете?";
			link.l1 = "Ох, лучше не спрашивайте, долгая история...";
			link.l1.go = "FT_3";
		break;
		case "FT_3":
			dialog.text = "Всё-таки, я бы послушал. Понимаете, я долгое время был лишён общения с нормальными людьми. 'Касперы' - и те со мной старались не общаться. Кстати, о клане 'Каспер'. Вы что, их всех уложили?";
		...
		break;
		case "FT_8":
			dialog.text = "Да... Потом объясню. Так вот, мы познакомились, поговорили, и я сделал всё, чтобы он уплыл отсюда и рассказал людям об этом острове.";
		...
		case "FT_11":
			...
			link.l1 = "В городе хозяйничает адмирал Чад Каппер. Граждане города живут согласно положениям Закона, одно из которых запрещает строить любые плавательные средства. Ну, а я случайно узнал"+ GetSexPhrase("","а") +" о Механике и решил"+ GetSexPhrase("","а") +" побеседовать с вами любой ценой. Вот, собственно, и всё...";
			link.l1.go = "FT_12";
		break;
		case "FT_12":
			dialog.text = "Чад Каппер твёрдо держит бразды правления Городом, как и раньше...";
		...
		break;
		case "FT_17":
			dialog.text = "Хорошо. Вы, наверное, понимаете, что я знаю о Городе больше, чем любой другой житель. К тому же, теперь я не делаю предположений, я говорю всё абсолютно точно, так как изучил строение Города Потерянных Кораблей под водой.";
			link.l1 = "Под водой?";
			link.l1.go = "FT_18";
		break;
		case "FT_18":
		...
		break;
		case "FT_24":
			dialog.text = "Дело в том, что у меня есть корабль на плаву. Пару лет назад к Городу прибило штормом судно невиданной конструкции, на рынде есть занятная маркировка: 'Балтиморские верфи', но это нелепица, Балтимор просто захудалая деревенька на берегу! Откуда там верфи? Конструкция и технические характеристики корабля просто невероятны! Из обрывков судового журнала узнал, что это клипер 'Буря', порт приписки Бостон. Был я там, шумный торговый порт и таких судов я там не видел. Была пара незначительных пробоин и поломан такелаж, ничего серьёзного. Удалось довольно быстро его подлатать. Пока у меня была такая возможность...";
		...
		break;
		case "FT_25":
			dialog.text = "А-а, ну это возможность спускаться под воду в моём костюме. Дело в том, что под водой завелись здоровенные твари, которые ходят по дну и жрут всё, что попало. Вот я и не рискую теперь спускаться. Я ведь учёный, а не боец...";
		...
		break;
		case "FT_32":
			dialog.text = "Они просто не состоятся, вот и всё... В общем, передавайте Чаду от меня привет, уведомите его об уничтожении 'касперов'. Не думаю, что это его обрадует, но сделать это нужно.";
			link.l1 = "Зачем?";
			link.l1.go = "FT_33";
		break;
		case "FT_33":
		...
		case "FT_35":
			...
			link.l1 = "Понятно... Ну что же, я всё понял"+ GetSexPhrase("","а") +". Пош"+ GetSexPhrase("ёл","ла") +" заниматься делами.";
			link.l1.go = "exit";
		...
		break;
		case "FreeCorvett_1":
			dialog.text = "Я уже всё подготовил. У меня на платформе стоит лебёдка, к ней я прикрепил обломки вокруг клипера. Так что стоит только привести механизм в действие - и вуаля, корабль свободен!";
		...
		case "FreeCorvett_2":
			...
			link.l1 = "Та-ак... Если мне не изменяет память, вы говорили, что у вас есть костюм для дыхания под водой. В нём мне и предстоит осуществить спуск на дно?";
		...
		case "takeArmor":
			...
			{
				link.l1 = "Мне нужно ещё раз спуститься на дно. Не успел"+ GetSexPhrase("","а") +" сделать там всё, что хотел"+ GetSexPhrase("","а") +".";
				link.l1.go = "DeepAgain";
				if (CheckCharacterItem(pchar, "Pinion"))
				{
					link.l2 = "Я наш"+ GetSexPhrase("ёл","ла") +" шестерёнку, вот она!";
		...
		case "takeArmor2":
			...
			{
				link.l3 = "Всё, я закончил"+ GetSexPhrase("","а") +" с погружениями. Более подводное царство меня не интересует.";
		...
		break;
		case "endImmersion_1":
			dialog.text = "Теперь, когда у меня есть всё необходимое, я займусь восстановлением механизма лебёдки, которая освободит ваш клипер. Это займёт у меня определённое время. Вам же пока не следует почивать на лаврах, а стоит провести ряд важных подготовительных мероприятий.";
			link.l1 = "Что вы имеете в виду?";
			link.l1.go = "endImmersion_2";
		break;
		case "endImmersion_2":
			dialog.text = "Для начала обеспечьте корабль всем необходимым. Весь набор боеприпасов, продовольствие, оружие... Ну, не мне вас учить. Всё найденное доставляйте на 'Сан Габриэль', а переправкой этого добра на судно я займусь сам.";
			link.l1 = "Я понял"+ GetSexPhrase("","а") +", постараюсь всё сделать.";
			link.l1.go = "exit";
		...
		case "takeGoods":
			...
			{
				link.l1 = "Всё, что нужно для плавания, у меня в трюме.";
				link.l1.go = "TakeCrew";
		...
		//Набор команды
		case "TakeCrew":
			dialog.text = "Прекрасно, "+ GetSexPhrase("друг мой","дорогая моя") +", всё уже размещено в трюме вашего клипера. Теперь вам нужно набрать команду из жителей Города. Сами понимаете, вдвоём мы с кораблём не управимся.";
		...
		case "TakeCrew_2":
			...
			link.l1 = "Как и всё, что вы делаете... Так, что там по поводу нехорошего оборота дел?";
		...
		case "inPrison":
			...
			link.l1 = "Точно! Приш"+ GetSexPhrase("ёл","ла") +" на выручку.";
			link.l1.go = "inPrison_1";
		break;
		case "inPrison_1":
			dialog.text = "Ну что же, спасибо, не ожидал... Только всё зря.";
			link.l1 = "Как понять?";
		...
		break;
		case "inPrison_4":
			dialog.text = "Нет, это вряд ли. Да и не стоит рисковать клипером и жизнями стольких людей, что уже находятся на нём. Слушайте, что вам нужно сделать...";
