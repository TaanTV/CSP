...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				if (pchar.questTemp.LSC == "barmenIsDeadYouKnow" || pchar.questTemp.LSC == "toAdmiralBarmenDead")
				{
					...
					{
						dialog.Text = "Слушаю тебя внимательно.";
						link.l4 = "Н-да, похоже, Хилл действительно мёртв...";
						link.l4.go = "NBB_1";
						break;
					}
				}
				...
				{
					link.l4 = "Послушай, я так и не наш"+ GetSexPhrase("ёл","ла") +" тебя в этом твоём винном погребе...";
					link.l4.go = "TizerCellarLate";
				}
				if (pchar.questTemp.LSC == "toDiffindoor" && CheckCharacterItem(pchar, "letter_LSC"))
				{
					link.l4 = "Я обследовал"+ GetSexPhrase("","а") +" 'Диффиндур' и наш"+ GetSexPhrase("ёл","ла") +" там сундук Тизера. В нём, кроме прочего, было ещё и очень интересное письмо...";
				...
				{
					link.l4 = "Ты знаешь, всё получилось!";
		...
		break;
		case "ansewer_1":
			dialog.text = "Это Город Потерянных Кораблей, "+ GetSexPhrase("приятель","подруга") +". Если попал"+ GetSexPhrase("","а") +" сюда, то забудь обо всём, чем жил"+ GetSexPhrase("","а") +" раньше. Отсюда не вырвешься, к сожалению...";
			link.l1 = "Дьявол!!";
			link.l1.go = "int_quests";
		...
		break;
		case "TizerCellar_2":
			dialog.text = "Ты, наверное, уже в курсе того, что Закон Города запрещает строить что-либо, на чем можно отсюда уплыть. Так вот, я просто ума не приложу, как это Тизер смог построить втихаря некое корыто, да ещё и успешно на нём добраться до Большой земли.";
		...
		case "TizerCellar_7":
			...
			{
				link.l1 = "Хм, ты знаешь, на 'Диффиндуре' я уже побывал"+ GetSexPhrase("","а") +". И наш"+ GetSexPhrase("ёл","ла") +" там сундук Тизера. В нём, кроме прочего, было ещё и очень интересное письмо...";
		...
		case "TizerCellarOk_5":
			dialog.text = "Не всё так просто, "+ GetSexPhrase("дружище","" + pchar.name + "") +". Мы должны дать людям надежду и цель, а не право ломать Город кто как пожелает. С отменой положения Закона мы сразу же должны принять новый, о постройке ОДНОГО корабля для ВСЕХ!";
		...
		case "TizerCellarOk_8":
			dialog.text = "В общем, всё это мои домыслы, к ним мы вернёмся позднее. А сейчас найди Андрэ Лабора и аккуратно выясни все о его участии в деле Тизера.";
		...
		case "AfterProtector_7":
			dialog.text = "У всех членов клана 'Нарвал' есть амулет с изображением перевёрнутой пятиконечной звезды. Достань амулет и иди с ним к адмиралу. Расскажешь ему о том, что наш"+ GetSexPhrase("ёл","ла") +" его на месте убийства милиционера и плотника.";
		...
		case "NarvalRing_1":
			dialog.text = NPCStringReactionRepeat("Отлично! Теперь дуй к адмиралу и расскажи, что наш"+ GetSexPhrase("ёл","ла") +" его в каюте 'Протектора'.",
		...
		case "InterceptionBadWork_2":
			dialog.text = "Как-то я случайно столкнулся с ними у решётки погреба, когда они разговаривали. Начал наблюдать за ними и понял, что они втихаря встречаются здесь время от времени. Хотел бы я услышать, о чём они говорят...";
		...
		case "InterceptionOk_4":
			...
			link.l1 = "М-да, важней некуда... Послушай, адмирал упоминал о каком-то Механике. Кто это?";
