...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "SeekCitizen_Choice_1":
			...
			else
			{
				...
				if (sld.sex == "man")
				{
					...
					Link.l2 = "Нет, не о нём. Давай ещё раз назову.";
		...
		break;
		case "StBegin_4":
			dialog.text = "Ну, там не всё так просто. Эрик нанял трёх головорезов, они там у него постоянно живут. Мне самому с ними не справиться, а вот ты сумеешь.";
		...
		case "StResult_1":
			...
			{
				link.l1 = "Слушай, ничего я там не наш"+ GetSexPhrase("ёл","ла") +". Не было там денег. Все обыскал"+ GetSexPhrase("","а") +", а денег не наш"+ GetSexPhrase("ёл","ла") +".";
				link.l1.go = "StResult_2";
			}
			else
			{
				npchar.quest.store.money = sti(pchar.money) - sti(npchar.quest.store.money); //чистая выручка
				link.l1 = "Всего я наш"+ GetSexPhrase("ёл","ла") +" там " + FindRussianMoneyString(sti(npchar.quest.store.money)) + ". Половина, как и договаривались, твоя.";
		...
		break;
		case "StResult_3":
			dialog.text = "Ну, вот и отлично! Рад, что всё так удачно завершилось... Ну, бывай, "+ GetSexPhrase("приятель","подруга") +". Будут проблемы - обращайся.";
