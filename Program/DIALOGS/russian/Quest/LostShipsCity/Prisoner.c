...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "SeekCrew_3":
			dialog.text = "Хех, каков"+ GetSexPhrase("","а") +" "+ GetSexPhrase("провидец","предсказательница") +", просто оторопь берёт! Я порекомендую вам умерить прыть.";
		...
		break;
		case "LastLegro_2":
			dialog.text = "Да, ты прав"+ GetSexPhrase("","а") +"\nНу что же, я верю Доминику. Передай ему, что я прошу у него прощения. Похоже, что я недостойно себя вёл.";
