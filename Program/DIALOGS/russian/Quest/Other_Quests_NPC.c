void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Andre_Abel_Quest_In_Prison_5":
			...
			link.l1 = "Эх-ех, не фламберг конечно, но уж чего есть, с тем на рожон и полезем... На всякий случай, меня зовут " + GetFullName(pchar) + "... Ну, с Богом.";
		...
		case "RatOfficer_5":
			dialog.text = "Да, в море нынче небезопасно. А всё проклятые буканьеры...ик! Охотники! Да какие они охотники! На торговцев честных, таких как мы с тобой, они охотники...";
			link.l1 = "Да, ты прав, чёрт возьми! Всё из-за них, богоотступников. Эх, и как это одно"+ GetSexPhrase("му","й") +" от Тортуги до Белиза добраться? А ведь ещё по пути в залив Гибели зайти надо - хозяин велел с индейцами тамошними сделку провести - камушков драгоценных у них за бесценок прикупить, да и запасы воды пресной пополнить... Слушай, а ты часом не в том направлении курс держишь? Может...";
			link.l1.go = "RatOfficer_6";
		break;

		case "RatOfficer_6":
			dialog.text = "Нет, мы совсем...ик! Совсем в другую сторону собираемся.";
			link.l1 = "Эх, жаль. Ну, будем молиться Всевышнему, чтобы он охранял нас в пути... На пушки надежды мало - в трюме всё место товар занимает, для ядер и пороха места-то почти и не осталось... Вот она - жадность людская!";
		...
		case "Miko_3":
			dialog.text = "Ты угрожаешь мне, "+ GetSexPhrase("приятель","деваха") +"? Напрасно. Ты здесь чуж"+ GetSexPhrase("ой","ая") +", и мы всем посёлком разорвём тебя в клочья\nЧто касается моего патрона, то он сейчас совсем недалеко. Гораздо ближе, чем ты думаешь. Но тебе никогда не найти Джа - потому что ты наш"+ GetSexPhrase("ёл","ла") +" меня, ха-ха-ха!!";
		...
		case "Ja_talk_5":
			dialog.text = "Подожди... Послушай, я предложу тебе другое. Ты "+ GetSexPhrase("отчаянный и толковый корсар","отчаянная и толковая девушка") +", а я... я проиграл свою игру. У меня нет больше людей, и уже нет корабля - ты выиграл"+ GetSexPhrase("","а") +" абордаж. Даже если мне удастся тебя сейчас убить, конец всё равно один\nМожет, лучше объединим силы? Я предлагаю сменить вражду на мир и готов работать с тобой, под твоим началом.";
		...
		case "AscoldSpainCapitain":
			...
			dialog.text = "Хватит! Пожалуйста, остановитесь!";
			link.l1 = "В чём дело? Испания доверила трусу командовать линейным кораблём?";
			link.l1.go = "AscoldSpainCapitain_offer_1";
		break;	
		
		case "AscoldSpainCapitain_offer_1":
			dialog.text = "Я знаю, что тебе нужно и предлагаю сделку.";
			link.l1 = "Инвентарный список у тебя? Отдай быстро! Тогда обещаю - умрёшь ты тоже быстро.";
			link.l1.go = "AscoldSpainCapitain_offer_2";
		break;
		
		case "AscoldSpainCapitain_offer_2":
			dialog.text = "Списка у меня нет. И в тумане битвы, список может оказаться на дне. Конечно, если точно не знать где он сейчас.";
			link.l1 = "Ты знаешь на каком он корабле?! Говори!..";
			link.l1.go = "AscoldSpainCapitain_offer_3";
		break;
		
		case "AscoldSpainCapitain_offer_3":
			dialog.text = "Я скажу! Но ты оставишь меня в живых, дашь возможность забрать мои вещи, четверть миллиона и возможность сойти где-нибудь в безопасном месте, ведь мне придётся до конца дней скрываться от властей Испании.";
			if (sti(pchar.money) < 250000)
			{
			link.l1 = "У меня нет такой суммы. Да и в живых я тебя оставлять не собираюсь!";
			link.l1.go = "AscoldSpainCapitain_fight";
			}
			else
			{
			link.l1 = "Даю слово. Больше нет времени разговаривать. Где список? Корабль! Говори!";
			link.l1.go = "AscoldSpainCapitain_offer_accepted";			
			link.l2 = "К чёрту! Я сам" + GetSexPhrase("","а") + " добуду список. Список у меня на корабле, слюнтяй на дне. Так правильно. Так и будет!";
		...
		case "AscoldSpainCapitain_offer_accepted":
			...
			dialog.text = "Это " + GetStrSmallRegister(XI_ConvertString(pchar.questTemp.Ascold.ShipType)) +" '"+pchar.questTemp.Ascold.ShipName+"'.";
			if (CheckAttribute(PChar, "questTemp.Ascold.Ship.foundcheck") && !CheckCharacterItem(pchar, "ShipsJournal"))
			{
				link.l1 = "Проклятье! Я уже смотрел" + GetSexPhrase("","а") + " на нём и ничего не смог" + GetSexPhrase("","ла") + " найти на этом корабле. Значит ты вводил меня в заблуждение. Умри!";
				link.l1.go = "AscoldSpainCapitain_fight";	
			}
			else if (LAi_IsDead(CharacterFromID(pchar.questTemp.Ascold.Ship)))
			{
			link.l1 = "Проклятье! На нём уже ничего не найти... Значит твоё предложение уже не актуально. Умри!";
			link.l1.go = "AscoldSpainCapitain_fight";
			}
			else 
			{
				link.l1 = "Замечательно! Скорее на абордаж!";
		...
		case "SF_InvitingOfficer":
			...
			dialog.text = "Здравствуйте, " + GetAddress_Form(pchar) + ". Ваше имя " + pchar.name + "?";
			link.l1 = "Да, верно.";
			link.l1.go = "SF_InvitingOfficerToHuber";
		break;
		
		case "SF_InvitingOfficerToHuber":
			dialog.text = "Вас желает к себе на приём его превосходительство генерал-губернатор Порт-Рояля сэр Томас Мэдифорд. Как только закончите дела в порту, прошу незамедлительно явиться в его резиденцию.";
			link.l1 = "Я вольный капитан. Мне совершенно не интересно шататься по лощёным залам губернаторских хором.";
			link.l1.go = "SF_InvitingOfficerToHuberFirstCancel";
			link.l2 = "Хорошо, я приду.";
			link.l2.go = "SF_InvitingOfficerToHuberTaken";
		break;
		
		case "SF_InvitingOfficerToHuberFirstCancel":
			dialog.text = "Вы уверены? Когда передумаете, наверняка его превосходительство уже не будет нуждаться в вас...";
			link.l1 = "Хорошо, убедили. Я приду.";
			link.l1.go = "SF_InvitingOfficerToHuberTaken";
			link.l2 = "Абсолютно уверен" + GetSexPhrase("","а") + ".";
			link.l2.go = "SF_InvitingOfficerToHuberCancelConfirmation";
		break;
		
		case "SF_InvitingOfficerToHuberTaken":
			dialog.text = "Он будет ждать вас. До свидания, " + GetAddress_Form(pchar) + ".";
			link.l1 = "До свидания.";
		...
		case "SF_InvitingOfficerToHuberCancelConfirmation":
			dialog.text = "Отказаться, - ваше право. До свидания, " + GetAddress_Form(pchar) + ".";
			link.l1 = "До свидания.";
			link.l1.go = "SF_InvitingOfficerToHuberExit";
		break;
		...
		case "SF_InvitingOfficerToHuberWaiting":
			dialog.text = "У меня нет приказа сопроводить вас до его превосходительства, " + GetAddress_Form(pchar) + ".";
			link.l1 = "Хорошо, хорошо.";
			link.l1.go = "SF_InvitingOfficerToHuberExit";
		break;
		case "SilverSurferCaptain":
			dialog.text = "Кхм... Кхе... Кхе...";
			link.l1 = "Живой ещё? Я не могу ждать пока ты выхаркаешь свое последнее слово.";
		...
		case "SilverSurferCaptainThreat":
			dialog.text = "Тебе не уйти отсюда... Кхе... Я всё предусмотрел. Сейчас мои люди пленят тебя, и мы выйдем с тобой из этой битвы на моём корабле. Не дури, иначе ты здесь просто сдохнешь.";
			link.l1 = "Ну... сначала, сдохни ты!";
		...
		case "SilverSurferCaptainTrap":
			dialog.text = "Капитан! Это ловушка! Они специально ждали, пока мы заберёмся подальше, и повылазили из своих нор. Внушительный отряд, подозреваю, что их лучших бойцов, заблокировал нас здесь. Уже идёт бой!";
			link.l1 = "Держитесь! Мы выберемся! Мы их всех перебъём!";
