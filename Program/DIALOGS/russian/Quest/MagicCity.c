...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "CaracasWervolf_2":
			...
			link.l1 = ""+ GetSexPhrase("Я погубил столько душ, что помощь в спасении хотя бы одной, пожалуй, мне зачтётся.","Хорошо.") +" Ждите здесь, постарайтесь продержаться!";
			link.l1.go = "exit";
			npchar.lifeDay = 0;
		...
		case "DMCitizHead":
			...
			link.l1 = "Да так, посмотреть, что к чему... Скажите, у вас всё нормально в городе?";
			...
			if (pchar.questTemp.MC == "toSeekSkull")
			{
				...
				link.l1 = "Я наш"+ GetSexPhrase("ёл","ла") +" Элизабет Шеппард.";
				link.l1.go = "FoundLiz";
			}
			...
			if (pchar.questTemp.MC == "over")
			{
				dialog.text = "Это вы сняли проклятие? В какой-то момент у нас всё нормализовалось!";
				link.l1 = "Да. Я наш"+ GetSexPhrase("ёл","ла") +" галеон 'Королева' и всё сделал"+ GetSexPhrase("","а") +". Рад"+ GetSexPhrase("","а") +", что сумел"+ GetSexPhrase("","а") +" вам помочь.";
		...
		break;
		case "DMCitizHead_1":
			dialog.text = "Всё хорошо. А почему вы спрашиваете?";
			link.l1 = "Вы знаете, когда я высадил"+ GetSexPhrase("ся","ась") +" у причала, то на меня напали какие-то люди. Совершенно непонятно почему.";
			link.l1.go = "DMCitizHead_2";
		break;
		case "DMCitizHead_2":
			dialog.text = "Ничего не могу вам сказать по этому поводу. Я не в курсе, кто это был.";
			link.l1 = "Понятно... Так, вы говорите, в Дес-Мойнесе всё спокойно.";
		...
		break;
		case "DMHeadOver_1":
			dialog.text = "Двадцать тысяч пиастров! Может быть для вас это немного, но наш город за проклятое время здорово обеднел, так что это всё, что у нас есть.";
		...
		break;
		case "DMSkelHead_1":
			dialog.text = "Расскажу, если вы этого желаете! Всё равно вы с этой историей далеко не уйдёте.";
			link.l1 = "Я не из пугливых, так что нечего меня стращать. Рассказывайте.";
			link.l1.go = "DMSkelHead_2";
		break;
		case "DMSkelHead_2":
			dialog.text = "Как-то причалил к нашему берегу пиратский галеон 'Королева'. Пираты здесь рабов контрабандистам продали. Договариваются между собой\n"+
				"Продали пираты рабов, а потом у них между собой ссора какая-то вышла. Часть команды здесь осталась, а остальные с капитаном на галеоне в море ушли. С той поры всё и началось.";
			link.l1 = "Да что началось? Говорите вы толком!";
			link.l1.go = "DMSkelHead_3";
		break;
		case "DMSkelHead_3":
			dialog.text = "Да вот то, что видите. Днём все люди как люди, а как солнце сядет - такой вот маскарад. Сообразили, что всё из-за пиратов этих, будь они прокляты! И хоть они тоже по ночам скелетами оборачивались, но всё равно - всех, кто у нас оставался, в одну прекрасную ночь на костёр отправили. Не помогло...";
		...
		break;
		case "DMSkelHead_6":
			dialog.text = "Вас сюда честно не пускала охрана у причала. Но вы всё равно прорвались к нам. Так что пеняйте только на себя.";
		...
		break;
		case "DMSkelHead_7":
			dialog.text = "Пожалуй, что так\nВпрочем, силком вас держать никто не будет. Всё равно вашим рассказам никто не поверит, а если поверят, пришлют Инквизицию с сотней солдат... и жизнь целого города будет на вашей совести. Впрочем, видно, вам-то не привыкать...";
		...
		case "LizBegin":
			...
			link.l1 = "Здравствуйте, Элизабет. Меня зовут " + GetFullName(pchar) + ". Я приш"+ GetSexPhrase("ёл","ла") +" переговорить с вами об одном... очень необычном деле.";
		...
		case "LizBegin_12":
			...
			link.l1 = "Элизабет, так что же произошло в Дес-Мойнесе? Почему это проклятие пало на жителей города?";
		...
		case "LizBegin_SeekSkull":
			...
			link.l1 = "Понятно. Если мне удастся раздобыть этот череп, то я первым делом отправлюсь сюда, в Чарльзтаун.";
		...
		break;
		case "DanAbordage_5":
			dialog.text = "Так вот кто это был! А я до сих пор и не знала, что это за чучело\nВы, наверное, знаете, что я дочь капитана Стейтона. Несмотря на то, что являюсь женщиной, я сумела отстоять своё право быть капитаном 'Королевы'.\n"+
				"Как-то раз в Шарп-Тауне я наняла шкипера, по имени Пьер Кабаль. Это было ошибкой, такого негодяя свет ещё не видывал. У берегов Санта-Каталины он схватил какого-то молодого индейца. К сожалению, я узнала об этом, когда мы уже подходили к Маракайбо. Негодяй решил продать его, а деньги забрать себе. Мотивировал он это тем, что индеец - лично его законная добыча.\n"+
				"Понятно, что такого беспредела я допустить не могла. За городскими воротами Дес-Мойнеса, в джунглях, во время заключения сделки, я заявила, что или мы продаём индейца на общих основаниях, или я отпускаю его. В итоге, пришлось отпустить. Но этот негодяй Кабаль выхватил пистолет и выстрелил индейцу в спину. Казалось, наш поединок было неминуем.\n"+
		...
		case "DanAbordage_11":
			...
			link.l1 = "Думаю, что нет. Всё кончилось.";
