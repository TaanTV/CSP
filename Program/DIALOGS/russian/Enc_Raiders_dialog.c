void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First Time":
			...
			Link.l1 = LinkRandPhrase(LinkRandPhrase("Что ты несёшь?","В чём дело?", "О чём это ты?"), "Что ты имеешь в виду?", "К чему это ты?");
			Link.l1.go = "Node_2";
		...
		case "Node_2":
			...
			Link.l2 = LinkRandPhrase(LinkRandPhrase("Денег захотел? Так подойди и возьми, а я погляжу, сколько пенсов ты стоишь.",
				...
				RandPhraseSimple("А с чего ты решил, что я стану перед тобой кошелём трясти?",
