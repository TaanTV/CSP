void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		//ОЗГ
			...
			else
			{
				...
				if (CheckAttribute(pchar,"contratrade.time"))
				{
					...
					{
						Link.l17 = "(негромко) Я слышал"+ GetSexPhrase("","а") +", что у вас можно купить кое-что интересное.";
						Link.l17.go = "Trade";
					}
				}
				...
				{
					Link.l18 = "Я бы хотел"+ GetSexPhrase("","а") +" пообщаться по поводу мушкетёра, живущего отшельником на соседнем острове.";
					Link.l18.go = "Rimalie";
				}
		...
		if (!CheckAttribute(pchar, "GenQuest.contraTravel.active") && !CheckAttribute(Pchar, "quest.Contraband.Active"))
		{
			...
			link.l1 = "Нет там рабов, а у меня дело горит. Времени совсем не осталось.";
