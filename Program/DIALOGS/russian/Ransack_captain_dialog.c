	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (attrLoc != "none" && FindColony(attrLoc) != -1)
			{
                ...
                link.l3 = "Мы сейчас близ города " + GetConvertStr(rColony.id +" Town", "LocLables.txt") + ". Я "+ GetSexPhrase("мог","могла") +" бы вас тут высадить.";
		...
		case "second time":
			...
			else
			{
				...
				{
					rColony = GetColonyByIndex(FindColony(attrLoc));
					link.l4 = "Мы сейчас близ города " + GetConvertStr(rColony.id +" Town", "LocLables.txt") + ". Я "+ GetSexPhrase("мог","могла") +" бы вас тут высадить.";
