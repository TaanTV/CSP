...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (pchar.questTemp.MC == "toCaracasPadre" && npchar.city == "Caracas")
			{
				...
				link.l1 = "Наш"+ GetSexPhrase("ёл","ла") +". Но сейчас не время разговаривать, позже всё расскажу...";
				link.l1.go = "exit";
				break;
			}
			if (pchar.questTemp.MC == "toCaracasTavern" && npchar.city == "Caracas")
			{
				...
				link.l1 = "Что случилось с этим вашим постояльцем?";
			...
			if(NPChar.quest.meeting == "0")
			{
				...
				dialog.Text = "Добро пожаловать, " + GetAddress_Form(NPChar) + ". Еда, выпивка и развлечения - всё по умеренным ценам!";
	...
	////////////////////////////////////////////////////////=============================================///////////////////////////////////////////////////////
        case "int_quests":
			...
			{
				link.l11 = "Я совершенно случайно неподалёку наш"+ GetSexPhrase("ёл","ла") +" судовые документы....";
		...
		case "ShipLetters_4":
			...
			{
				dialog.text = "Дайте-ка взглянуть! О! Судя по имени, они принадлежат одному моему постоянному посетителю, весьма достойному человеку. Я могу сам"+NPCharSexPhrase(NPChar,"","а")+" передать документы владельцу.";
				link.l1	= "Пожалуй, нет ...";
				link.l1.go = "exit";
		...
		if (sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)
		{
			...
			if (!CheckAttribute(PChar, "QuestTemp.AndreAbelQuest_StartQuest") && PChar.location == "FortFrance_tavern" && sti(pchar.rank) >= iOrionRankReq)
			{
				...
				link.l1 = "Спасибо, вот я и наш"+GetSexPhrase("ёл","ла")+" работу.";
				link.l1.go = "exit";
        ...
        case "ConvoyAreYouSure":
		    dialog.text = LinkRandPhrase("Да кто ж его знает. Ты спросил"+ GetSexPhrase("","а") +" - я ответил.", "Ну, это уж тебе само"+ GetSexPhrase("му","й") +" решать. Я бы тут ни за кого не поручил"+NPCharSexPhrase(NPChar,"ся","ась")+".", "Решай сам"+ GetSexPhrase("","а") +", ко мне всякие захаживают. Это же таверна, а не резиденция губернатора.");
		...
		case "w_find_desouza_2":
			dialog.text = "А вот теперь мы подошли к главной причине, почему я тебе всё это рассказывал. Ясное дело, такая информация денег стоит. Десять тысяч.";
