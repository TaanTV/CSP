...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "28":
			...
			SetCameraDialogMode(CharacterFromID(sCharacter));
			dialog.text = "" + characters[GetCharacterIndex(sCharacter)].name + ", сбегай к капитану, одолжи у него кошелёк.";
			link.l1 = "...";
			link.l1.go = "29";
		...
		case "42":
			if(sti(PChar.GenerateShipWreck.ToSquadronWithMoney) == 1)
			{
				...
				SetCameraDialogMode(CharacterFromID(sCharacter));
				dialog.text = "Мы согласны. " + characters[GetCharacterIndex(sCharacter)].name + ", ты знаешь что делать.";
		...
		case "NewCap_2":
			dialog.text = "Меня зовут " + GetFullName(NPChar) + ". Я настоящий капитан корабля. Около месяца назад нас абордировало пиратское судно. Я бы не разговаривал с вами сейчас, если бы вовремя не подоспел военный патруль. Мы кое-как отбились, а вражеский корабль на всех парусах уплыл\nДогнать его патрулю не удалось. Я торговец, постоянно перевожу товары. В одном из портов мне пришлось нанять новую команду. Из старых членов осталось не больше тридцати... А теперь нас осталось всего " + sti(PChar.GenerateShipWreck.PrisonedCrew) + " человек. Неделю назад я понял, что нанял отпетых негодяев\nВ одну из ночей, накачавшись ромом, они сбили замок на арсенале и, вооружившись, начали громить корабль, попутно убивая всех, кто им встретится. Не знаю, как так получилось, но они сами срубили мачты и подорвали несколько бочонков с порохом. Хорошо, что пробоины были небольшие. Протрезвев, они загнали нас в эту каюту. Что было дальше, вы понимаете сами.";
