...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "hired":
			...
			{
				Link.wChinaSword = "Посмотри, что у меня для тебя есть. Было непросто, но всё же я достала твой меч.";
		...
		case "cancelcontract1":
			dialog.text = "Довольно неожиданно, капитан. И в чём же я " + NPCharSexPhrase(NPChar,"провинился?","провинилась?");
	...
	case "WantToRemove":
		...
		case "Companion_Tasks":
			...
			// {
			    Link.l3 = "Я хочу, чтобы ты на время выш"+ NPCharSexPhrase(NPChar,"ел","ла") +" из состава моей эскадры и поискал"+ NPCharSexPhrase(NPChar,"","а") +" удачу самостоятельно.";
		...
		case "CompanionTravel":
			if(GetAttrValue(PChar, "CompanionTravel") == 3) // Если уже троих отпустил
			{
				dialog.Text = "Капитан, я не выйду из состава вашей эскадры, так как вы и так уже отпустили три корабля.";
					Link.l1 = "Да, пожалуй ты прав"+NPCharSexPhrase(NPChar,"","а")+".";
					Link.l1.go = "exit";
				break;
			}
		...
		case "TravelToNomanIslands":
			dialog.Text = "К какому именно?";
				...
				Link.l5 = "Знаешь, пока останься, я передумал"+ GetSexPhrase("","а")+".";
				Link.l5.go = "exit";
		break;
		...
		case "TravelToCaiman":
			...
			dialog.Text = "И так, Вы хотите чтоб я отправил"+NPCharSexPhrase(NPChar,"ся","ась")+" до "+XI_ConvertString("Caiman" + "Gen")+", капитан?";
		...
		case "TravelToLucia":
			...
			dialog.Text = "И так, Вы хотите чтоб я отправил"+NPCharSexPhrase(NPChar,"ся","ась")+" до "+XI_ConvertString("StLucia" + "Gen")+", капитан?";
			Link.l1 = "Да, именно так.";
            Link.l1.go = "CompanionTravel_PrepareStart";
			Link.l2 = "Нет, я передумал"+ GetSexPhrase("","а")+", не бери в голову.";
            Link.l2.go = "exit";
		...
		case "TravelToDominica":
			...
			dialog.Text = "И так, Вы хотите чтоб я отправил"+NPCharSexPhrase(NPChar,"ся","ась")+" до "+XI_ConvertString("Dominica" + "Gen")+", капитан?";
			Link.l1 = "Да, именно так.";
            Link.l1.go = "CompanionTravel_PrepareStart";
			Link.l2 = "Нет, я передумал"+ GetSexPhrase("","а")+", не бери в голову.";
            Link.l2.go = "exit";
		...
		case "TravelToTerks":
			...
			dialog.Text = "И так, Вы хотите чтоб я отправил"+NPCharSexPhrase(NPChar,"ся","ась")+" до "+XI_ConvertString("Terks" + "Gen")+", капитан?";
			Link.l1 = "Да, именно так.";
            Link.l1.go = "CompanionTravel_PrepareStart";
			Link.l2 = "Нет, я передумал"+ GetSexPhrase("","а")+", не бери в голову.";
            Link.l2.go = "exit";
		...
		case "TravelToLaVega":
			...
			dialog.Text = "И так, Вы хотите чтоб я отправил"+NPCharSexPhrase(NPChar,"ся","ась")+" до "+XI_ConvertString("Colony" + characters[sti(NPChar.realcompanionidx)].CompanionTravel.ToColonyID + "Gen")+", капитан?";
			Link.l1 = "Да, именно так.";
            Link.l1.go = "CompanionTravel_PrepareStart";
			Link.l2 = "Нет, я передумал"+ GetSexPhrase("","а")+", не бери в голову.";
            Link.l2.go = "exit";
		...
		case "CompanionTravel_NoFood":
			dialog.Text = "Кэп, для такого путешествия у меня на борту маловато провианта.";
				Link.l1 = "Да, ты прав"+NPCharSexPhrase(NPChar,"","а")+". Путешествие подождёт, пока-что...";
				Link.l1.go = "exit";
		break;
		...
		case "CompanionTravel_NoHP":
			dialog.Text = "Кэп, но мой корабль в плохом состоянии. Мне не разумно выходить в море одному при таких обстоятельствах!";
				Link.l1 = "Да, ты прав"+NPCharSexPhrase(NPChar,"","а")+". Твой корабль нуждается в ремонте.";
				Link.l1.go = "exit";
		break;
		...
		case "CompanionTravel_ToSquadron":
			...
			{
				dialog.text = "Кэп, у вас же и так эскадра из восьми кораблей, куда-же девятый ещё? В общем, я буду ждать вас тут, на своём корабле - все эти ваши новые компаньоны не надёжны - их судна очень скоро пойдут ко дну и вы за мной вернетесь.";
		...
		case "ColonyGuarding_Hovernor_AutoCrewHire_On_Colony_1":
			...
			link.l2 = LinkRandPhrase("Мм... Пожалуй, ты прав"+ NPCharSexPhrase(NPChar,"","а") +" - стоит подумать ещё.", "Я передумал"+ GetSexPhrase("","а") +".", "Эээ... Нет, не уверен"+ GetSexPhrase("","а") +".");
			link.l2.go = "exit";
		break;
