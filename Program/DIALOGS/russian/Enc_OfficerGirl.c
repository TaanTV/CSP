...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Yoko_1":
			...
			dialog.Text = "Погуляла я вчера ночью в местной таверне, а утром оказалось, что пропали все деньги и моя сторта. Если сумеете их найти и вернуть мне мой меч - я пойду в вашу команду.";
		...
		case "Helen_meet":
			...
			if (sti(pchar.reputation) > 70)
			{
				...
				Link.l1 = "Ты прежде была капитаном? Значит, должна разбираться хоть понемногу во всём, так?";
				Link.l1.go = "Helen";
			}
			else
			{
		...
		break;
		case "Helen_1":
			dialog.Text = "Есть здесь один мерзавец, даже не знаю, почему его тут все терпят... Так вот, он распускает обо мне всякие грязные слухи, из-за чего никто не желает брать меня к себе.";
