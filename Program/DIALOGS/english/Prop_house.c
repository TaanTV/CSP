		...
		case "HouseMan":
			...
			else
			{
			    ...
			    if(!CheckAttribute(NPChar, "quest.propositioned") || NPChar.quest.propositioned == "0")
                {
                    ...
                    if (!CheckAttribute(pchar, "questTemp.different.Church_NightGuard") && pchar.questTemp.different == "free" && !CheckAttribute(pchar, "questTemp.different.FackHouse") && !isBloodPrologue)
                    {
