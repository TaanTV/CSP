...
bool LAi_CharacterLogin(aref chr, string locID)
{
	...
	if (GetArraySize(&LAi_loginedcharacters) < MAX_CHARS_IN_LOC)
	{
		trace("arraysize "+GetArraySize(&LAi_loginedcharacters));
...
void LAi_CharacterPostLogin(ref location)
{
	...
	if(!actLoadFlag)
	{
		...
		if(bDisableLandEncounters || !CheckAttribute(pchar,"quest.SimulatePGGLife") || isBloodPrologue)
		{
			pchar.quest.SimulatePGGLife = true;
			Log_TestInfo(GetConvertStrWithReplace("Variable_LAi_login_1", "Logs.txt", "#space#", " "));
		}
		else
		{
			SimulatePGGLife();
...
void SimulatePGGLife()
{
	if (findsubstr(pchar.location, "_town", 0) != -1 || findsubstr(pchar.location, "_ExitTown", 0) != -1 || findsubstr(pchar.location, "_Bank", 0) != -1 || findsubstr(pchar.location, "_store", 0) != -1 || findsubstr(pchar.location, "_Shipyard", 0) != -1 || findsubstr(pchar.location, "_church", 0) != -1 || findsubstr(pchar.location, "_SecBrRoom", 0) != -1 || findsubstr(pchar.location, "_PortOffice", 0) != -1 || findsubstr(pchar.location, "_Fort", 0) != -1 || findsubstr(pchar.location, "_Dungeon", 0) != -1)
	{
		...
		if(heroSelected == 0)
		{
			...
			{
				Log_TestInfo(GetConvertStrWithReplace("Variable_LAi_login_2", "Logs.txt", "#space#", " "));
			}
			else
			{
				Log_TestInfo(GetConvertStrWithReplace("Variable_LAi_login_3", "Logs.txt", "#space#", " "));
				...
				if (findsubstr(pchar.location, "_town", 0) != -1)
				{
					...
					{
						chr = GetCharacter(NPC_GenerateCharacter("PGGOfficer" + i, "officer_" + (rand(42) + 1), "man", "man", sld.rank, PIRATE, 0, true));
...
void GenerateSpySeeker(ref location)
{
	...
	if(HasSubStr(location.id, "Common") && rand(1000) > 750 && pchar.dayrandom != pchar.SpySeeker.dayrandom && !HasSubStr(location.id, "Crypt") && GetCityNation(location.fastreload) != 4 && Characters[GetFortCommanderIDX(colonies[findcolony(location.fastreload)].id)].fort.mode != FORT_DEAD)
	{
		if(bOK || bOK2)
		{
			if (MOD_SKILL_ENEMY_RATE == 3 && bHardAnimations) ref rChar = GetCharacter(NPC_GenerateCharacter("SpySeeker", "officer_"+ (1 + drand(42)), "man", "man_fast", pchar.rank, GetCityNation(location.fastreload), -1, false)); //LEO: Превозмогаторам страдание 08.12.2021
			else rChar = GetCharacter(NPC_GenerateCharacter("SpySeeker", "officer_"+ (1 + drand(42)), "man", "man", pchar.rank, GetCityNation(location.fastreload), -1, false));
