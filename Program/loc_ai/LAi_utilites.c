...
void CreateCitizens(aref loc)
{
	...
	if (checkAttribute(loc, "citizens") || loc.type == "church")
	{
		...
		if (loc.type == "port")
		{
			iCitizensQuantity = rand(4) + 4;
			iSailorQty = rand(2) + 1;
			if (loc.id == "FortOrange_port") iCitizensQuantity = rand(4) + 2;
		}
	...
	if (checkAttribute(loc, "soldiers") && CheckAttribute(loc, "locators.patrol"))
	{
		...
		if (loc.type == "port")
		{
			iCitizensQuantity = rand(2)+4;
			if (loc.id == "PuertoPrincipe_port") iCitizensQuantity = rand(2)+2;
		}
		...
		for (i=1; i<=3; i++)
		{
			...
			if (CheckAttribute(loadedLocation,"fastreload"))
			{
				if (loadedLocation.fastreload == "Caiman")
				{
					...
					{
						if(PChar.ColonyBuilding.SoldiersType == "officer") { sType = "officer_" + (rand(42) + 1)+ "_mush";}
...
/// <--- Navarra
				LAi_group_MoveCharacter(chr, slai_group);
				chr.Name = GetConvertStrWithReplace("Variable_LAi_utilites_1", "Names.txt", "#space#", " ");
				chr.LastName = "";
...
/// <--- Navarra
			...
			if (!IsCharacterPerkOn(pchar, "Adventurer"))
			{
				...
				{ // карты кладов
					Log_TestInfo(GetConvertStrWithReplace("Variable_LAi_utilites_1", "Logs.txt", "#space#", " "));
			...
			if (IsCharacterPerkOn(pchar, "Adventurer"))
			{
				...
				{ // карты кладов
					Log_TestInfo(GetConvertStrWithReplace("Variable_LAi_utilites_2", "Logs.txt", "#space#", " "));
					iChar = NPC_GeneratePhantomCharacter("pirate", iNation, MAN, 1);
					chr = &characters[iChar];
					SetNPCModelUniq(chr, "pirate", MAN);
					chr.City = Colonies[iColony].id;
	...
	if (loc.id == "Reefs_chapter")
	{
		if (CheckNPCQuestDate(loc, "Reefs_Guard_date"))
		{
			...
			for (int j=1; j<=6; j++)
			{
				...
				else	sld = GetCharacter(NPC_GenerateCharacter("MaltGuard_"+j, "officer_" + (rand(42)+1), "man", "man", 35, PIRATE, 1, true));
				sld.City = "Santiago";
	...
	if (CheckAttribute(loc, "pearlVillage") && sti(loc.pearlVillage) == true)
	{
		...
		{//если не завалил часового
			if (characters[iTemp].location == "none")
			{
				...
				Log_SetStringToLog(GetConvertStrWithReplace("Variable_LAi_utilites_3", "Logs.txt", "#space#", " "));
				return;
			}
			else
		...
		chr.name = LinkRandPhrase(GetConvertStrWithReplace("Variable_LAi_utilites_2", "Names.txt", "#space#", " "), GetConvertStrWithReplace("Variable_LAi_utilites_3", "Names.txt", "#space#", " "), GetConvertStrWithReplace("Variable_LAi_utilites_4", "Names.txt", "#space#", " "));
		chr.lastname = "";
		chr.CityType = "citizen";
		...
		if (loc.id == "Pearl_town_1" && !CheckAttribute(pchar, "GenQuest.GhostShip.LastBattle"))
		{
			...
			chr.name = GetConvertStrWithReplace("Variable_LAi_utilites_5", "Names.txt", "#space#", " ");
			chr.lastname = GetConvertStrWithReplace("Variable_LAi_utilites_6", "Names.txt", "#space#", " ");
			chr.CityType = "citizen";
...
int GetCityNation(string _sColony) // boal метод вернёт нацию по ид города
{
