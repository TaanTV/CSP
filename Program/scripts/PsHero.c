	...
	if (CheckAttribute (chr, "PGGAi.Task") && chr.PGGAi.Task != PGG_TASK_NOTASK)
	{
		...
		if (sTask == PGG_TASK_SAILTOISLAND || sTask == PGG_TASK_WORKCONVOY || sTask == PGG_TASK_WORKONSTORE)
		{
			if (CheckAttribute(chr, "PGGAi.Task.days") && sti(chr.PGGAi.Task.Target.days) <= GetNpcQuestPastDayParam(chr, "PGGAi.Task.days"))
			{
				...
				//Log_TestInfo(GetHeroName(sti(chr.PGGAi.HeroNum))+GetConvertStrWithReplace("Variable_PsHero_2", "Logs.txt", "#space#", " ") +
...
*/
	...
	//выжил или нет
			...
			{
				Log_TestInfo(chr.id + GetConvertStrWithReplace("Variable_PsHero_3", "Logs.txt", "#space#", " "));
			}
			else
			{
				if (IsOfficer(chr) || IsCompanion(chr))
				{
					Log_TestInfo(chr.id + GetConvertStrWithReplace("Variable_PsHero_4", "Logs.txt", "#space#", " "));
				}
				else
				{
					if (CheckAttribute(chr, "willDie"))
					{
						Log_TestInfo(chr.id + GetConvertStrWithReplace("Variable_PsHero_5", "Logs.txt", "#space#", " "));
					}
					else
					{
						chr.PGGAi.Rebirth = true;
						Log_TestInfo(chr.id + GetConvertStrWithReplace("Variable_PsHero_6", "Logs.txt", "#space#", " "));
	...
	if (!CheckAttribute(chr, "DontCountDeath"))
	{
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_PsHero_7", "Logs.txt", "#space#", " ")+ pchar.PGG_killed);
	}
	else
	{
	...
	if (sTask == PGG_TASK_WORKONSTORE || sTask == PGG_TASK_SAILTOISLAND || sTask == PGG_TASK_WORKCONVOY)
	{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					if (chr.PGGAi.Task == PGG_TASK_WORKCONVOY && sti(chr.nation) != PIRATE && !CheckAttribute(chr, "PGG_warrior"))
					{
						...
						Log_TestInfo(GetHeroName(sti(chr.PGGAi.HeroNum))+GetConvertStrWithReplace("Variable_PsHero_8", "Logs.txt", "#space#", " "));
						chr.mapEnc.Name = GetConvertStrWithReplace("Variable_PsHero_1", "Names.txt", "#space#", " ") + GetHeroName(sti(chr.PGGAi.HeroNum));
						chr.PGG_Convoy = true;
						chr.PGG_trader = true;
					}
			...
			if (CheckAttribute(chr, "PGG_Hunter"))
			{
				...
				chr.mapEnc.Name = chr.mapEnc.Name + GetConvertStrWithReplace("Variable_PsHero_2", "Names.txt", "#space#", " ");
			...
			if (CheckAttribute(chr, "PGG_warrior"))
			{
				...
				Log_TestInfo(GetHeroName(sti(chr.PGGAi.HeroNum))+GetConvertStrWithReplace("Variable_PsHero_9", "Logs.txt", "#space#", " "));
				chr.PGG_Grudge = true;
			}
			else
			{
				...
				Map_CreateTrader(chr.PGGAi.location.town, sColony, chr.id, daysQty);
				//Log_TestInfo(GetHeroName(sti(chr.PGGAi.HeroNum))+GetConvertStrWithReplace("Variable_PsHero_10", "Logs.txt", "#space#", " ") +
...
void PGG_UpdateShipEquip(ref chr)
{
	...
	if (chr.HeroParam.HeroType == "Trader")
	{
...
string PGG_FindTargetTown(ref chr)
{
    ...
    Log_TestInfo("PGG_FindTargetTown: для " + chr.id + GetConvertStrWithReplace("Variable_PsHero_11", "Logs.txt", "#space#", " ") + sTargetTown + GetConvertStrWithReplace("Variable_PsHero_12", "Logs.txt", "#space#", " ") + i);
...
string SelectRandomPGG(string sex, string animation)
{
	...
	if (numPGG > 0)
	{
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_PsHero_13", "Logs.txt", "#space#", " ")+ GetFullName(sld));
		return sld.id;
	}
	Log_TestInfo(GetConvertStrWithReplace("Variable_PsHero_14", "Logs.txt", "#space#", " "))
	return "";
}
...
string SelectWhisperPGG()
{
	...
	{
		sld = CharacterFromID("PsHero_"+i);
		if (sld.name == GetConvertStrWithReplace("Variable_PsHero_4", "Names.txt", "#space#", " "))
		{
			return sld.id;
		}
	}
