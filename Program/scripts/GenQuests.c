...
void ContraDeliverQuest_ReloadConterToRoom(string sQuest) // генерим клона контрабандиста
{
	...
	rChar.Name = GetConvertStrWithReplace("Variable_GenQuests_1", "Names.txt", "#space#", " ");
	rChar.LastName = "";
...
void ContraDeliverQuest_GeneratePatrolToRoom()
{
	...
	for(int i=0; i<=2; i++)
	{
		...
		SetFantomParamFromRank(rChar, sti(PChar.rank)*2, true);
		rChar.Name = GetConvertStrWithReplace("Variable_GenQuests_2", "Names.txt", "#space#", " ");
		rChar.LastName = "";
...
void ContraMeetManQuest_InitStartParam() // Инициализация стартовых параметров
{
	if(rand(1) == 0)
	{
		Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_1", "Logs.txt", "#space#", " "));
		PChar.GenQuest.ContraMeetMan.SoldiersInShore = true;
	}
	else
	{
		Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_2", "Logs.txt", "#space#", " "));
		if(rand(1) == 0)
		{
			Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_3", "Logs.txt", "#space#", " "));
			PChar.GenQuest.ContraMeetMan.ManIsTraitor = true;
		}
		else Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_4", "Logs.txt", "#space#", " "));
	}
}
	...
	else // Солдеры
	{
		...
		for(int i=0; i<=iSoldCount; i++)
		{
			...
			if(i==0) // Офицер
			{
				...
				SetFantomParamFromRank(rChar, sti(PChar.rank)*2, true);
				rChar.Name = GetConvertStrWithReplace("Variable_GenQuests_3", "Names.txt", "#space#", " ");
				rChar.LastName = "";
			}
			else // Солдаты
			{
				...
				rChar.Name = GetConvertStrWithReplace("Variable_GenQuests_4", "Names.txt", "#space#", " ");
				rChar.LastName = "";
			}
...
void ContraMeetManQuest_ReloadConterToRoom(string sQuest)
{
	...
	for(int i=0; i<=1; i++)
	{
		...
		SetFantomParamFromRank(rChar, sti(PChar.rank)*2, true);
		rChar.Name = GetConvertStrWithReplace("Variable_GenQuests_5", "Names.txt", "#space#", " ");
		rChar.LastName = "";
		rChar.Greeting = "Gr_Smuggler Agent";
		LAi_SetActorType(rChar);
		rChar.Dialog.FileName = "GenQuests_Dialog.c";
...
void ContraMeetManQuest_CheckContraDead()
{
	if(LAi_isDead(CharacterFromID("ContraMeetManQuest_Contrick_0")) && LAi_isDead(CharacterFromID("ContraMeetManQuest_Contrick_1")))
	{
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_5", "Logs.txt", "#space#", " "));
...
void CitizenFindItemMiniQuest_CheckItem()
{
	if(CheckCharacterItem(PChar, "DOjeronRing")) // Проверка незначительная - тормозов не будет
	{
		...
		Log_Info(GetConvertStrWithReplace("Variable_GenQuests_6", "Logs.txt", "#space#", " ") + sCity + GetConvertStrWithReplace("Variable_GenQuests_7", "Logs.txt", "#space#", " "));
	}
}
...
void Church_GenQuest1_Timer(string sQuest)
{
	...
	items[FindItem("Bible")].Name = "itmname_bible"; // Вернёт аттрибуты
...
{
	Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_8", "Logs.txt", "#space#", " "));
...
void Church_GenQuest2_DeadBandits(string sQuest)
{
	if(CheckAttribute(PChar, "GenQuest.ChurchQuest_2.WayOfShortVariant"))
	{
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_9", "Logs.txt", "#space#", " "));
	}
...
{
	Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_10", "Logs.txt", "#space#", " "));
...
void ReasonToFast_GenerateHunter() // генерация ОЗГ
{
	...
	chr = GetCharacter(NPC_GenerateCharacter("ReasonToFast_Hunter", "officer_"+(rand(42)+1), "man", "man", iRank, PIRATE, -1, true));
...
void ReasonToFast_GenerateVictimShip(int iShipType, int iShipGoods) // генерация призового корабля
{
	...
	ref chref = GetCharacter(NPC_GenerateCharacter("VictimCap_1", "officer_"+(rand(42)+1), "man", "man", 25, iNation, -1, true));
...
void ReasonToFast_PreparePirateShip(string qName) // генерация пиратского корабля
{
	...
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_11", "Logs.txt", "#space#", " "));

	sld = GetCharacter(NPC_GenerateCharacter("PirateCapt", "officer_"+(rand(42)+1), "man", "man", iRank, PIRATE, 30, true));
	sld.dialog.filename      = "GenQuests_Dialog.c";
...
{
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_12", "Logs.txt", "#space#", " "));
...
void ReasonToFast_SetTreasureBoxFromMap(string qName)
{
    ...
    if (GetCharacterItem(pchar, "mapQuest") > 0 )
    {
        ...
        Log_Info(GetConvertStrWithReplace("Variable_GenQuests_13", "Logs.txt", "#space#", " "));
...
bool ReasonToFast_CreateResidenceNPC(aref loc)
{
	...
	if (CheckAttribute(loc, "PlayHall") || CheckAttribute(loc, "Bedroom") || (loc.id.label  == "TownhallRoom") || CheckAttribute(loc, "Office"))
	{
		...
		Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_14", "Logs.txt", "#space#", " "));
...
void RemoveCrewEpidemy_GenQuest()
{
	...
	Log_SetStringToLog(GetConvertStrWithReplace("Variable_GenQuests_19", "Logs.txt", "#space#", " "));
	for(i = 0; i < COMPANION_MAX; i++)
	{
		...
		if(cn >= 0)
		{
			...
			Log_SetStringToLog(GetConvertStrWithReplace("Variable_GenQuests_20", "Logs.txt", "#space#", " ") + chref.Ship.Name + " " + crew + GetConvertStrWithReplace("Variable_GenQuests_21", "Logs.txt", "#space#", " "));
...
{
	Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_22", "Logs.txt", "#space#", " "));
	...
	Log_Info("" + XI_ConvertString(rBaseShip.BaseName) + " '" + sld.Ship.Name + "' " + GetShipSexWord(rBaseShip.BaseName, GetConvertStrWithReplace("Variable_GenQuests_23", "Logs.txt", "#space#", " "), GetConvertStrWithReplace("Variable_GenQuests_24", "Logs.txt", "#space#", " ")) + GetConvertStrWithReplace("Variable_GenQuests_25", "Logs.txt", "#space#", " "));
...
void CheckQuestAboardCabinSituation(ref echar)
{
	...
	{
		echar.FaceId = 333;
		echar.name = GetConvertStrWithReplace("Variable_GenQuests_6", "Names.txt", "#space#", " ");
		echar.lastname = "";
		bQuestDisableMapEnter = true;
...
void EncGirl_GenQuest_GetCoins(string qName)
{
	...
	sld.lifeDay = 0;
	AddSimpleRumour(GetConvertStrWithReplace("Variable_GenQuests_1", "AddRumours.txt", "#space#", " ") + pchar.GenQuest.EncGirl.name + GetConvertStrWithReplace("Variable_GenQuests_2", "AddRumours.txt", "#space#", " "), sti(sld.nation), 5, 1);
	ChangeCharacterReputation(pchar, -10);
	AddQuestRecord("JungleGirl", "3");
	AddQuestUserData("JungleGirl", "sSex", GetSexPhrase("",GetConvertStrWithReplace("Variable_GenQuests_3", "AddRumours.txt", "#space#", " ")));
...
void EncGirl_GenerateLeatherBag(string qName)
{
	...
	Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_26", "Logs.txt", "#space#", " "));
}
...
void EncGirl_SetBerglar(string qName)
{
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("Berglar_EncGirl", "officer_"+(rand(42)+1), "man", "man", iTemp, PIRATE, -1, true));
...
void EncGirl_GenQuest_GetBag(string qName)
{
	ref sld = CharacterFromID("CangGirl");
	sld.lifeDay = 0;
	AddSimpleRumour(RandPhraseSimple(GetConvertStrWithReplace("Variable_GenQuests_4", "AddRumours.txt", "#space#", " "),
				GetConvertStrWithReplace("Variable_GenQuests_5", "AddRumours.txt", "#space#", " ")),
...
{
	int iTemp = sti(pchar.rank) + rand(5);
	ref sld = GetCharacter(NPC_GenerateCharacter("Lover_EncGirl", "officer_"+(rand(42)+1), "man", "man", iTemp, sti(pchar.GenQuest.EncGirl.nation), -1, true));
	...
	pchar.GenQuest.EncGirl.SpeakLover = true;
}

void EncGirl_SpeakLover(string qName)
{
	// quest ended
	bool bOk = CheckAttribute(pchar, "GenQuest.EncGirl");
	// quest ended but another started with different outcome
	bool bOk2 = CheckAttribute(pchar, "GenQuest.EncGirl.SpeakLover") && pchar.GenQuest.EncGirl.SpeakLover == true;
	if (!bOk || !bOk2) {
		trace("Interesting outcome");
		return;
	}
	chrDisableReloadToLocation = true;
...
void EncGirl_DeliveToLoverParents(string qName)
{
	...
	if (pchar.location == "DesMoines_town" && CheckAttribute(pchar, "questTemp.MCzagadka"))	// Sinistra Иммунитет от квеста "Зачарованный город" (не блокирует двери)
	{
...
void CaptainComission_GetPirateIsland()
{
	...
	switch (rand(1))
	{
		...
		case 1:
			...
			pchar.GenQuest.CaptainComission.PirateName = GetFullName(sld);
		break;
	}
...
void CaptainComission_GenerateManager()
{
	...
	switch(rand(2))
	{
		case 0:
			sModel = "trader_2";
			sName = GetConvertStrWithReplace("Variable_GenQuests_7", "Names.txt", "#space#", " ");
			sLastName = GetConvertStrWithReplace("Variable_GenQuests_8", "Names.txt", "#space#", " ");
		break;
		case 1:
			sModel = "pirate_1";
			sName = GetConvertStrWithReplace("Variable_GenQuests_9", "Names.txt", "#space#", " ");
			sLastName = GetConvertStrWithReplace("Variable_GenQuests_10", "Names.txt", "#space#", " ");
		break;
		case 2:
			sModel = "trader_3";
			sName = GetConvertStrWithReplace("Variable_GenQuests_11", "Names.txt", "#space#", " ");
			sLastName = GetConvertStrWithReplace("Variable_GenQuests_12", "Names.txt", "#space#", " ");
		break;
	}

	Log_TestInfo(GetConvertStrWithReplace("Variable_GenQuests_27", "Logs.txt", "#space#", " "));
...
void CaptainComission_GenerateSlave(string qName)
{
	...
	{
		Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_28", "Logs.txt", "#space#", " "));
	...
	if(pchar.GenQuest.CaptainComission.GetSlave == "NoSlave")
	{
		Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_29", "Logs.txt", "#space#", " "));
...
void CaptainComission_GeneratePirateShips(string qName)
{
	...
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_30", "Logs.txt", "#space#", " "));
	...
	for (i=1; i<=2; i++)
	{
		...
		sld = GetCharacter(NPC_GenerateCharacter("CaptainAttack_"+i, "officer_"+(rand(42)+1), "man", "man", Rank, PIRATE, 3, true));
...
{
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_31", "Logs.txt", "#space#", " "));
...
{
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_32", "Logs.txt", "#space#", " "));
	Group_DeleteGroup("Pirate_Attack");
    Island_SetReloadEnableGlobal(pchar.GenQuest.CaptainComission.Island, true);
...
void CaptainComission_GenerateQuestVariant()
{
	...
	{
		case "A1":
			Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_33", "Logs.txt", "#space#", " "));
			CaptainComission_GenerateShip();
		break;
		case "A2":
			Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_34", "Logs.txt", "#space#", " "));
			CaptainComission_GenerateRumourCaptainPrison();
		break;
		case "A3":
			Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_35", "Logs.txt", "#space#", " "));
...
void CaptainComission_toShore() // вышвыриваем в бухту без штанов
{
	...
	AddSimpleRumour(RandPhraseSimple(GetConvertStrWithReplace("Variable_GenQuests_6", "AddRumours.txt", "#space#", " "),
				GetConvertStrWithReplace("Variable_GenQuests_7", "AddRumours.txt", "#space#", " ")), sti(pchar.GenQuest.CaptainComission.Nation), 5, 1);
...
{
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_36", "Logs.txt", "#space#", " "));
...
void CaptainComission_AfterShoreBattle(string qName)
{
	...
	if (LAi_IsDead(sld))
	{
		...
		AddQuestUserData("CaptainComission2", "sShoreName", XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Voc"));
		AddQuestUserData("CaptainComission2", "sName", pchar.GenQuest.CaptainComission.Name);
		CloseQuestHeader("CaptainComission2");
		DeleteAttribute(pchar, "GenQuest.CaptainComission");
	}
...
void CaptainComission_ChangeCaptain()
{
	...
	chref = GetCharacter(NPC_GenerateCharacter("CapComission_2", "officer_"+(rand(42)+1), "man", "man", iRank, iNation, -1, true));
...
void CaptainComission_GangDied(string qName)
{
	if(pchar.GenQuest.CaptainComission.variant == "A1")
	{
		...
		AddQuestUserData("CaptainComission2", "sName", pchar.GenQuest.CaptainComission.Name);
		AddQuestUserData("CaptainComission2", "sShoreName", UpperFirst(XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore + "Voc")));
		CloseQuestHeader("CaptainComission2");
...
void CaptainComission_GenerateRumourCaptainPrison()
{
	...
	AddSimpleRumourCity(LinkRandPhrase(GetConvertStrWithReplace("Variable_GenQuests_8", "AddRumours.txt", "#space#", " ") + GetAddress_Form(pchar) + GetConvertStrWithReplace("Variable_GenQuests_9", "AddRumours.txt", "#space#", " ") + sShipType + " " + pchar.GenQuest.CaptainComission.Name + GetConvertStrWithReplace("Variable_GenQuests_10", "AddRumours.txt", "#space#", " "),
		GetConvertStrWithReplace("Variable_GenQuests_11", "AddRumours.txt", "#space#", " ") + sShipType + " " + pchar.GenQuest.CaptainComission.Name + GetConvertStrWithReplace("Variable_GenQuests_12", "AddRumours.txt", "#space#", " ") + GetAddress_Form(pchar) + GetConvertStrWithReplace("Variable_GenQuests_13", "AddRumours.txt", "#space#", " "),
		RandPhraseSimple(GetConvertStrWithReplace("Variable_GenQuests_14", "AddRumours.txt", "#space#", " ") + pchar.GenQuest.CaptainComission.Name + GetConvertStrWithReplace("Variable_GenQuests_15", "AddRumours.txt", "#space#", " ") + GetAddress_Form(pchar) + GetConvertStrWithReplace("Variable_GenQuests_16", "AddRumours.txt", "#space#", " "),
		GetConvertStrWithReplace("Variable_GenQuests_17", "AddRumours.txt", "#space#", " ") + sShipType + " " + pchar.GenQuest.CaptainComission.Name + GetConvertStrWithReplace("Variable_GenQuests_18", "AddRumours.txt", "#space#", " "))),
...
{
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_37", "Logs.txt", "#space#", " "));
	ref sld = characterFromId("CapComission_1");
	RemovePassenger(pchar, sld);
	ChangeCharacterAddress(sld, "none", "");
	sld.lifeDay = 0;
...
{
	string sShipType = GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipType),"Name") + "Acc"));
	AddSimpleRumourCity(LinkRandPhrase(GetConvertStrWithReplace("Variable_GenQuests_19", "AddRumours.txt", "#space#", " ") + GetAddress_Form(pchar) + GetConvertStrWithReplace("Variable_GenQuests_20", "AddRumours.txt", "#space#", " ") + sShipType +" " + pchar.GenQuest.CaptainComission.Name + GetConvertStrWithReplace("Variable_GenQuests_21", "AddRumours.txt", "#space#", " "),
		GetConvertStrWithReplace("Variable_GenQuests_22", "AddRumours.txt", "#space#", " ") + GetAddress_Form(pchar) + GetConvertStrWithReplace("Variable_GenQuests_23", "AddRumours.txt", "#space#", " ") + sShipType + " " + pchar.GenQuest.CaptainComission.Name + GetConvertStrWithReplace("Variable_GenQuests_24", "AddRumours.txt", "#space#", " "),
		RandPhraseSimple(GetConvertStrWithReplace("Variable_GenQuests_25", "AddRumours.txt", "#space#", " ") + GetAddress_Form(pchar) + GetConvertStrWithReplace("Variable_GenQuests_26", "AddRumours.txt", "#space#", " "),
		GetConvertStrWithReplace("Variable_GenQuests_27", "AddRumours.txt", "#space#", " ") + pchar.GenQuest.CaptainComission.Name + GetConvertStrWithReplace("Variable_GenQuests_28", "AddRumours.txt", "#space#", " ") + sShipType + GetConvertStrWithReplace("Variable_GenQuests_29", "AddRumours.txt", "#space#", " ") + GetAddress_Form(pchar) +GetConvertStrWithReplace("Variable_GenQuests_30", "AddRumours.txt", "#space#", " "))),
...
void CaptainComission_EnterToSeaAfterShoreBattle()
{
	...
	for (int i = 1; i <= 3; i++)
    {
		sld = GetCharacter(NPC_GenerateCharacter("Pirate_Captain0" + i, "officer_"+(rand(42)+1), "man", "man", 5, PIRATE, 3, true));
...
{
	Log_QuestInfo(GetConvertStrWithReplace("Variable_GenQuests_41", "Logs.txt", "#space#", " "));
	bQuestDisableMapEnter = true;
	QuestCloseSeaExit();
...
void Hold_GenQuest_SetTreasureBoxFromMap(string qName)
{
	...
	if (GetCharacterItem(pchar, "mapQuest") > 0 )
    {
        Log_Info(GetConvertStrWithReplace("Variable_GenQuests_42", "Logs.txt", "#space#", " "));
		//Запрет спавна проклятого кэпа в этой локации
		ref locLoad = &locations[reload_location_index];
		pchar.treasurelocation = locLoad.id;
