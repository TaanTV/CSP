...
float ChangeCrewExp(ref chr, string sType, float fNewExp)
{
	...
	float fCrewExp = stf(chr.Ship.Crew.Exp.(sType));
	if (stf(chr.Ship.Crew.Quantity) == 0) return fCrewExp;//фикс ошибки деления на ноль
	...
	fCrewExp = fCrewExp + fNewExp * (100/stf(chr.Ship.Crew.Quantity));//не все матросы учатся разом
	if (fCrewExp > 100) { fCrewExp = 100; }
	if (fCrewExp < 1) { fCrewExp = 1; }
	chr.Ship.Crew.Exp.(sType) = fCrewExp;
	return fCrewExp;
}
...
void MunityOnShip(string _stat)
{
    ...
    Locations[i].image = "loading\jonny_load\load\rebel.tga"; // это клоновая локация, вернётся само при перетирании другим
	...
	if (state == "after" && CheckAttribute(Pchar, "Partition.before.Money"))
	{
		...
		{  // Делим бабки
			if (bPartitionSet)
			{
				...
				Log_TestInfo(GetConvertStrWithReplace("Variable_crew_1", "Logs.txt", "#space#", " ") + TotalAmount);
				...
				Log_TestInfo(GetConvertStrWithReplace("Variable_crew_2", "Logs.txt", "#space#", " ") + ret + GetConvertStrWithReplace("Variable_crew_3", "Logs.txt", "#space#", " ") + Pchar.Partition.MonthPart);
			}
		}
	}
}
