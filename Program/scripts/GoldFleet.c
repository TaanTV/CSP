...
void GoldFleet()
{
	...
	if(!CheckAttribute(pchar, "QuestTemp.GoldFleetMorganaTaken") && rand(3) == 1)
	{
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_GoldFleet_1", "Logs.txt", "#space#", " "));
	}
...
void DefeatGoldFleet(string temp)
{
	...
	{
		pchar.DevFleet = true;
		Log_Info(GetConvertStrWithReplace("Variable_GoldFleet_2", "Logs.txt", "#space#", " "));
		...
		for (int i = 0; i < 17; i++)
		{
			...
			sld.mapEnc.Name = GetConvertStrWithReplace("Variable_GoldFleet_1", "Names.txt", "#space#", " ");
			sld.mapEnc.type = "war";
			...
			if (i == 4)
			{
				sld.name 	= "retardokillas";
				sld.Ship.name 	= "retardokillas";
			}
			if (i == 5)
			{
				sld.name 	= "Lipsar";
			...
			if (i == 10)
			{
				sld.name 	= "Bestreducer";
				sld.Ship.name 	= "Bestreducer";
			}
			if (i == 11)
			{
				sld.name 	= "IG Baron";
				sld.Ship.name 	= "IG Baron";
			}
			if (i == 12)
			{
				sld.name 	= "kb31";
			...
			{
				sld.name 	= "Noctus";
				sld.Ship.name 	= "Noctus";
			}
			if (i == 15)
			{
				sld.name 	= "Shanks";
				sld.Ship.name 	= "Shanks";
			}
			if (i == 16)
			{
				sld.name 	= "Terra";
    ...
    if (isGoldFleet==true)
    {
        switch (igoldpos)
        {
            case 0 :
                ...
                Map_CreateTraderXZ(-858.089, 897.072, hvx, hvz, "Head_of_Gold_Squadron", 6);
...
void StartGoldFleet(string temp);
{
    ...
    && !CheckAttribute(Colonies[FindColony("Havana")], "Siege") && !CheckAttribute(Colonies[FindColony("PortoBello")], "Siege"))
    {
            ...
            Map_CreateTraderXZ(pbx, pbz, -858.089, 897.072, "Head_of_Gold_Squadron", 33);
            igoldpos = 0;
    }
}
