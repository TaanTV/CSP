...
//		DoQuestReloadToLocation("Clone_location", pchar.location.group, pchar.location.locator, "Duel_Fight_Right_Now");
		...
		if(CheckAttribute(PChar,"questTemp.duel.enemyQty")) // не один !!
		{
			...
			{
				sld = GetCharacter(NPC_GenerateCharacter("Berglar_Duel"+i, "officer_"+(rand(42)+1), "man", "man", iTemp, PIRATE, 1, true));
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_place_duelist_tavern", "Logs.txt", "#space#", " ") + PlaceCharacter(npchar, "goto", "Clone_location") + GetConvertStrWithReplace("Variable_duel_1", "Logs.txt", "#space#", " ") + "goto");
	}
	else
	{
