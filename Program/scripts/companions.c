...
bool Ship_AutoAbordage(ref rCharacter, float fMinEnemyDistance)
{
    ...
    //Log_SetStringToLog(GetConvertStrWithReplace("Variable_companions_1", "Logs.txt", "#space#", " ") + rCharacter.Ship.Name + " cap="+rCharacter.id+GetConvertStrWithReplace("Variable_companions_2", "Logs.txt", "#space#", " ") + Characters[sti(rCharacter.SeaAI.Task.Target)].Ship.Name+" cap="+Characters[sti(rCharacter.SeaAI.Task.Target)].id);
    if (fMinEnemyDistance < 70)
    {
		...
		if (bGrapplingProfessional || bAngleTest)
		{
			...
			if (rCharacter.id == "SilverSurferCaptain") fOurCrewFencing *= 10; // СФ
			if (rEnemyCharacter.id == "SilverSurferCaptain") fEnCrewFencing *= 10; // СФ

            if (fOurCrewFencing >= fEnCrewFencing)
            { // победа
                if (bIsEnemyCompanion)
			    {
			        Log_SetStringToLog(GetConvertStrWithReplace("Variable_companions_3", "Logs.txt", "#space#", " ") + rEnemyCharacter.Ship.Name + GetConvertStrWithReplace("Variable_companions_4", "Logs.txt", "#space#", " "));
			    }
			    else
			    {
			        iNation = sti(rEnemyCharacter.nation);
					Log_SetStringToLog(GetConvertStrWithReplace("Variable_companions_5", "Logs.txt", "#space#", " ") + rEnemyCharacter.Ship.Name + GetConvertStrWithReplace("Variable_companions_6", "Logs.txt", "#space#", " ") + NationNameGenitive(iNation) + GetConvertStrWithReplace("Variable_companions_7", "Logs.txt", "#space#", " "));
			    }
            ...
            { // поражение
                if (bIsCharCompanion)
			    {
			        Log_SetStringToLog(GetConvertStrWithReplace("Variable_companions_8", "Logs.txt", "#space#", " ") + rCharacter.Ship.Name + GetConvertStrWithReplace("Variable_companions_9", "Logs.txt", "#space#", " "));
			    }
			    else
			    {
			        iNation = sti(rCharacter.nation);
					Log_SetStringToLog(GetConvertStrWithReplace("Variable_companions_10", "Logs.txt", "#space#", " ") + rCharacter.Ship.Name + GetConvertStrWithReplace("Variable_companions_11", "Logs.txt", "#space#", " ") + NationNameGenitive(iNation) + GetConvertStrWithReplace("Variable_companions_12", "Logs.txt", "#space#", " "));
			    }
...
void SeaExchangeCharactersShips(ref rOneChr, ref rSecChr, bool _showLog, bool _swapCargo)
{
	...
	if (_showLog)
	{
		Log_SetStringToLog(GetFullName(rOneChr) + GetConvertStrWithReplace("Variable_companions_13", "Logs.txt", "#space#", " ") +
