...
#include "scripts\SilverFleet.c" // Zendayo 10.09.24 квест/ивент "Серебряный флот". 
...
void ChangeNationRelationFromRelationAgentComplete(string sQuest)
{
	...
	Log_Info(GetConvertStrWithReplace("Variable_utils_1", "Logs.txt", "#space#", " "));
}
...
void GiveItemToTrader(aref ch)
{
	...
	// Квест "Загадочный остров" (проклятие на ГГ) -->
	if(PChar.MysteriousIsland.Action == true)
	{
    		TakeNItems(ch,"potion1", -100);
    		TakeNItems(ch,"food1", -100);
		
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry1", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry2", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry3", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry5", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry6", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry7", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry10", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry11",-100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry12", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry13", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry14", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry16", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry17", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"jewelry18", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"indian4", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"indian7", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"indian8",-100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"indian9", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"indian13", -100);}
		irand = rand(2);
		if (irand == 1) {TakeNItems(ch,"indian16", -100);}
		// Квест "Загадочный остров" (проклятие на ГГ) <--
	...
	//ржавые
	irand = rand(14);
    if(irand == 1) GenerateAndAddItems(ch, "blade5", 1); // кинжал
    if(irand == 2) GenerateAndAddItems(ch, "slave_01", 1);
    if(irand == 3) GenerateAndAddItems(ch, "blade04", 1);
    if(irand == 4) GenerateAndAddItems(ch, "blade2", 1);
    if(irand == 5) GenerateAndAddItems(ch, "slave_02", 1);
	if(irand == 6) GenerateAndAddItems(ch, "blade3", 1);
	if(irand == 7) GenerateAndAddItems(ch, "blade03", 1);
	if(irand == 8) GenerateAndAddItems(ch, "blade08", 1);
    if(irand == 9) GenerateAndAddItems(ch, "blade09", 1);
    if(irand == 10) GenerateAndAddItems(ch, "blade10", 1);
    if(irand == 11) GenerateAndAddItems(ch, "blade35", 1);
	if(irand == 12) GenerateAndAddItems(ch, "blade02", 1);
    if(irand == 13) GenerateAndAddItems(ch, "topor3", 1);
    if(irand == 14) GenerateAndAddItems(ch, "topor1", 1);
	//жёлтые
	if(rank >= 5)
	{
	    irand = rand(30);
		if(irand == 1) GenerateAndAddItems(ch, "blade6", 1);
		if(irand == 3) GenerateAndAddItems(ch, "blade9", 1);
		if(irand == 5) GenerateAndAddItems(ch, "blade45", 1);
		if(irand == 7) GenerateAndAddItems(ch, "blade7", 1);
		if(irand == 9) GenerateAndAddItems(ch, "blade011", 1);
		if(irand == 11) GenerateAndAddItems(ch, "topor_05", 1);
		if(irand == 13) GenerateAndAddItems(ch, "blade4", 1);
		if(irand == 15) GenerateAndAddItems(ch, "blade1", 1);
		if(irand == 17) GenerateAndAddItems(ch, "blade12", 1);
		if(irand == 19) GenerateAndAddItems(ch, "blade18", 1);
		if(irand == 21) GenerateAndAddItems(ch, "blade17", 1);
		if(irand == 23) GenerateAndAddItems(ch, "blade8", 1);
		if(irand == 25) GenerateAndAddItems(ch, "blade11", 1);
		if(irand == 27) GenerateAndAddItems(ch, "blade16", 1);
		if(irand == 29) GenerateAndAddItems(ch, "blade07", 1);
	}
	//кирасы
	irand = rand(10);
	...
	{
	    irand = rand(380);
		if(irand == 10) GenerateAndAddItems(ch, "blade19", 1);
		if(irand == 30) GenerateAndAddItems(ch, "blade22", 1);
		if(irand == 50) GenerateAndAddItems(ch, "blade36", 1);
		if(irand == 70) GenerateAndAddItems(ch, "blade05", 1);
		if(irand == 90) GenerateAndAddItems(ch, "blade011", 1);
		if(irand == 120) GenerateAndAddItems(ch, "topor31", 1);
		if(irand == 140) GenerateAndAddItems(ch, "blade34", 1);
		if(irand == 160) GenerateAndAddItems(ch, "blade40", 1);
		if(irand == 180) GenerateAndAddItems(ch, "blade46", 1);
		if(irand == 200) GenerateAndAddItems(ch, "blade06", 1);
		if(irand == 220) GenerateAndAddItems(ch, "blade014", 1);
		if(irand == 240) GenerateAndAddItems(ch, "blade012", 1);
		if(irand == 260) GenerateAndAddItems(ch, "blade01", 1);
		if(irand == 280) GenerateAndAddItems(ch, "blade15", 1);
		if(irand == 300) GenerateAndAddItems(ch, "blade010", 1);
		if(irand == 320) GenerateAndAddItems(ch, "blade21", 1);
		if(irand == 340) GenerateAndAddItems(ch, "blade29", 1);
		if(irand == 360) GenerateAndAddItems(ch, "blade015", 1);
		if(irand == 380) GenerateAndAddItems(ch, "topor2", 1);
	}
	//красные
    if(rank >= 25)
	{
		irand = rand(150);
        if(irand == 10) GenerateAndAddItems(ch, "blade23", 1);
		if(irand == 50) GenerateAndAddItems(ch, "blade25", 1);
		if(irand == 100) GenerateAndAddItems(ch, "blade13", 1);
		if(irand == 150) GenerateAndAddItems(ch, "blade016", 1);
	}
	...
	{
		irand = rand(100);
   		if(irand == 11) AddItems(ch, "mushket_drob", 1);
		if(irand == 51) AddItems(ch, "mushket_Arguebuze", 1);
		if(irand == 100) AddItems(ch, "mushket_Temppo", 1);
	}

	if(rank >= 10)
	{
		irand = rand(100);
		if(irand == 12) AddItems(ch, "mushket", 1);
		if(irand == 88) AddItems(ch, "mushket_mushketon1", 1);
	}

	if(rank >= 15)
	{
		irand = rand(100);
		if(irand == 13) AddItems(ch, "mushket_SHZ", 1);
		if(irand == 99) AddItems(ch, "flint_mushketon", 1);
	}
	...
	irand = rand(9);
	if(irand == 1) AddItems(ch, "mineral3", Rand(4) + 1);
...
void GiveItemToSmuggler(aref ch) // Торговля предметов с контриками
{
	...
	if(rank >= 15)
	{
		...
		if(irand == 320) GenerateAndAddItems(ch, "blade21", 1);
		if(irand == 340) GenerateAndAddItems(ch, "blade29", 1);
		if(irand == 360) GenerateAndAddItems(ch, "blade016", 1);
		if(irand == 380) GenerateAndAddItems(ch, "topor2", 1);
	}
	//красные
    if(rank >= 25)
...
{  // location = "random"  "random_free"  "random_must_be" -   random_must_be_near - должен быть, даже если все занято  random_must_be_near - рядом
	...
	if (location == "random" || location == "random_must_be")
	{
		...
		if (checkattribute(ch, "fixteleport"))//девка в прятки именно в ветке random_must_be
		{
			while(homelocator == "boat")
			{
				homelocator = LAi_FindFreeRandomLocator(group);//ищем другой локатор
			}
		}
	...
	if (homelocator == "" && location == "random_must_be") // дело плохо, свободных уже нет, но НПС-то нужен, ищем затяный
	{
		...
		if (checkattribute(ch, "fixteleport"))//девка в прятки именно в ветке random_must_be
		{
			while(homelocator == "boat")
			{
				homelocator = LAi_FindFarLocator(group, locx, locy, locz);//ищем другой локатор
			}
		}
		ChangeCharacterAddressGroup(ch, pchar.location, group, homelocator);  // всегда, где ГГ
	}
...
void EmptyAllFantomCharacter()
{
	...
	for (int i=GlobalCharacters; i<MAX_CHARACTERS; i++)
	{
		if (!CheckAttribute(&characters[i], "id") || characters[i].id == "0") continue;// mitrokosta фикс бесконечных переинитов
		if (CheckCompanionInCharacter(pchar, characters[i].id) || CheckPassengerInCharacter(pchar, characters[i].id))
		{    
		  if(stf(characters[i].chr_ai.hp) < 1.0) characters[i].chr_ai.hp = 1.0;
		  continue;
		}
		if (CheckAttribute(&characters[i], "OfficerInStockMan") || CheckAttribute(&characters[i], "ShipInStockMan")) continue;
	...
	trace("EmptyAllFantomCharacter(), MAX_CHARACTERS - " + MAX_CHARACTERS);
}
...
int NPC_GenerateCharacterIndep(string _id, string _model, string _sex, string _ani, int _rank, int _nation, int _LifeDay, bool _equip)
{
	...
	if (IsCharacterPerkOn(ch, "Ciras") && rand(4)==0)
	{
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_utils_2", "Logs.txt", "#space#", " ")+ch.name+GetConvertStrWithReplace("Variable_utils_3", "Logs.txt", "#space#", " ")+cirnum);
	}
...
ref GetOurSailor(string _id) // моежт быть нужно несколько
{
	...
	CrOur.name     = GetConvertStr("Variable_quests_reaction_46", "Names.txt");
	CrOur.lastname = "";
...
string GetSpeciality(ref chref);
{
	if (IsCharacterPerkOn(chref, "Adventurer")) return "Adventurer";
	if (IsCharacterPerkOn(chref, "SeaWolf")) return "SeaWolf";
	if (IsCharacterPerkOn(chref, "Agent")) return "Agent";
	if (IsCharacterPerkOn(chref, "Grunt")) return "Grunt";
	if (IsCharacterPerkOn(chref, "Fencer")) return "Fencer";
	if (IsCharacterPerkOn(chref, "Trader")) return "Trader";
	if (IsCharacterPerkOn(chref, "Buccaneer")) return "Buccaneer";
}
...
{	
	...
	NPChar.LifeDay = 0; // затирка двух персонажей, на случай если был создан клон по айди
	RemovePassenger(pchar, NPChar);
	int psgQuant = GetPassengersQuantity(pchar);
	int cn;
	for(int i=0;i<psgQuant;i++)
	{
		cn = GetPassenger(pchar,i);
		if(cn==-1) continue;
		if (isPrisonedChar(&characters[cn]) && characters[cn].id == NPChar.id)
		{
			DeleteAttribute(&characters[cn], "prisoned");
			characters[cn].LifeDay = 0; // затирка двух персонажей, на случай если был создан клон по айди
			RemovePassenger(pchar, &characters[cn]);			
		}
	}
}
