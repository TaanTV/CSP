	...
	for (int i=0; i<MAX_WEATHERS; i++)
	{
		...
		if (Weathers[i].id == sWeatherID)
		{
			...
			if (bBettaTestMode)
			{
				...
				Log_SetStringToLog(GetConvertStrWithReplace("Variable_WhrWeather_1", "Logs.txt", "#space#", " ") + Weathers[i].Lighting);
			}
			return;
		}
	}
...
//navy <-- Rain
 ...
 // boal -->
	...
	if (iBlendWeatherNum < 0 || bQuestlockWeather)
	{
		...
		//интерфейс на суше обновляется каждые 9,289 секунды. В море - вдвое реже. интерфейсные часы отстают на период обновления
		Weather.Time.speed = 483.02; // количество секунд на смену погоды - 15:1
		Weather.Time.updatefrequence = 52; // количество обновлений на смену погоды	
	} else {
		Weather.Time.time = GetTime();
		Weather.Time.speed = 241.51;
		Weather.Time.updatefrequence = 26; // количество обновлений на смену погоды
		if (bSeaActive && !bAbordageStarted)
		{
				Weather.Time.speed = 241.51; // количество секунд на смену погоды
				Weather.Time.updatefrequence = 13;
		}
		else
		{ // суша
			Weather.Time.speed = 362.26;
			Weather.Time.updatefrequence = 39; // количество обновлений на смену погоды
		}
	}
...
void Whr_LoadNextWeather(int nPlus)
{
	...
	Log_TestInfo(GetConvertStrWithReplace("Variable_WhrWeather_2", "Logs.txt", "#space#", " ") + Weathers[iCurWeatherNum].id + GetConvertStrWithReplace("Variable_WhrWeather_3", "Logs.txt", "#space#", " ") + Weathers[iCurWeatherNum].Lighting);
...
void FillWeatherData(int nw1, int nw2)
{
	...
	if( nw2<0)
	{
		...
		if(bSeaActive && !bAbordageStarted)
		{
			Sea.Sea2.WaterColor = Whr_GetColor(&Weathers[nw1],"Sea2.WaterColor");
		}
	}
	else
	{
		...
		if(bSeaActive && !bAbordageStarted)
		{
			Sea.Sea2.WaterColor = Whr_BlendColor( fBlend, Whr_GetColor(&Weathers[nw1],"Sea2.WaterColor"), Whr_GetColor(&Weathers[nw2],"Sea2.WaterColor") );
		}
	}
}
