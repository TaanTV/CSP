...
void FaceMaker(aref rCharacter)
{
	...
	switch (checkedmodel)
	{
		...
		case "officer_35":		rCharacter.FaceId = 427; break;
		...
		// --> Старые офицеры, которые не используются, но можно юзать как квестовых
		case "reservist_1": 		rCharacter.FaceId = 436; break;
		case "reservist_2": 		rCharacter.FaceId = 437; break;
		case "reservist_3": 		rCharacter.FaceId = 438; break;
		case "reservist_4": 		rCharacter.FaceId = 442; break;
		case "reservist_5": 		rCharacter.FaceId = 444; break;
		case "reservist_6": 		rCharacter.FaceId = 445; break;
		case "reservist_7": 		rCharacter.FaceId = 447; break;
		case "reservist_8": 		rCharacter.FaceId = 448; break;
		case "reservist_9": 		rCharacter.FaceId = 449; break;
		case "reservist_10":		rCharacter.FaceId = 455; break;
	/*	case "officer_54":		rCharacter.FaceId = 473; break; эти портреты свободны, можно юзать
		...
		case "PKM_rab_2":		rCharacter.FaceId = 479; break;
		case "PKM_rab_3":		rCharacter.FaceId = 480; break;
		case "PKM_rab_4":		rCharacter.FaceId = 480; break;
		// <-- ПКМ рабы

		// --> Нежить-офицеры  PGG_Undead
		case "PGG_Skeletcap_0":	rCharacter.FaceId = 511; break;
		case "PGG_Ghost_0":		rCharacter.FaceId = 527; break;
		case "PGG_Undead":		rCharacter.FaceId = 537; break;
		case "skel1":			rCharacter.FaceId = 351; break;
		case "skel_2":			rCharacter.FaceId = 351; break;
		case "skel_4":			rCharacter.FaceId = 352; break;
		case "skel_5":			rCharacter.FaceId = 353; break;
		case "skelt_0":			rCharacter.FaceId = 354; break;
		...
		case "citiz_13":		rCharacter.FaceId = 426; break;
		...
		case "PGG_Chani":		rCharacter.FaceId = 400; break;
		case "PGG_Darkhuman_0":		rCharacter.FaceId = 473; break;
		case "PGG_PettY_0":		rCharacter.FaceId = 474; break;
		case "PGG_Montoia_0":		rCharacter.FaceId = 503; break;
		case "PGG_Skeletcap_0":		rCharacter.FaceId = 511; break;
		case "Angelica":		rCharacter.FaceId = 350; break;
		...
		case "ManBal5":		    rCharacter.FaceId = 425; break;
		case "GrunMaurits":		rCharacter.FaceId = 150; break;
	...
	{
		if (!CheckAttribute(rCharacter,"HeroModel") && !CheckExclude(rCharacter))
		{
...
bool CheckExclude(ref character)
{
	int i;
	for(i = 1; i <= 4; i++)
	{
		if (HasSubStr(character.model,"Skel_"+i+"_mush")) return true;
		if (HasSubStr(character.model,"Skel"+i+"_mush")) return true;
		if (HasSubStr(character.model,"PotCMob"+i+"_mush")) return true;
	}
	for(i = 1; i <= 2; i++)
	{
		if (HasSubStr(character.model,"Animist"+i+"_mush")) return true;
	}
	return false;
}
...
string GetRandMushketerSkeleton()	//скелеты-мушкетёры
{
    string emodel = "mummy";
    switch (rand(1))
    {
        case 0: emodel = "Skel_"+(1+rand(3))+"_mush";   break;
        case 1: emodel = "Skel"+(1+rand(3))+"_mush";   break;
    }
    return emodel;
}

string GetRandMushketerFishman()	//скелеты-мушкетёры
{
    string emodel = "mummy";
	emodel = "PotCMob"+(1+rand(3))+"_mush";
    return emodel;
}
