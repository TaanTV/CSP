	...
	if (CheckAttribute(pchar, "questTemp.Azzy.Immortal"))
    {
        ...
        if (Result < 0)
        {
            ...
            if (MaxHP < 1)
            {
                ...
                Log_SetStringToLog(GetConvertStrWithReplace("Variable_QuestsUtilite_1", "Logs.txt", "#space#", " "));
            }
            else
            {
                Log_SetStringToLog(GetConvertStrWithReplace("Variable_QuestsUtilite_2", "Logs.txt", "#space#", " "));
            }
...
void FillQuestHoldBox(ref _location)
{
	...
	if(CheckAttribute(pchar,"GenQuest.CaptainComission.CanFindChest"))
	{
		...
		{
			Log_QuestInfo(GetConvertStrWithReplace("Variable_QuestsUtilite_3", "Logs.txt", "#space#", " "));
...
void FillAboardCabinBox(ref _location, ref _npchar)
{
	...
	if (HasSubStr(_npchar.id,"Hunter01") && rand(9)==0 && !CheckAttribute(pchar,"BermudesSurprise"))
	{
	...
	if (CheckAttribute(_npchar, "Ship.Name") && _npchar.Ship.Name == "Колесо Фортуны" || CheckAttribute(_npchar, "Ship.Name") && _npchar.Ship.Name == "Wheel of Fortune")
	{
		DeleteAttribute(_location, "box1");
		if(drand(100) > 85)
		{
	...
	if (CheckAttribute(PChar, "questTemp.Ascold.Ship"))
	{
        ...
        {
			pchar.questTemp.Ascold.Ship.foundcheck = true;
            DeleteAttribute(_location, "box1");
	...
	{
		if (CheckAttribute(_npchar, "SilverSurferCap"))
		{
			DeleteAttribute(_location, "box1");
			_location.box1.money = 800 + drand(200);
			_location.box1.items.rabble = 1;
			_location.box1.items.jewelry7 = 1 + drand(2);
			_location.box1.items.potionrum = 1 + drand(1);
			_location.box1.items.book4_12 = 1;
			_location.box1.items.mineral6 = 1;
			ok = false;
		}
	}
	if (CheckAttribute(_npchar, "sftresureship"))
	{
		int sfshipclass = GetCharacterShipClass(_npchar);
		DeleteAttribute(_location, "box1");
		_location.box1.money = 50000 + makeint(drand(500000)/sfshipclass);
		_location.box1.items.Chest = 1 + drand(5-sfshipclass);
		_location.box1.items.jewelry17 = 15 + makeint(drand(100)/sfshipclass);
		_location.box1.items.jewelry5 = 3 + makeint(drand(20)/sfshipclass);
		_location.box1.items.jewelry1 = 10 + makeint(drand(80)/sfshipclass);
    		_location.box1.items.jewelry2 = 3 + makeint(drand(50)/sfshipclass);
    		_location.box1.items.jewelry3 = 7 + makeint(drand(65)/sfshipclass);
    		_location.box1.items.jewelry4 = 1 + makeint(drand(30)/sfshipclass);
		ok = false;
	}
    ...
    if (ok) // не квестовый
    {
		...
		if(!CheckAttribute(PChar, "QuestTemp.MorganKeyTaken") && rand(250-(sti(pchar.rank) * 3)) == 5 && sti(pchar.rank)>10)
		{
			...
			Log_TestInfo(GetConvertStrWithReplace("Variable_QuestsUtilite_4", "Logs.txt", "#space#", " "));
		}
...
void FantomMakeCoolSailor(ref _Character, int _ShipType, string _ShipName, int _CannonsType, int _Sailing, int _Accuracy, int _Cannons)
{
    ...
    RepairMasts(_Character);// вернуть сбытые мачты
...
void FantomMakeCoolFighter(ref _Character, int _Rank, int _Fencing, int _Pistol, string _Blade, string _Gun, float _AddHP)
{
	...
	If (_Blade == ""){
        _Blade = LAi_NPC_EquipBladeSelection(sti(_Character.rank), false);//не допускаем лапки
    }
...
void GiveGunAmmunition(ref _Character, string sItem)
{
	...
	TakeNItems(_Character,itm.type.t1.bullet, 20+rand(20));
	if(itm.type.t1.gunpowder != "") AddItems(_Character, itm.type.t1.gunpowder, 20+rand(20));// порох при необходимости
...
int GetCoffDiff(float _num, int _maxRange)
{
	...
	{
		case SAILOR_DIFFICULTY:  _num *= 0.6;  break;
		case CAPTAIN_DIFFICULTY: _num *= 1; break;
		case ADMIRAL_DIFFICULTY: _num *= 1.4; break;
	}
	_num += 0.5;
...
int GetCoffDiffSkill(float _num, int _maxRange)
{
	switch (MOD_SKILL_ENEMY_RATE)
	{
		case SAILOR_DIFFICULTY:  _num *= 0.8;  break;
		case CAPTAIN_DIFFICULTY: _num *= 1; break;
		case ADMIRAL_DIFFICULTY: _num *= 1.2; break;
	}
	_num += 0.5;
	if (_num > _maxRange) return _maxRange;
	else return sti(_num);
    ...
    switch(_character.quest.repeat.(_Node))
	{
        ...
        case "1":
            if (_strBad1 == "none" || _strBad1 == "")
            {
				...
				{
					strBack = LinkRandPhrase("I don't want to talk about the same thing several times.",
        ...
        case "2":
            if (_strBad2 == "none" || _strBad2 == "")
            {
				if (LanguageGetLanguage() != "russian")
				{
					strBack = LinkRandPhrase("I repeat once again that I do not want to talk about the same thing ten times!",
						"I repeat, we have already talked about this!",
						"I tell you, we have already discussed this. Memory problems?");
				}
				else
				{
        ...
        case "3":
            if (_strBad3 == "none" || _strBad3 == "")
            {
				if (LanguageGetLanguage() != "russian")
				{
					strBack = LinkRandPhrase("I don't intend to communicate with you anymore.",
						"I won't talk to you anymore!",
						"You made me pretty angry"+ GetSexPhrase("","a") +", no more communication.");
				}
				else
				{
    ...
    switch(Temp)
	{
        ...
        case "1":
            if (_strBad1 == "none" || _strBad1 == "")
            {
				if (LanguageGetLanguage() != "russian")
				{
					strBack = LinkRandPhrase("Okay, buddy.",
						"Whatever you say.",
						"I blurted it out"+ GetSexPhrase("","a") +", sorry...");
				}
				else
				{
        ...
        case "2":
            if (_strBad2 == "none" || _strBad2 == "")
            {
				if (LanguageGetLanguage() != "russian")
				{
					strBack = LinkRandPhrase("Good, good. Just don't get angry.",
						"Whatever you say, just don't shout.",
						"Sorry, I'm not in shape today... My head is splitting in the morning...");
				}
				else
				{
        ...
        case "3":
            if (_strBad3 == "none" || _strBad3 == "")
            {
				if (LanguageGetLanguage() != "russian")
				{
					strBack = LinkRandPhrase("As you know.",
						"Very sorry ...",
						"Small loss.");
				}
				else
...
void SetQuestAboardCabinDialog(ref refChar)
{
	...
	if (CheckAttribute(refChar, "CaptanId"))
	{
		...
		if (CheckAttribute(pchar, "questTemp.Ascold.Ship") && !CheckAttribute(pchar, "questTemp.Ascold.IKnowTheShip") && !CheckCharacterItem(pchar, "ShipsJournal") && !CheckAttribute(PChar, "questTemp.Ascold.SpainCapitainDialogue"))
		{
			sld = CharacterFromID(refChar.CaptanId);
			if (CheckAttribute(sld, "Ascold.ratmark"))
			{
				LAi_SetCheckMinHP(refChar, 10, true, "QuestAboardCabinDialog");  // сколько НР мин
				refChar.Dialog.FileName = "Quest\Other_Quests_NPC.c";
				refChar.Dialog.CurrentNode = "AscoldSpainCapitain"; //даем абордажную ноду
			}
		}
		if (CheckAttribute(PChar, "questTemp.silverfleet") && pchar.questTemp.silverfleet == "ThirdSilverFleet")
		{	
			sld = CharacterFromID(refChar.CaptanId);			
			if (CheckAttribute(sld, "SilverSurferCap")) // СФ
			{
				LAi_SetCheckMinHP(refChar, 10, true, "QuestAboardCabinDialog");
				refChar.Dialog.FileName = "Quest\Other_Quests_NPC.c";
				refChar.Dialog.CurrentNode = "SilverSurferCaptain";
			}
		}
	}
}
...
void SelectAscoldRendom()
{
	if (CheckAttribute(&colonies[1], "nation"))
	{
		...
		{
			if (colonies[n].nation != "none" && sti(colonies[n].nation) != PIRATE && colonies[n].id != "DesMoines")
			{
...
void IsabellaInit()
{
	...
	ch.name 	= GetConvertStr("Variable_QuestsUtilite_0.1", "Names.txt");
	ch.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_1", "Names.txt", "#space#", " ");
...
void SilencePriceInit()
{
	...
	ch.name = GetConvertStr("Variable_QuestsUtilite_2", "Names.txt");
	ch.lastname = GetConvertStr("Variable_QuestsUtilite_3", "Names.txt");
...
void PDMQuestsInit()
{
	...
	sld.name	= GetConvertStr("Variable_QuestsUtilite_3.1", "Names.txt");
	sld.lastname	= GetConvertStr("Variable_QuestsUtilite_3.2", "Names.txt");
	...
	//******В поисках новой родины Sinistra******
	//Хьюго Лесопилка
	sld = GetCharacter(NPC_GenerateCharacter("Hugo_Lesopilka", "officer_40", "man", "man", 10, PIRATE, -1, true));
	sld.name	= GetConvertStr("Variable_QuestsUtilite_3.3", "Names.txt");
	sld.lastname	= GetConvertStr("Variable_QuestsUtilite_3.4", "Names.txt");
	sld.sex = "man";
	...
	sld.name	= GetConvertStr("Variable_QuestsUtilite_3.5", "Names.txt");
	sld.lastname	= "";
	...
	sld.name	= GetConvertStr("Variable_QuestsUtilite_3.6", "Names.txt");
	sld.lastname	= GetConvertStr("Variable_QuestsUtilite_3.7", "Names.txt");
	...
	sld.name	= GetConvertStr("Variable_QuestsUtilite_3.8", "Names.txt");
	sld.lastname	= "";
	...
	sld.name	= GetConvertStr("Variable_QuestsUtilite_3.9", "Names.txt");
	sld.lastname	= "";
	...
	sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_4", "Names.txt", "#space#", " ");
	sld.lastname = "";
	...
	pchar.quest.PDM_ONV_Nachalo.win_condition.l1.value = 15;
...
void KSMQuestsInit()
{
	...
	Island_SetReloadEnableLocal("Reefs", "reload_3", false)
	Island_SetGotoEnableLocal("Reefs", "reload_3", false);
}
//Проклятые Судьбой
void KPSQuestsInit()
{
	...
	pchar.quest.KPS_ZO_JimSmit_Spawn.win_condition.l1.value = 15;
...
void VSEnpcInit()
{
	...
	LAi_SetCitizenType(sld);
	sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_5", "Names.txt", "#space#", " ");
	sld.lastname = "";
	...
	sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_6", "Names.txt", "#space#", " ");
	sld.lastname = "";
	LAi_SetWarriorType(sld);
	...
	//Антонио де Соуза, Глава Инквизиции
	sld = GetCharacter(NPC_GenerateCharacter("AntonioDeSouza", "priest", "man", "man1", 10, SPAIN, -1, false));
    sld.name 	= GetConvertStr("Variable_QuestsUtilite_0.2", "Names.txt");
    sld.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_7", "Names.txt", "#space#", " ");
	sld.model.animation = "man1";
	...
	pchar.quest.CSP_KvM_SpawnRibak.win_condition.l1.value = 5;
	pchar.quest.CSP_KvM_SpawnRibak.win_condition.l1.operation = ">=";
	PChar.quest.CSP_KvM_SpawnRibak.win_condition = "CSP_KvM_SpawnRibak";
	
//============== Локаторы для вызова акулы =================
	Locations[FindLocation("SantoDomingo_town")].locators_radius.item.akula1 = 1.0;
	Locations[FindLocation("SantoDomingo_town")].locators_radius.item.akula2 = 2.0;
	Locations[FindLocation("SantoDomingo_town")].locators_radius.item.akula3 = 1.8;
	Locations[FindLocation("SantoDomingo_town")].locators_radius.item.akula4 = 1.5;
	Locations[FindLocation("SantoDomingo_town")].locators_radius.item.akula5 = 1.5;
	Locations[FindLocation("SantoDomingo_town")].locators_radius.item.akula6 = 1.5;
	
//============== Индейская деревня =================	

	//Шаманка Чани
	sld = GetCharacter(NPC_GenerateCharacter("Chani", "PGG_Chani", "woman", "Chani", 10, PIRATE, -1, false));
	sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_8", "Names.txt", "#space#", " ");
	sld.lastname = "";
	sld.dialog.Filename = "Indian_dialog.c";
	sld.dialog.currentnode = "Chani_1";
	LAi_SetOwnerType(sld);
	LAi_group_MoveCharacter(sld, "ItzaIndian");
	ChangeCharacterAddressGroup(sld, "Indian_Hut1", "barmen", "stay");
	
	SetCharacterPerk(sld, "Doctor1");
	SetCharacterPerk(sld, "Doctor2");
	SetCharacterPerk(sld, "EmergentSurgeon");
	SetCharacterPerk(sld, "Energaiser");
	SetSPECIAL(sld, 6,5,8,5,9,10,5);
	SetSelfSkill(sld, 60,40,40,1,30);
	SetShipSkill(sld, 50,4,1,1,1,15,20,60,40);

	//Красавица Шоко
	sld = GetCharacter(NPC_GenerateCharacter("ItzaKrasavitsa", "TribeWife", "woman", "YokoDias", 10, PIRATE, -1, false));
	sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_9", "Names.txt", "#space#", " ");
	sld.lastname = "";
	sld.dialog.Filename = "Indian_dialog.c";
	sld.dialog.currentnode = "Shoko_1";
	LAi_SetCitizenType(sld);
	LAi_group_MoveCharacter(sld, "ItzaIndian");
	ChangeCharacterAddressGroup(sld, "IndianVillage", "quest", "teleport1");

	//******Макуауитль Куали******
	//Воин Куали
	pchar.quest.Kuali_Spawn.win_condition.l1 = "Rank";
	pchar.quest.Kuali_Spawn.win_condition.l1.value = 5;
...
void OfficerGirlInit()
{
	...
	sld.name 	= GetConvertStr("Variable_QuestsUtilite_0.3", "Names.txt");
	sld.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_10", "Names.txt", "#space#", " ");
	sld.loyality = 12;
	sld.rank 	= 15;
	...
	sld.name 	= GetConvertStr("Variable_QuestsUtilite_0.4", "Names.txt");
	sld.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_11", "Names.txt", "#space#", " ");
	sld.loyality = 20;
	sld.rank 	= 15;
    sld.reputation = 50;
	sld.alignment = "good";
	...
	sld.name 	= GetConvertStr("Variable_QuestsUtilite_0.5", "Names.txt");
	sld.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_12", "Names.txt", "#space#", " ");
	sld.loyality = 20;
	sld.rank 	= 15;
    sld.reputation = 15;
	sld.alignment = "bad";
...
void OfficerMushketerInit()
{
	...
	sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_13", "Names.txt", "#space#", " ");
	sld.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_14", "Names.txt", "#space#", " ");
	...
	sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_15", "Names.txt", "#space#", " ");
	sld.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_16", "Names.txt", "#space#", " ");
	sld.greeting = "Gr_Officer";
	sld.loyality = 13;
	sld.rank = 15;
	sld.reputation = 85;
	...
	for(int i = 0; i < num; i++)
	{
		...
		if (i == 0)
		{
			if (GetDataDay() == 3 && GetDataMonth() == 3 && GetTime() >= 3.0 && GetTime() < 4.0 && loc == "MountainPath" && !CheckAttribute(pchar,"salasarmet") && CheckAttribute(pchar,"SalasarEventKnow"))
			{
				...
				sld.name = GetConvertStrWithReplace("Variable_QuestsUtilite_17", "Names.txt", "#space#", " ");
				sld.lastname = GetConvertStrWithReplace("Variable_QuestsUtilite_18", "Names.txt", "#space#", " ");
				...
				FantomMakeCoolFighter(sld, 55, 100, 100, RandPhraseSimple("blade46","blade010"), "pistol8", 300); //55 лвл, Кракемарт или Кавалерийский палаш, Бландербуз
				...
				Log_info(GetConvertStrWithReplace("Variable_QuestsUtilite_5", "Logs.txt", "#space#", " "));
				Log_info(GetConvertStrWithReplace("Variable_QuestsUtilite_6", "Logs.txt", "#space#", " "));
				AddCharacterHealth(pchar, -10);
				chrDisableReloadToLocation = true; //пока Салазар жив - хер, а не выход
			}
			if (loc != "MountainPath")
			{
				if (pchar.name != "Виспер" && !CheckAttribute(pchar, "PGGWhisperQuest"))
				{
					Log_info(GetConvertStrWithReplace("Variable_QuestsUtilite_7", "Logs.txt", "#space#", " "));
				...
				else
				{
					...
					{
						Log_info(GetConvertStrWithReplace("Variable_QuestsUtilite_8", "Logs.txt", "#space#", " "));
						chrDisableReloadToLocation = true; //пока Злой Скелет Гигант жив - хер, а не выход
						pchar.GiantEvilSkeleton = true;
						LAi_group_SetCheck("ReefAssholes", "SpawnGiantEvilSkeleton");
					}
		...
		if (loc == "MountainPath")
		{
			...
			if (i == 0 && sld.name == GetConvertStrWithReplace("Variable_QuestsUtilite_19", "Names.txt", "#space#", " "))
			{
...
void QuestCheckTakeBoxes(ref itemsRef)
{
	...
	if (!CheckAttribute(pchar,"treasurelocation"))
	{
		...
		{
			Log_info(GetConvertStrWithReplace("Variable_QuestsUtilite_9", "Logs.txt", "#space#", " "));
			/*string chesttype = "";
			switch (drand(3))
			{
			...
			Log_info(GetConvertStrWithReplace("Variable_QuestsUtilite_10", "Logs.txt", "#space#", " "));*/
			Log_info(GetConvertStrWithReplace("Variable_QuestsUtilite_11", "Logs.txt", "#space#", " "));
	...
	{
		Log_Info(GetConvertStrWithReplace("Variable_QuestsUtilite_12", "Logs.txt", "#space#", " "));
		PlaySound("interface\notebook.wav");
		...
		if(sti(pchar.questTemp.treasurecount) >= 10) UnlockAchievement("AchTreasure", 2);
		if(sti(pchar.questTemp.treasurecount) >= 20) UnlockAchievement("AchTreasure", 3);
