...
#include "characters\Questions.c"
...
void CharactersInit()
{
	...
	LoadSegment(sPath + "init\DesMoines.c");				ReloadProgressUpdate();
	...
	UnloadSegment(sPath + "init\DesMoines.c");
...
{
	if (CheckAttribute(character, "HeroModel") && character.model != "protocusto" && !CheckAttribute(character,"Cirgnore")) ////добавил проверку на атрибут, блокирующий смену модели кирасной логикой
	{
		if (CheckAttribute(character,"equip.suit"))
		{
			bool costumetrue = CheckAttribute(character,"Costume") && sti(character.costume)==1;
			if (costumetrue || !CheckAttribute(character,"cirassId"))
			{
				character.model = GetSubStringByNum(character.HeroModel,sti(Items[FindItem(character.equip.suit)].model));
				character.model = CheckForMushketerAni(character);
...
string CheckForMushketerAni(ref character)
{
	if (HasSubStr(character.model.animation,"mushket") && !HasSubStr(character.model,"quest_mush") && !HasSubStr(character.model,"MusketeerEnglish_")
	&& !HasSubStr(character.model,"mushketer_") && !HasSubStr(character.model,"_mush_") && !CheckAttribute(character,"AboardFantom")) character.model = character.model+"_mush";
