...
/* Флаг показа сообщения "добавлено/Потрачено" при вызове AddMoneyToCharacter. Используется в азартных играх, что бы не спамить на каждый розыгрыш сообщениями */
bool showMoneyLog_default = true;
...
float GetMastPercent(ref _refCharacter)
{
	int nShipType = GetCharacterShipType(_refCharacter);
	//if (nShipType==SHIP_NOTUSED) return 100;
	ref refBaseShip = GetRealShip(nShipType);
	if (checkattribute(refBaseShip, "mastHP"))
	{
		string sMastNum;
		float fBaddestMast=0.0;//ищем самую поврежденную мачту
		float fMast;
		aref arMasts;
		makearef(arMasts, refBaseShip.mastHP);
		for(int iM=0; iM<GetAttributesNum(arMasts); iM++)
		{
			sMastNum = GetAttributeName(GetAttributeN(arMasts, iM));
			fMast = GetAttributeValue(GetAttributeN(arMasts, iM));
			if (checkattribute(refBaseShip, "mastHPRep."+sMastNum)) fMast -= makefloat(refBaseShip.mastHPRep.(sMastNum));//учитываем отложенный ремонт
			if (fMast>fBaddestMast && fMast<1.0) fBaddestMast = fMast; //полностью сломанные мачты игнорим
		}
		return (1.0-fBaddestMast)*100.0;
	}
	else return 100.0;
}
...
float GetSailRPD(ref _refCharacter) // процент ремонта парусов в день
{
	...
	if (S_armor(_refCharacter)) ret /= 3; // СФ
...
float GetSailSPP(ref _refCharacter) // количество парусины на один процент починки
{
	...
	if (S_armor(_refCharacter)) ret *= 3;
...
int GetNotCaptivePassengersQuantity(ref _refCharacter)
{
	...
	{
		bool bIsPrisoned = CheckAttribute(characters[idx],"prisoned");
		if(!bIsPrisoned)
		{
			result = result+1;
...
/* Отключить показ лога денег ГГ для однократного вызова AddMoneyToCharacter. После вызова флаг сброситься на true */
void setShowMoneyLogValue(bool showLog)
{
	showMoneyLog_default = showLog;
}

/* Вернуть флаг показа лога денег ГГ для однократного вызова AddMoneyToCharacter */
bool getShowMoneyLogValue()
{
	return showMoneyLog_default;
}
...
int AddMoneyToCharacter(ref _refCharacter,int _Money)
{
	...
	if (sti(_refCharacter.index) == GetMainCharacterIndex()) // флаг лога
	{
		if (_Money > 0)
		{
      if(getShowMoneyLogValue())
				{
			    Log_Info(GetConvertStrWithReplace("Variable_CharacterUtilite_1", "Logs.txt", "#space#", " ") + _Money + GetConvertStrWithReplace("Variable_CharacterUtilite_2", "Logs.txt", "#space#", " "));
        }
		...
		else
		{
         ...
         if(getShowMoneyLogValue())
				{
				  Log_Info(GetConvertStrWithReplace("Variable_CharacterUtilite_3", "Logs.txt", "#space#", " ") + abs(_Money) + GetConvertStrWithReplace("Variable_CharacterUtilite_4", "Logs.txt", "#space#", " "));
        }
	...
	/* Восстанавливаем показ лога, который был отключен при вызове азартной игры. Обязательно вызывать */
	setShowMoneyLogValue(true);
	// boal <--
	return newMoney;
}
...
void AddAchievementPoints(int points) // Даем очки достижений
{
	...
	Log_Info(GetConvertStrWithReplace("Variable_CharacterUtilite_5", "Logs.txt", "#space#", " ") + pchar.ach_points);
}
...
void UnlockAchievement(string ach_name, int level) // Открываем достижениие
{
	...
	Log_Info(GetConvertStrWithReplace("Variable_CharacterUtilite_6", "Logs.txt", "#space#", " ") + achievement + "' (+" + points + GetConvertStrWithReplace("Variable_CharacterUtilite_7", "Logs.txt", "#space#", " "));
...
void PerformAchievement(string AchID, string AchStep, string LogAch)
{
	if(pchar.questTemp.Achievements.(AchID).(AchStep) != "true")
	{
		...
		Log_Info(GetConvertStrWithReplace("Variable_CharacterUtilite_8", "Logs.txt", "#space#", " ") +LogAch+"'"); // Выводим в лог
...
{
	if (rCharacter.sex == "crab") {rCharacter.name = GetConvertStrWithReplace("Variable_CharacterUtilite_1", "Names.txt", "#space#", " "); rCharacter.lastname = "№" + (1+rand(998)); return;}
...
{
	if (rCharacter.sex == "crab") {rCharacter.name = GetConvertStrWithReplace("Variable_CharacterUtilite_2", "Names.txt", "#space#", " "); rCharacter.lastname = "№" + (1+rand(998)); return;}
	int iNation = sti(rCharacter.nation);
	if (iNation == -1) iNation = PIRATE;
	while (iNation == PIRATE) { iNation = rand(MAX_NATIONS - 2); }
...
void SetEquipedItemToCharacter(ref chref, string groupID, string itemID)
{
    ...
    case CIRASS_ITEM_TYPE:
        if (CheckAttribute(chref, "HeroModel")) // все, у кого есть что одеть
        {
            ...
            chref.model = CheckForMushketerAni(chref);
        }
        else
        {
	...
	case GUN_ITEM_TYPE:
		...
		else
		{
			...
			if(CheckAttribute(chref,"chr_ai.gunpowder"))	DeleteAttribute(chref,"chr_ai.gunpowder");
	...
	break;
	case AMMO_ITEM_TYPE:
		if(CheckAttribute(arItm, "id")) chref.curammo = arItm.id; 
		else trace("БАГ! Нет айди у боеприпаса. SetEquipedItemToCharacter(ref chref, string groupID, string itemID)" + chref.id + "|" + groupID + "|" + itemID);
	break;
	// <-- ugeen
	}
}
...
void EquipCharacterByItem(ref chref, string itemID)
{
	...
	if (groupName == BOOK_ITEM_TYPE && IsMainCharacter(chref)) // Книги, экипировка - Gregg
	{
		...
		if (sti(chref.books.(sBook)) > 0)
		{
			...
			Log_Info(GetConvertStrWithReplace("Variable_CharacterUtilite_9", "Logs.txt", "#space#", " ") + chref.books.(sBook) + GetConvertStrWithReplace("Variable_CharacterUtilite_10", "Logs.txt", "#space#", " "));
		}
	}
}
 // to_do
 	...
 	if (CheckOfficersPerk(_refCharacter, "Troopers"))
    {
        ...
        for (j=1; j<COMPANION_MAX; j++) // ГГ отдельно
        {
	        ...
	        if (cn>0)
	        {
                ...
                if (compCrew > 0)
                {
	                RemoveCharacterGoodsSelf(officer, GOOD_WEAPON, makeint(compCrew/3 + 0.5));
	                SetCrewQuantity(officer, GetCrewQuantity(officer) - compCrew);
                	Log_SetStringToLog(GetConvertStrWithReplace("Variable_CharacterUtilite_11", "Logs.txt", "#space#", " ") + officer.Ship.Name + GetConvertStrWithReplace("Variable_CharacterUtilite_12", "Logs.txt", "#space#", " ") + compCrew + GetConvertStrWithReplace("Variable_CharacterUtilite_13", "Logs.txt", "#space#", " "));
    ...
    if (CheckAttribute(_refCharacter, "TempPerks.QuestTroopers"))
	{
		...
		for (j=1; j<COMPANION_MAX; j++) // ГГ отдельно
        {
	        ...
	        if (cn>0)
	        {
		        ...
		        Log_SetStringToLog(GetConvertStrWithReplace("Variable_CharacterUtilite_14", "Logs.txt", "#space#", " ") + officer.Ship.Name + GetConvertStrWithReplace("Variable_CharacterUtilite_15", "Logs.txt", "#space#", " ") + compCrew + GetConvertStrWithReplace("Variable_CharacterUtilite_16", "Logs.txt", "#space#", " "));
                if (compCrew > 0)
                {
                	sumCrew = sumCrew + compCrew;
                }
...
{
    Log_SetStringToLog(GetConvertStrWithReplace("Variable_CharacterUtilite_17", "Logs.txt", "#space#", " "));
...
{
	ref rItem;
	String sItem = GetCharacterEquipByGroup(rChar, SUIT_ITEM_TYPE);
...
bool CheckMapForEquipped(ref refCh, string itemID)
{
	...
	if(sti(refCh.index) == GetMainCharacterIndex() && CheckCharacterItem(refCh, arItem.id))
	{
		...
		if(!IsEquipCharacterByMap(refCh, itemID)) { // проверяем, экипирован ли ГГ  этой картой
			...
			Log_SetStringToLog(LanguageConvertString(idLngFile, arItem.name) +GetConvertStrWithReplace("Variable_CharacterUtilite_18", "Logs.txt", "#space#", " "));
...
void Lilarcor_Talks()
{
	...
	log_info(GetConvertStrWithReplace("Variable_CharacterUtilite_19", "Logs.txt", "#space#", " "));
}
...
void RepairMasts(ref _Char)
{
	DeleteAttribute(_Char,"ship.masts");

	ref realShip;
	int shipType = sti(_Char.Ship.Type);
	if(shipType != SHIP_NOTUSED) realShip = GetRealShip(shipType); else return;

	DeleteAttribute(realShip,"mastHP");
	DeleteAttribute(realShip,"mastHPrep");
}


int GetCharacterItemWithCabin(ref _refCharacter,string itemName,bool ignore)
{
	int qty = 0;
	if(CheckAttribute(_refCharacter,"Items."+itemName))	qty = qty + sti(_refCharacter.Items.(itemName));
	if (!ignore && loadedLocation.id != Get_My_Cabin()) return qty;
	aref chests;
	if (Get_My_Cabin() == "") return qty;
	if (!CheckAttribute(locations[FindLocation(Get_My_Cabin())],"box1")) return qty;
	int chestsnum = CheckCabinBoxes(locations[FindLocation(Get_My_Cabin())]);
...
void TakeNItemsWithCabin(ref _refCharacter,string itemName,int quant)
{
	...
	if(CheckAttribute(_refCharacter,"Items."+itemName) && sti(_refCharacter.Items.(itemName)) != 0)
	{
		...
		{
			//log_info("Вы передали "+qty+" шт. "+itmname);
			TakeNItems(_refCharacter,itemName,qty);
			return;
		}
		else 
		{
			//log_info("Вы передали "+sti(_refCharacter.Items.(itemName))+" шт. "+itmname);
	...
	for(int i = 1; i <= chestsnum; i++)
	{
		...
		if(CheckAttribute(chests,"Items."+itemName) && sti(chests.Items.(itemName)) != 0)
		{
			...
			{
				//log_info(qty+" шт. "+itmname+" взято из сундука каюты "+i);
				TakeNItems(chests,itemName,qty);
				return;
			}
			else 
			{
				//log_info(sti(chests.Items.(itemName))+" шт. "+itmname+" взято из сундука каюты "+i);
