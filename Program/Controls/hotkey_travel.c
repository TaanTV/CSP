void HKT_Button(string sHKB) // быстрый переход
{
	...
	if(bOk)
	{
		if (sHKB == "Fast_port")		//Телепортация в ГОРОД (или в порт) через ALT
		{
			if (sCityID != "PuertoPrincipe" && sCityID != "FortOrange" && sCityID != "DesMoines") PlayerFastTravel(curLocIdx, sCityID + "_town", "reload1");
	...
	{
		Log_info(XI_ConvertString(GetConvertStrWithReplace("Variable_hotkey_travel_1", "Logs.txt", "#space#", " ")));
