...
{
    ref sld;
	Pchar.questTemp.WhisperLine = true;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (int i = 0; i < 10; i++)
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_whisper_line_functions_1", "Names.txt", "#space#", " ")+(800+rand(150));
		sld.lastname = "";
		sld.CantLoot = true;
		if (rand (1) == 0)
		{
...
void WhisperScientist(string qName)
{
	...
	sld.name = GetConvertStrWithReplace("Variable_whisper_line_functions_2", "Names.txt", "#space#", " ");
	sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_3", "Names.txt", "#space#", " ");
	...
	sld.dialog.filename = "Quest\WhisperLine\Whisper.c";
	sld.name = GetConvertStrWithReplace("Variable_whisper_line_functions_4", "Names.txt", "#space#", " ");
	sld.lastname = "";
	sld.dialog.currentnode = "First time";
	//sld.greeting = "cit_common";
...
void WhisperTeleport(string qName)
{
	...
	sld.dialog.currentnode = "First time";
	sld.name = GetConvertStrWithReplace("Variable_whisper_line_functions_5", "Names.txt", "#space#", " ");
	sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_6", "Names.txt", "#space#", " ");
...
void WhisperLine_Inquisition(string qName)
{
	...
	DoQuestFunctionDelay("WhisperLine_Cage", 1.5);
}
void WhisperLine_Cage(string qName)
{
	...
	sld = GetCharacter(NPC_GenerateCharacter("AntonioDeSouza", "PGG_Vincento_0", "man", "man", 10, SPAIN, -1, false));
   	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.1", "Names.txt");
    sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_7", "Names.txt", "#space#", " ");
    sld.Dialog.Filename = "Quest\WhisperLine\Whisper.c";
	...
	for (i=1; i<=4; i++)
	{
		...
		else
		{
			if(CheckAttribute(pchar, "Whisper.IncqGuard_bad"))
			{
				...
				WhisperOpenTunnel("WhisperEscape");
				TakeNItems(sld, "pistol3", -1);
				RemoveCharacterEquip(sld, GUN_ITEM_TYPE);
			}
		}
...
void GiveMCItemToSomeoneElse(ref sld)
{
	...
	switch (ch.HeroParam.HeroType)
	{
        ...
        case "Trader":	// Торговец
			...
			switch (sti(ch.nation))
			{
				case ENGLAND:
					...
					sld.money = 9000/MOD_SKILL_ENEMY_RATE*3;
				break;
				case FRANCE:
	    ...
	    case "Adventurer":	// Авантюрист
            ...
            EquipCharacterbyItem(sld, itemID);
			TakenItems(sld, "potion1", rand(10));
			GiveItem2Character(sld, "Totem_" + rand(14));
			GiveItem2Character(sld, "Totem_" + rand(14));
			GiveItem2Character(sld, "Totem_" + rand(14));
...
void WhisperPirateTown(string qName)
{
	sld = GetCharacter(NPC_GenerateCharacter("NineFingers", "PGG_Alex_blade_0", "man", "man", 10, PIRATE, -1, false));
   	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.2", "Names.txt");
	chrDisableReloadToLocation = true;
    sld.lastname = "";
    sld.Dialog.Filename = "Quest\WhisperLine\Whisper.c";
...
void WhisperMeetCrew(string qName)
{
	...
	sld = GetCharacter(NPC_GenerateCharacter("Wh_Jack", "PGG_HuanChahotka_0", "man", "man", 5, PIRATE, -1, false));
   	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.3", "Names.txt");
    sld.lastname = "";
    sld.Dialog.Filename = "Quest\WhisperLine\Whisper.c";
	...
	sld = GetCharacter(NPC_GenerateCharacter("Wh_Jim", "PGG_Doggerty_0", "man", "man", 5, PIRATE, -1, false));
   	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.4", "Names.txt");
    sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_8", "Names.txt", "#space#", " ");
    sld.Dialog.Filename = "Quest\WhisperLine\Whisper.c";
...
void WhisperChinamanCapSpeaks(string qName)
{
	...
	LAi_SetActorTypeNoGroup(sld);
	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.5", "Names.txt");
    sld.lastname = "";
	...
	LAi_SetActorTypeNoGroup(sld);
	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.6", "Names.txt");
    sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_9", "Names.txt", "#space#", " ");
	sld.Dialog.Filename = "Quest\WhisperLine\Whisper_cabin_dialog.c";
    ChangeCharacterAddressGroup(sld, Get_My_Cabin(), "rld", LAi_FindNearestFreeLocator2Pchar("rld"));
}
...
void WhisperHuntersCaveEntrance(string qName)
{
	...
	sld.CanTakeMushket = true;
	sld.HasNoFear = true;
	sld.name = GetConvertStrWithReplace("Variable_whisper_line_functions_10", "Names.txt", "#space#", " ");
	sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_11", "Names.txt", "#space#", " ");
	LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
	sld.dialog.filename = "Quest\WhisperLine\Whisper.c";
	LAi_SetActorType(sld);
...
void WhisperDeSouzaSeaBattle()
{
	...
	sld.Ship.name = GetConvertStrWithReplace("Variable_whisper_line_functions_12", "Names.txt", "#space#", " ");
	LAi_SetImmortal(sld, false);
	//SetBaseShipData(sld);
...
void WhisperWarDogSeaBattle()
{
	...
	sld.Ship.name = GetConvertStrWithReplace("Variable_whisper_line_functions_13", "Names.txt", "#space#", " ");
	//SetBaseShipData(sld);
	sld.Ship.Cannons.Type = CANNON_TYPE_CANNON_LBS24;
...
void WhisperSpawn_Chard()
{
	...
	sld.name = GetConvertStrWithReplace("Variable_whisper_line_functions_14", "Names.txt", "#space#", " ");
	sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_15", "Names.txt", "#space#", " ");
	//LAi_SetImmortal(sld, true);
...
void WhisperSpawnPortman(ref npchar)
{
	...
	sld.mapEnc.Name = GetConvertStrWithReplace("Variable_whisper_line_functions_16", "Names.txt", "#space#", " ");
	sld.mapEnc.type = "trade";
	FantomMakeCoolFighter(sld, 70, 50 + difficulty, 50 + difficulty, "blade23", "pistol3", 60);
	FantomMakeCoolSailor(sld, SHIP_UNICORN, GetConvertStr("Variable_whisper_line_functions_1", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS32, 50 + difficulty, 50 + difficulty, 50 + difficulty);
	sld.dialog.filename = "Quest\WhisperLine\Whisper_cabin_dialog.c";
	sld.DeckDialogNode = "Portman";
	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.7", "Names.txt");
	sld.lastname = GetConvertStrWithReplace("Variable_whisper_line_functions_17", "Names.txt", "#space#", " ");
	sld.AlwaysSandbankManeuver = true;
	sld.DontRansackCaptain = true;
...
void DarkHuman_Temple(string qName)
{
	...
	for (i = 0; i <= sti(pchar.maxIndians) * 2; i++)
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_whisper_line_functions_18", "Names.txt", "#space#", " ");
		sld.lastname = "";
		DeleteAttribute(sld, "equip");
		DeleteAttribute(sld, "items");
...
{
	//Log_Info(GetConvertStrWithReplace("Variable_whisper_line_functions_1", "Logs.txt", "#space#", " "))
...
{
	Log_info(GetConvertStrWithReplace("Variable_whisper_line_functions_2", "Logs.txt", "#space#", " ")+(1500 + rand(750)));
	DoQuestFunctionDelay("DHmessages_wait",1.0);
	DoQuestFunctionDelay("DHmessages_2",3.0);
}
void DHmessages_2(string qName)
{
	Log_info(GetConvertStrWithReplace("Variable_whisper_line_functions_3", "Logs.txt", "#space#", " ")+(1500 + rand(750)));
	DoQuestFunctionDelay("DHmessages_wait",1.0);
	DoQuestFunctionDelay("DHmessages_3",3.0);
}void DHmessages_3(string qName)
{
	Log_info(GetConvertStrWithReplace("Variable_whisper_line_functions_4", "Logs.txt", "#space#", " ")+(1500 + rand(750)));
	DoQuestFunctionDelay("DHmessages_wait",1.0);
	DoQuestFunctionDelay("DHmessages_4",3.0);
}void DHmessages_4(string qName)
{
	Log_info(GetConvertStrWithReplace("Variable_whisper_line_functions_5", "Logs.txt", "#space#", " ")+(1500 + rand(750)));
	DoQuestFunctionDelay("DHmessages_wait",1.0);
	DoQuestFunctionDelay("DHmessages_5",3.0);
}void DHmessages_5(string qName)
{
	Log_info(GetConvertStrWithReplace("Variable_whisper_line_functions_6", "Logs.txt", "#space#", " ")+(1500 + rand(750)));
}void DHmessages_wait(string qName)
{
	Log_info(GetConvertStrWithReplace("Variable_whisper_line_functions_7", "Logs.txt", "#space#", " "));
}
...
void CaptureCapitol_SeaBattle()
{
	...
	for (i = 1; i < 8; i++)
	{
		...
		if (i == 1) FantomMakeQuickSailor(sld, SHIP_WASA, GetConvertStr("Variable_whisper_line_functions_2", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 2) FantomMakeQuickSailor(sld, SHIP_GALEON, GetConvertStr("Variable_whisper_line_functions_3", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS24, 50, 50, 50);
		if (i == 3) FantomMakeQuickSailor(sld, SHIP_REVENGE, GetConvertStr("Variable_whisper_line_functions_4", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 4) FantomMakeQuickSailor(sld, SHIP_GALEON_H, GetConvertStr("Variable_whisper_line_functions_5", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 5) FantomMakeQuickSailor(sld, SHIP_FELIPE, GetConvertStr("Variable_whisper_line_functions_6", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 6) FantomMakeQuickSailor(sld, SHIP_SANTIAGO, GetConvertStr("Variable_whisper_line_functions_7", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 7) FantomMakeQuickSailor(sld, SHIP_ROTERLOW, GetConvertStr("Variable_whisper_line_functions_8", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS16, 50, 50, 50);
...
{
	pchar.LockMapReload = GetConvertStr("Variable_whisper_line_functions_10", "Logs.txt");
	pchar.LockShoreReload = GetConvertStr("Variable_whisper_line_functions_11", "Logs.txt");
}

void CaptureCapitol_EndSeaBattle(string q)
{
	DeleteAttribute(pchar, "LockShoreReload");
	pchar.LockMapReload = GetConvertStr("Variable_whisper_line_functions_12", "Logs.txt");
...
void CaptureCapitol_ShoreBattleRespite(string q)
{
	...
	if (pchar.name == GetConvertStrWithReplace("Variable_whisper_line_functions_19", "Names.txt", "#space#", " "))
	{
	...
	sld.name 	= GetConvertStr("Variable_whisper_line_functions_0.8", "Names.txt");
	sld.lastname 	= GetConvertStr("Variable_whisper_line_functions_0.8.1", "Names.txt");
...
void CaptureCapitol_OnLeaderDeath_1(string q)
{
	...
	if (pchar.name == GetConvertStrWithReplace("Variable_whisper_line_functions_20", "Names.txt", "#space#", " "))	sld = CharacterFromID("Wh_Jim");
	else sld = GetCharacter(NPC_GenerateCharacter("Wh_Jim", "officer_"+(rand(42)+1), "man", "man", 5, PIRATE, -1, false));
...
void RefreshTitanic_1(string q)
{
	if(CheckAttribute(pchar, "TitanicIsKicking"))
	{
		...
		Log_TestInfo(GetConvertStrWithReplace("Variable_whisper_line_functions_8", "Logs.txt", "#space#", " ") + pchar.TitanicCity);
...
{
	Log_Info(GetConvertStrWithReplace("Variable_whisper_line_functions_9", "Logs.txt", "#space#", " "))
