...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
//	Тестовый запуск для разработчиков
/////////////////////////////////////////////////////////////////////////////////////////////////////////

void TEST_StartGame(string qName)
{
	bBettaTestMode = true;									//Режим разработчика
	bLocatorShow  = false;									//Локаторы
	bDisableLandEncounters = true;							//Энкаунтеры выключены
	setWDMPointXZ("DesMoines_port");						//Корабль на глобалке
	setCharacterShipLocation(pchar, "DesMoines_port");		//Корабль в порту
	
	SetCharacterPerk(pchar, "SeaDogProfessional");			//Гроза морей, чтобы энки не нападали на глобалке
	GiveItem2Character(PChar, "talisman2");					//Оберег Иона, чтобы шторм не нападал на глобалке
	EquipCharacterByItem(Pchar, "talisman2");
	
	pchar.rank = 30;
	SetSPECIAL(pchar, 10, 10, 10, 10, 10, 10, 10);
	SetSelfSkill(pchar, 100, 100, 100, 100, 100);
	SetShipSkill(pchar, 100, 100, 100, 100, 100, 100, 100, 100, 100);
	LAi_SetHP(pchar, 5000.0, 5000.0);
	AddBonusEnergyToCharacter(pchar, 5000);
	LAi_GetCharacterMaxEnergy(pchar);
	
	GiveItem2Character(PChar, "blade34");
	...
	pchar.Ship.name="Рыжая Лисица";
	SetBaseShipData(pchar);
	pchar.Ship.Cannons.Type = CANNON_TYPE_CANNON_LBS28;
	SetCrewQuantityFull(pchar);
	realships[sti(pchar.ship.type)].SpeedRate = 30;
	realships[sti(pchar.ship.type)].TurnRate  = 25;
	SetCharacterGoods(pchar, GOOD_FOOD, 1000);
	
	pchar.quest.TEST = true;	
	DoQuestFunctionDelay("TEST_StartGame_2", 0.5);
	
	setCharacterShipLocation(pchar, "DesMoines_port");
...
void TEST_StartGame_2(string qName)
{
	ClearAllLogStrings();
	Log_info(GetConvertStrWithReplace("Variable_mainhero_prologues_functions_1", "Logs.txt", "#space#", " "));
}
...
void Sharp_MeetSiblings(string qName)
{
	...
	if (startherotype == 4)
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_1", "Names.txt", "#space#", " ");
		sld.lastname = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_2", "Names.txt", "#space#", " ");
	...
	else
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_3", "Names.txt", "#space#", " ");
		sld.lastname = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_4", "Names.txt", "#space#", " ");
...
void SharleMary_Indians(string qName)
{
	...
	for (int i = 1; i<=maxIndians; i++)
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_6", "Names.txt", "#space#", " ");
		sld.lastname = "";
	...
	if (startherotype == 6)
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_7", "Names.txt", "#space#", " ");
		sld.lastname = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_8", "Names.txt", "#space#", " ");
	...
	else
	{
		...
		sld.name = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_9", "Names.txt", "#space#", " ");
		sld.lastname = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_10", "Names.txt", "#space#", " ");
...
void Undead_StartGame(string qName)
{
	...
	sld.name = GetConvertStrWithReplace("Variable_mainhero_prologues_functions_11", "Names.txt", "#space#", " ");
	sld.lastname = "";
...
{
	sld = GetCharacter(NPC_GenerateCharacter("Gravedigger", "HermitMonk", "man", "man", 1, PIRATE, 0, true));
	sld.dialog.filename   = "Quest\MainheroPrologues\Prologue_Undead_dialog.c";
	...
	DeleteAttribute(pchar, "questdata.ProklyatyIdol");
}
void Undead_Start_Graveyard_3(string qName)
{
	//TakeNItems(pchar, "suit_1", 1);
	//Log_Info(GetConvertStrWithReplace("Variable_mainhero_prologues_functions_2", "Logs.txt", "#space#", " "));
