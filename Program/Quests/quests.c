...
#include "quests\angelika_tich_line_functions.c"
...
string GetDateString()
{
	...
	if (checkattribute(pchar, "questTemp.WhisperLine"))
	{
...
void AddQuestUserData(string idQuest, string strID, string strData)
{
	...
	if(LanguageGetLanguage() != "russian" && findsubstr(strID, "Sex", 0) != -1) return;
...
void CloseQuestHeader(string idQuest)
{
	...
	Log_SetStringToLog(GetConvertStrWithReplace("Variable_quests_2", "Logs.txt", "#space#", " ")+GetConvertStr(pchar.QuestInfo.(idQuest).idRef, "QuestBookStrings.txt")+")");
	object questRef;
...
void QuestToSeaLogin_Launch()
{
	...
	SendMessage(&fader, "ls",FADER_PICTURE0,"loading\jonny_load\sea\sea_"+rand(3)+".dds");
...
bool isLocationFreeForQuests(string loc_id)
{
	...
	{ // проверяем НПС, обычно там никого нет, если есть - зянято
		for(i = 0; i<TOTAL_CHARACTERS; i++)
		{
			...
			{
				if (!IsOfficer(chr))
				{   // это не ГГ и офы, фикс квестов с заключенным под боком
					if (chr.id == "ScamCharacter" || GetCharacterIndex(pchar.questTemp.jailCanMove.prisonerId) >= 0) continue;
					bEnableEncounters = false;
					break;
				}
			}
...
void CalculateCheatsInfo() // Для статистики по читам. Используется в интерфейсе Debuger
{
	...
	string sTextGameVersion = GetConvertStrWithReplace("Variable_game_version", "Interface.txt", "#enter#", "\n");
	string sTextCheats = GetConvertStrWithReplace("Variable_cheats", "Interface.txt", "#enter#", "\n");;

	SetFormatedText("INFO_TEXT2", sTextGameVersion + pchar.versionnumber + sTextCheats);
	...
	for(i=0; i<iAllCheats; i++)
	{
		...
		if(HasSubStr(sCheat, "F"))
			AddLineToFormatedText("INFO_TEXT2", XI_ConvertString("B_" + sCheat) + " - " + iCurCheatCount + " " + XI_ConvertString("Times"));
		else
			AddLineToFormatedText("INFO_TEXT2", sCheat + " - " + iCurCheatCount + " " + XI_ConvertString("Times"));

		iAllCheatsCount = iAllCheatsCount + iCurCheatCount;
	}

	PChar.Statistic.Cheats = iAllCheatsCount;
	AddLineToFormatedText("INFO_TEXT2", XI_ConvertString("Total") + " " + iAllCheatsCount + " " + XI_ConvertString("Times"));

	if(iAllCheatsCount == 0) 
	{
		sTextCheats = GetConvertStrWithReplace("Variable_no_cheats", "Interface.txt", "#enter#", "\n");
		SetFormatedText("INFO_TEXT2", sTextGameVersion + pchar.versionnumber + sTextCheats);
	}
