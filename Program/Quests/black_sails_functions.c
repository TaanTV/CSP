...
void BSOnTheHorizon_start(string qName)
{
	...
	sld.talker = 10;
	sld.name = GetConvertStrWithReplace("Variable_black_sails_functions_4", "Names.txt", "#space#", " ");
	sld.lastname = GetConvertStrWithReplace("Variable_black_sails_functions_5", "Names.txt", "#space#", " ");
	...
	if(pchar.location == pchar.location.from_sea || HasSubStr(pchar.location,"_town" || HasSubStr(pchar.location,"shore")))
	{
		if (CheckAttribute(pchar, "BSCPFlint_talk") && !bSeaActive)
		{
			...
			if (!CheckAttribute(sld, "curtown") || sld.curtown != pchar.location)
			{
				...
				chr.name = GetConvertStrWithReplace("Variable_black_sails_functions_16", "Names.txt", "#space#", " ");
				chr.lastname = GetConvertStrWithReplace("Variable_black_sails_functions_17", "Names.txt", "#space#", " ");
...
{
	WaitDate("",0,0,3,3,3);
	SetLaunchFrameFormParam(GetConvertStr("Variable_black_sails_functions_1", "Frames.txt"), "", 0.1, 2.0);
	LaunchFrameForm();
...
void BSBons_SeaBattle(bool bonsAdmiral)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		FantomMakeCoolSailor(sld, SHIP_ROTTERDAM, GetConvertStr("Variable_black_sails_functions_1", "ShipNames.txt"), CANNON_TYPE_CANNON_LBS32, 100, 100, 100);
		sld.AlwaysEnemy = true;
...
void BSUrka_Pirates_town(string q)
{
	...
	sld.name = GetConvertStrWithReplace("Variable_black_sails_functions_32", "Names.txt", "#space#", " ");
	sld.lastname = "";
...
void BSUrka_Curacao_SeaBattle()
{
	...
	for (i = 1; i < 7; i++)
	{
		...
		if (i == 1)
		{
			...
			//sld.Ship.Name = GetConvertStrWithReplace("Variable_black_sails_functions_33", "Names.txt", "#space#", " ");
			//sld.Name = GetConvertStrWithReplace("Variable_black_sails_functions_34", "Names.txt", "#space#", " ");
			//sld.Lastname = GetConvertStrWithReplace("Variable_black_sails_functions_35", "Names.txt", "#space#", " ");
		}
...
void BSUrka_PlaceUrka()
{
	...
	{
		sMast = "mast" + m;
		sld.ship.masts.(sMast) = true;//пофиксить слом мачт. почему 4ре ломаем?
	}
...
{
	WaitDate("",0,0,3,3,3);
	SetLaunchFrameFormParam(GetConvertStr("Variable_black_sails_functions_2", "Frames.txt"), "", 0.1, 2.0);
	LaunchFrameForm();
...
{
	WaitDate("",0,0,3,3,3);
	SetLaunchFrameFormParam(GetConvertStr("Variable_black_sails_functions_3", "Frames.txt"), "", 0.1, 2.0);
	LaunchFrameForm();
	...
	for (int i = 1; i <= 4; i++)
	{
		...
		if(LAi_IsDead(sld))
		{
			if(!CheckAttribute(pchar, "BSHangover_IsEveryoneOkay"))
			{
				...
				SetLaunchFrameFormParam(GetConvertStr("Variable_black_sails_functions_4", "Frames.txt"), "", 0.1, 2.0);
				LaunchFrameForm();
			}
			DeleteCharacter(sld);
			LAi_SetCurHPMax(sld);
...
{
	SetLaunchFrameFormParam(GetConvertStr("Variable_black_sails_functions_5", "Frames.txt"), "", 0.1, 2.0);
	LaunchFrameForm();
	WaitDate("",0,0,0,3,3);
	if (WhisperIsHere())
	{
