...
void SetCamShuttle(ref loc) // boal вынес в метод
{
	...
	else
	{
		if (CheckAttribute(pchar, "GenQuest.CamShuttle"))
		{
			...
			{
				//Log_Info(GetConvertStrWithReplace("Variable_locations_loader_1", "Logs.txt", "#space#", " "));
			}
			else
			{
				if (Pchar.Drunked.Today != Pchar.dayRandom) Log_Info(GetConvertStrWithReplace("Variable_locations_loader_2", "Logs.txt", "#space#", " "));
			}
...
bool LoadLocation(ref loc)
{
	...
	//<-- проверяем наличие контры для установки нужного патча
	if(Whr_IsDay() != 0 || IsLocationCaptured(loc.id))
	{
		//Day
...
bool LocLoadModel(aref loc, string sat, string addition)
{
	...
	if(SendMessage(loc, "le", MSG_LOCATION_GET_MODEL, &mdl) != 0)
    {
        ...
        {
            SendMessage(&Flag, "lil", MSG_FLAG_INIT, &mdl, GetCityNation(colonies[FindColony(loc.townsack)].id));
        }
        else 
        {
...
void UpdateFortWeather() {
	DelEventHandler("FaderEvent_EndFadeIn", "UpdateFortWeather");
...
void LocLoadShips(ref Location)
{
	...
	if(Location.id == "BOARDING_FORT")
	{
		...
		rCharacter.Ship.Pos.x = stf(Location.locators.ships.shipGG.x);
		rCharacter.Ship.Pos.z = stf(Location.locators.ships.shipGG.z);
		rCharacter.Ship.Ang.y = GetAngleY(stf(Location.locators.ships.shipGG.vZ.x),stf(Location.locators.ships.shipGG.vZ.z));
		rCharacter.Ship.stopped = true;
		rCharacter.Ship.Speed.z = 0.0;
		SendMessage(&locShips[locNumShips],"laa",MSG_SHIP_CREATE,&rCharacter,&rShip);
		pchar.fakefireindex = locNumShips;
		SetEventHandler("FaderEvent_EndFadeIn", "UpdateFortWeather", 0);
		InitFakeFire();
	}
