...
void InitInterface_RR(string iniName, ref ContraTrader , ref pStore)
{
	...
	GameInterface.TABLE_LIST.hr.td2.str = GetConvertStrWithReplace("Variable_contraband_1", "Interface.txt", "#space#", " ");
	...
	GameInterface.TABLE_LIST.hr.td7.str = GetConvertStrWithReplace("Variable_contraband_2", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td7.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td8.str = GetConvertStrWithReplace("Variable_contraband_3", "Interface.txt", "#space#", " ");
...
void ShowGoodsInfo(int iGoodIndex)
{
    ...
    goodsDescr += newStr() + XI_ConvertString("weight") + " " + Goods[iGoodIndex].weight + " " + XI_ConvertString("cwt") +
	              GetConvertStrWithReplace("Variable_contraband_4", "Interface.txt", "#space#", " ") + Goods[iGoodIndex].Units + " " + XI_ConvertString("units");
...
void SetVariable()
{
	...
	sText = XI_ConvertString("OurMoney") + " " + FindRussianMoneyString(sti(pchar.money)) +  GetConvertStrWithReplace("Variable_contraband_5", "Interface.txt", "#space#", " ") + FindRussianMoneyString(sti(sld.SmugglingMoney));
...
void ShowHelpHint()
{
	...
	{// покажем помощь по работе с формой
        sHeader = GetConvertStrWithReplace("Variable_contraband_6", "Interface.txt", "#space#", " ");
...
void ChangeQTY_EDIT()
{
	...
	else
	{
		if (sti(GameInterface.qty_edit.str) < 0 || BuyOrSell == -1)
		{
		    ...
		    // проверка на колво доступное <--
		    SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_contraband_8", "Interface.txt", "#space#", " "));
		    SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_contraband_9", "Interface.txt", "#space#", " ") + makeint(iStorePrice*stf(GameInterface.qty_edit.str) / iUnits + 0.5) +
			                GetConvertStrWithReplace("Variable_contraband_10", "Interface.txt", "#space#", " ") + iWeight + GetConvertStrWithReplace("Variable_contraband_11", "Interface.txt", "#space#", " "));
		}
		else
		{
			BuyOrSell = 1;
			...
			SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_contraband_12", "Interface.txt", "#space#", " "));
			SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_contraband_13", "Interface.txt", "#space#", " ") + makeint(iShipPrice*stf(GameInterface.qty_edit.str) / iUnits + 0.5) +
			                GetConvertStrWithReplace("Variable_contraband_14", "Interface.txt", "#space#", " ") + iWeight + GetConvertStrWithReplace("Variable_contraband_15", "Interface.txt", "#space#", " "));
		}
	}
