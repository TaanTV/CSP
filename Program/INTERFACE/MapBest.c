...
void FillTable()
{
					...
					if(fMouseY >= stf(GameInterface.MAP.imagelist.(sColony).y - 10))	// 0
					{
						if(fMouseY <= stf(GameInterface.MAP.imagelist.(sColony).y) + 60.0)	// 50
						{
							if(sColony != "Panama" && sColony != "IslaMona" && sColony != "KhaelRoa" && sColony != "Caiman")
							{
								...
								SetFormatedText("LOCS_CAPTION", GetConvertStrWithReplace("Variable_MapBest_1", "Interface.txt", "#space#", " "));
								...
								string row = "";
								int n = 1;
								GameInterface.TABLE_LOCS.hr.td1.str = GetConvertStrWithReplace("Variable_MapBest_2", "Interface.txt", "#space#", " ");
								GameInterface.TABLE_LOCS.hr.td2.str = GetConvertStrWithReplace("Variable_MapBest_3", "Interface.txt", "#space#", " ");
								for(int z=0; z<MAX_LOCATIONS; z++)
								{
									if (GiveArealByLocation(&locations[z]) == island)
									{
										...
										if (HasSubStr(locations[z].id.label,"upstairs")) GameInterface.TABLE_LOCS.(row).td2.str = GetConvertStrWithReplace("Variable_MapBest_4", "Interface.txt", "#space#", " ");
										n++;
									}
								}
	...
	if (colonyindex != i)
	{
		...
		if (fMouseX >= 1628.0 && fMouseX <= 1698.0 && fMouseY >= 1607.0 && fMouseY <= 1711.0)
		{
		...
		XI_WindowShow("TP_WINDOW", true);
		XI_WindowDisable("TP_WINDOW", false);
		SetFormatedText("LOCS_CAPTION", GetConvertStrWithReplace("Variable_MapBest_5", "Interface.txt", "#space#", " "));
		Table_Clear("TABLE_LOCS",false,true,false);
		GameInterface.TABLE_LOCS.select = 0;
		GameInterface.TABLE_LOCS.top = 0;
		...
		string row1 = "";
		int x = 1;
		GameInterface.TABLE_LOCS.hr.td1.str = GetConvertStrWithReplace("Variable_MapBest_6", "Interface.txt", "#space#", " ");
		GameInterface.TABLE_LOCS.hr.td2.str = GetConvertStrWithReplace("Variable_MapBest_7", "Interface.txt", "#space#", " ");
		for(int y=0; y<MAX_LOCATIONS; y++)
		{
			if (GiveArealByLocation(&locations[y]) == ssColony || CheckForIslandID(&locations[y]) == ssColony)
			{
				...
				if (HasSubStr(locations[y].id.label,"upstairs")) GameInterface.TABLE_LOCS.(row1).td2.str = GetConvertStrWithReplace("Variable_MapBest_8", "Interface.txt", "#space#", " ");
				if (HasSubStr(locations[y].id,"Arena")) GameInterface.TABLE_LOCS.(row1).td2.str = GetConvertStrWithReplace("Variable_MapBest_9", "Interface.txt", "#space#", " ");
				if (HasSubStr(locations[y].id,"FencingTown_F")) GameInterface.TABLE_LOCS.(row1).td2.str = GetConvertStrWithReplace("Variable_MapBest_10", "Interface.txt", "#space#", " ");
				x++;
			}
		}
		Table_UpdateWindow("TABLE_LOCS");
...
void ShowColonyInfo(int iColony)
{
	...
	for(iType=0; iType<GOODS_QUANTITY; iType++)
	{
		...
		sText = GetConvertStr("Variable_MapBest_11", "Interface.txt");
		AddLineToFormatedText("COLONY_INFO_TEXT", sText);
...
float _GetDirToColony(string _sColony)
{
	...
	if(_sColony == "FortOrange") sColonyTown = "FortOrange_port";
