...
void InitInterface_R(string iniName, ref pStore)
{
	...
	if (CheckAttribute(PChar,"ColonyBuilding.StoreHouse.Resize") && PChar.ColonyBuilding.StoreHouse.Resize == true) iMaxGoodsStore = 100000;
	refCharacter = pchar;

	GameInterface.TABLE_LIST.hr.td1.str = XI_ConvertString("In the hold");
	GameInterface.TABLE_LIST.hr.td1.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td2.str = GetConvertStrWithReplace("Variable_storage_1", "Interface.txt", "#space#", " ");
	...
	GameInterface.TABLE_LIST.hr.td5.str = GetConvertStrWithReplace("Variable_storage_2", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td5.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td6.str = GetConvertStrWithReplace("Variable_storage_3", "Interface.txt", "#space#", " ");
...
void ShowHelpHint()
{
	...
	{// покажем помощь по работе с формой
        sHeader = GetConvertStrWithReplace("Variable_storage_4", "Interface.txt", "#space#", " ");
		sText1 = GetConvertStr("Variable_guide_trade", "Interface.txt");

        sText2 = GetConvertStr("Variable_fast_transshipment", "Interface.txt");
...
void SetVariable()
{
	...
	sText = GetConvertStrWithReplace("Variable_storage_6", "Interface.txt", "#space#", " ") + GetStorageUsedWeight(refStore) + " / " + iTotalSpace + GetConvertStrWithReplace("Variable_storage_7", "Interface.txt", "#space#", " ");
...
void ShowGoodsInfo(int iGoodIndex)
{
    ...
    goodsDescr += newStr() + XI_ConvertString("weight") + " " + Goods[iGoodIndex].weight + " " + XI_ConvertString("cwt") +
	              GetConvertStrWithReplace("Variable_storage_8", "Interface.txt", "#space#", " ") + Goods[iGoodIndex].Units + " " + XI_ConvertString("units");
...
void ChangeQTY_EDIT()
{
	...
	else
	{
		if (sti(GameInterface.qty_edit.str) < 0 || BuyOrSell == -1)
		{
		    ...
		    // проверка на колво доступное <--
		    SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_storage_9", "Interface.txt", "#space#", " "));
		}
		else
		{
			BuyOrSell = 1;
		    ...
		    // проверка на колво доступное <--
			SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_storage_10", "Interface.txt", "#space#", " "));
		}
	}
	// если получили ноль
	if (sti(GameInterface.qty_edit.str) == 0)
