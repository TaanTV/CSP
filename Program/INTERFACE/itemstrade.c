...
void InitInterface_R(string iniName, ref pTrader)
{
	...
	GameInterface.TABLE_LIST.hr.td1.str = GetConvertStrWithReplace("Variable_itemstrade_1", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td1.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td2.str = GetConvertStrWithReplace("Variable_itemstrade_2", "Interface.txt", "#space#", " ");
	...
	GameInterface.TABLE_LIST.hr.td4.str = GetConvertStrWithReplace("Variable_itemstrade_3", "Interface.txt", "#space#", " ");
	...
	GameInterface.TABLE_LIST.hr.td6.str = GetConvertStrWithReplace("Variable_itemstrade_4", "Interface.txt", "#space#", " ");//XI_ConvertString("In the store");
	GameInterface.TABLE_LIST.hr.td6.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td7.str = GetConvertStrWithReplace("Variable_itemstrade_5", "Interface.txt", "#space#", " ");
...
void AddToTable()
{
	...
	for (i = 0; i < GetAttributesNum(arInventory); i++)
	{
		...
		if (iTemp > 0)
		{
			GameInterface.TABLE_LIST.(row).td4.str = GameInterface.TABLE_LIST.(row).td4.str + GetConvertStrWithReplace("Variable_itemstrade_6", "Interface.txt", "#space#", " ") + iTemp + ")";
...
void ShowHelpHint()
{
	...
	if (bCheckboxes)
	{
		...
		sText1 = GetConvertStrWithReplace("Variable_itemstrade_7", "Interface.txt", "#space#", " ");
		sText3 = GetConvertStrWithReplace("Variable_itemstrade_8", "Interface.txt", "#space#", " ");
		sText2 = GetConvertStrWithReplace("Variable_itemstrade_9", "Interface.txt", "#space#", " ");
	}
	else
	{// покажем помощь по работе с формой
		sHeader = XI_ConvertString("titleItemsTrade");
		sText1 = GetConvertStr("Variable_guide_trade", "Interface.txt");
		sText3 = GetConvertStrWithReplace("Variable_itemstrade_11", "Interface.txt", "#space#", " ");
		sText2 = GetConvertStr("Variable_quick_sell", "Interface.txt");
	}
...
void FillCharactersScroll()
{
	...
	bool ok = (rColony.from_sea == "") || (Pchar.location.from_sea == rColony.from_sea) || (Pchar.location == "Caiman_Store");
	if (Pchar.SystemInfo.CabinType != "" && sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//фикс - ошибки в логе, если ГГ без корабля и каюты
	{
...
void SetVariable()
{
	...
	sMaxGoodsStore = GetConvertStrWithReplace("Variable_itemstrade_12", "Interface.txt", "#space#", " "); //XI_ConvertString("store");
...
void FillCharacterInfo()
{
	...
	else
	{
		switch (GameInterface.CHARACTERS_SCROLL.(sAttr).character)
		{
			case "box1": sFullName = GetConvertStrWithReplace("Variable_itemsbox_31", "Interface.txt", "#space#", " ") break;
			case "box2": sFullName = GetConvertStrWithReplace("Variable_itemsbox_32", "Interface.txt", "#space#", " ") break;
			case "box3": sFullName = GetConvertStrWithReplace("Variable_itemsbox_33", "Interface.txt", "#space#", " ") break;
			case "box4": sFullName = GetConvertStrWithReplace("Variable_itemsbox_34", "Interface.txt", "#space#", " ") break;
		}
	}
...
void TransactionOK()
{
	...
	if (BuyOrSell == 0)
	{
 	...
 	if (BuyOrSell == 1) // BUY
	{
		if (loadedLocation.id == "EsmeraldaStoreBig" && sCurItem == "map_LSC") bDisableFastReload = false;
...
void ChangeQTY_EDIT()
{
	...
	else
	{
		if (sti(GameInterface.qty_edit.str) < 0 || BuyOrSell == -1)
		{
			...
			// квестовые не продать <--
			SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_itemstrade_13", "Interface.txt", "#space#", " "));
			SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_itemstrade_14", "Interface.txt", "#space#", " ") + makeint(iStorePrice*stf(GameInterface.qty_edit.str) + 0.5) +
							GetConvertStrWithReplace("Variable_itemstrade_15", "Interface.txt", "#space#", " ") + FloatToString(iWeight, 1) );
		}
		else
		{
			BuyOrSell = 1;
			...
			SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_itemstrade_16", "Interface.txt", "#space#", " "));
			SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_itemstrade_17", "Interface.txt", "#space#", " ") + makeint(iCharPrice*stf(GameInterface.qty_edit.str) + 0.5) +
							GetConvertStrWithReplace("Variable_itemstrade_18", "Interface.txt", "#space#", " ") + FloatToString(iWeight, 1) );
		}
	}
	// если получили ноль
	if (sti(GameInterface.qty_edit.str) == 0)
...
void Autotrade_All()
{
	if (!IsPCharHaveTreasurer()) {Log_Info(GetConvertStrWithReplace("Variable_itemstrade_19", "Interface.txt", "#space#", " ")); return;}
	Fastsale_trash();
	int iOfficer = -1;
	ref _chr;
...
//двумя циклами сначала продавать со всех, а потом покупать - это позволит GetConvertStrWithReplace("Variable_itemstrade_20", "Interface.txt", "#space#", " ") излишки через торговца. Но так будут теряться монеты на разнице цен.
...
void Autotrade_This()
{
	if (!IsPCharHaveTreasurer()) {Log_Info(GetConvertStrWithReplace("Variable_itemstrade_21", "Interface.txt", "#space#", " ")); return;}
...
void BuyConsume(ref chr)
{
	...
	string sTemp = "";
	if (chr.sex == "woman") sTemp = GetConvertStrWithReplace("Variable_itemstrade_22", "Interface.txt", "#space#", " ");
...
//Log_TestInfo(GetConvertStrWithReplace("Variable_itemstrade_25", "Interface.txt", "#space#", " ") + sItem + "|" + qty + "|" + curqty + "|" + curTraderQty);
		...
		if (qty > 0 && GetFullName(chr) == GetFullName(PChar)) Log_Info(GetFullName(chr) + GetConvertStrWithReplace("Variable_itemstrade_26", "Interface.txt", "#space#", " ") + "'" + LanguageConvertString(idLngFile, rItem.name) + "' " + qty + GetConvertStrWithReplace("Variable_itemstrade_27", "Interface.txt", "#space#", " ") + iCost*qty + GetConvertStrWithReplace("Variable_itemstrade_28", "Interface.txt", "#space#", " "));
	...
	if (iMoneyQty != 0) // Если хоть что-то купили
	{
		...
		int iExpAdd = MakeInt(abs(iMoneyQty * 0.5) / 800) + rand(1) + 1;//TO DO - это начисление опыта за торговлю товарами, перепроверить, что за предметы столько же идёт
		AddCharacterExpToSkill(rTreasurer, "Commerce", iExpAdd);
		AddCharacterExpToSkill(pchar, "Commerce", iExpAdd);
	}
	...
	{
		if (bAdded) 			{Log_Info(GetFullName(chr)+GetConvertStrWithReplace("Variable_itemstrade_29", "Interface.txt", "#space#", " ") + sTemp + GetConvertStrWithReplace("Variable_itemstrade_30", "Interface.txt", "#space#", " "));	return;}
		if (bNotOverWeight)		{Log_Info(GetFullName(chr)+GetConvertStrWithReplace("Variable_itemstrade_31", "Interface.txt", "#space#", " ") + sTemp + GetConvertStrWithReplace("Variable_itemstrade_32", "Interface.txt", "#space#", " ")); return;}
		if (bNotEnough) 		{Log_Info(GetFullName(chr)+GetConvertStrWithReplace("Variable_itemstrade_33", "Interface.txt", "#space#", " ") + sTemp + GetConvertStrWithReplace("Variable_itemstrade_34", "Interface.txt", "#space#", " ")); return;}
		if (bNotEnoughTrader) 	{Log_Info(GetFullName(chr)+GetConvertStrWithReplace("Variable_itemstrade_35", "Interface.txt", "#space#", " ") + sTemp + GetConvertStrWithReplace("Variable_itemstrade_36", "Interface.txt", "#space#", " ")); return;}
							 	 Log_Info(GetFullName(chr)+GetConvertStrWithReplace("Variable_itemstrade_37", "Interface.txt", "#space#", " ") + sTemp + GetConvertStrWithReplace("Variable_itemstrade_38", "Interface.txt", "#space#", " "));
	}
...
void SellExcessConsume(ref chr)
{
	...
	string sTemp = "";
	if (chr.sex == "woman") sTemp = GetConvertStrWithReplace("Variable_itemstrade_39", "Interface.txt", "#space#", " ");
	...
	if(iMoneyQty != 0) // Если хоть что-то продали
	{
		...
		AddCharacterExpToSkill(rTreasurer, "Commerce", iExpAdd);
		AddCharacterExpToSkill(pchar, "Commerce", iExpAdd);
		Log_Info(GetFullName(chr)+GetConvertStrWithReplace("Variable_itemstrade_40", "Interface.txt", "#space#", " ") + sTemp + GetConvertStrWithReplace("Variable_itemstrade_41", "Interface.txt", "#space#", " "));
	}
}
...
void Fastsale_trash()
{
	...
	if (rSaleProps.money != 0) { // если хоть что-то продали
		if (CheckAttribute(&InterfaceStates, "AutoSellLogs"))
		{
			Log_TestInfo(GetConvertStrWithReplace("Variable_itemstrade_42", "Interface.txt", "#space#", " ") + rSaleProps.BladeCount + GetConvertStrWithReplace("Variable_itemstrade_43", "Interface.txt", "#space#", " ") + rSaleProps.GunCount +
				GetConvertStrWithReplace("Variable_itemstrade_44", "Interface.txt", "#space#", " ") + rSaleProps.TalismanCount + GetConvertStrWithReplace("Variable_itemstrade_45", "Interface.txt", "#space#", " ") + rSaleProps.RubbishCount);
		}
		AddmoneyToCharacter(PChar, sti(rSaleProps.money));
		ref rTreasurer = GetPCharTreasurerRef(); //Казначей. Ему даем экспу
		int iExpAdd = MakeInt(abs(sti(rSaleProps.money) * 0.5) / 800) + rand(1) + 1;//TO DO - это начисление опыта за торговлю товарами, перепроверить, что за предметы столько же идёт
		AddCharacterExpToSkill(rTreasurer, "Commerce", iExpAdd);
		AddCharacterExpToSkill(pchar, "Commerce", iExpAdd);
	}
}
...
void Fastsale_trash_single(ref character)
{
	...
	if (rSaleProps.money != 0) { // если хоть что-то продали
		if(CheckAttribute(&InterfaceStates, "AutoSellLogs"))
		{
			Log_TestInfo(GetConvertStrWithReplace("Variable_itemstrade_46", "Interface.txt", "#space#", " ") + rSaleProps.BladeCount + GetConvertStrWithReplace("Variable_itemstrade_47", "Interface.txt", "#space#", " ") + rSaleProps.GunCount +
				GetConvertStrWithReplace("Variable_itemstrade_48", "Interface.txt", "#space#", " ") + rSaleProps.TalismanCount + GetConvertStrWithReplace("Variable_itemstrade_49", "Interface.txt", "#space#", " ") + rSaleProps.RubbishCount);
		}
		AddmoneyToCharacter(PChar, sti(rSaleProps.money));
		ref rTreasurer = GetPCharTreasurerRef(); //Казначей. Ему даем экспу
		int iExpAdd = MakeInt(abs(sti(rSaleProps.money) * 0.5) / 800) + rand(1) + 1;//TO DO - это начисление опыта за торговлю товарами, перепроверить, что за предметы столько же идёт
