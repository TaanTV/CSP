...
//НАДО ЛИ это затирание? Может, наоборот при закрытии все строчки заполнять, чтоб чекатрибут не делать?
					...
					SetFormatedText("GOODS_SET_SUM", GetConvertStrWithReplace("Variable_GoodsTransfer_1", "Interface.txt", "#space#", " ") + curBuyGoodsWeight + GetConvertStrWithReplace("Variable_GoodsTransfer_2", "Interface.txt", "#space#", " "));
				}
				else
				{
...
//НАДО ЛИ это затирание? Может, наоборот при закрытии все строчки заполнять, чтоб чекатрибут не делать?
					...
					SetFormatedText("ITEMS_SET_SUM", GetConvertStrWithReplace("Variable_GoodsTransfer_3", "Interface.txt", "#space#", " ") + FloatToString(curBuyItemsWeight,1));
				}
				UnShowWindow();
			}
			break;
...
void FillGoodsTable()
{
	...
	Table_UpdateWindow("GOODS_TABLE_LIST");
	SetFormatedText("GOODS_SET_SUM", GetConvertStrWithReplace("Variable_GoodsTransfer_6", "Interface.txt", "#space#", " ") + curBuyGoodsWeight + GetConvertStrWithReplace("Variable_GoodsTransfer_7", "Interface.txt", "#space#", " "));
}
...
void FillItemsTable()
{
	...
	Table_UpdateWindow("CONSUME_TABLE_LIST");
	SetFormatedText("ITEMS_SET_SUM", GetConvertStrWithReplace("Variable_GoodsTransfer_11", "Interface.txt", "#space#", " ") + FloatToString(curBuyItemsWeight,1));
}
...
void FillSetNameG(int iNum)
{
	...
	string sName;
	if (!checkattribute(pchar, sSET))	sName = GetConvertStrWithReplace("Variable_GoodsTransfer_12", "Interface.txt", "#space#", " ") + iNum;	else sName = pchar.(sSET);
	log_info(sName);
...
{
	string sSET = "GoodsSet" + iGOODS_SET;
	if (!checkattribute(pchar, sSET))	GameInterface.EDIT_BOX_GOODS.str = GetConvertStrWithReplace("Variable_GoodsTransfer_12", "Interface.txt", "#space#", " ") + iGOODS_SET;	else GameInterface.EDIT_BOX_GOODS.str = pchar.(sSET).nameset;
...
void SaveGoodsSet()
{
	...
	if (!checkattribute(pchar, sSET)) pchar.(sSET) = GetConvertStrWithReplace("Variable_GoodsTransfer_12", "Interface.txt", "#space#", " ") + iGOODS_SET;//в первый раз записываем "Комплект N"
	aref arToChar;
	aref arFromChar;
...
{
	string sSET = "ItemsSet" + iITEMS_SET;
	if (!checkattribute(pchar, sSET))	GameInterface.EDIT_BOX_ITEMS.str = GetConvertStrWithReplace("Variable_GoodsTransfer_12", "Interface.txt", "#space#", " ") + iITEMS_SET;	else GameInterface.EDIT_BOX_ITEMS.str = pchar.(sSET).nameset;
...
void SaveItemsSet()
{
	...
	if (!checkattribute(pchar, sSET)) pchar.(sSET) = GetConvertStrWithReplace("Variable_GoodsTransfer_12", "Interface.txt", "#space#", " ") + iITEMS_SET;//в первый раз записываем "Комплект N"
