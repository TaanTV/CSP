...
void InitInterface_R(string iniName, ref pStore)
{
	...
	GameInterface.TABLE_LIST.hr.td2.str = GetConvertStrWithReplace("Variable_ColonyCapture_1", "Interface.txt", "#space#", " ");
	...
	GameInterface.TABLE_LIST.hr.td5.str = GetConvertStrWithReplace("Variable_ColonyCapture_2", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td5.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td6.str = GetConvertStrWithReplace("Variable_ColonyCapture_3", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td6.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td7.str = "Общий вес";
	...
	if(refStore.Colony == "none")
	{
		SetFormatedText("STORECAPTION1", GetConvertStrWithReplace("Variable_ColonyCapture_4", "Interface.txt", "#space#", " ") + refShipChar.ship.name + "'");
	}
	else
	{
		SetFormatedText("STORECAPTION1", XI_ConvertString("Colony" + refStore.Colony) + GetConvertStrWithReplace("Variable_ColonyCapture_5", "Interface.txt", "#space#", " "));
	}
...
void AddToTable()
{
    ...
    for (i = 0; i< GOODS_QUANTITY; i++)
	{
		...
		GameInterface.TABLE_LIST.(row).td7.str = GetGoodWeightByType(i, sti(sStoreQ);
...
void ShowHelpHint()
{
	...
	{// покажем помощь по работе с формой
        sHeader = GetConvertStrWithReplace("Variable_ColonyCapture_6", "Interface.txt", "#space#", " ");
		sText1 = GetConvertStr("Variable_guide_trade", "Interface.txt");
        sText2 = GetConvertStr("Variable_fast_take", "Interface.txt");
		sText3 = GetConvertStrWithReplace("Variable_ColonyCapture_8", "Interface.txt", "#space#", " ") + newStr() +
				 GetConvertStrWithReplace("Variable_ColonyCapture_9", "Interface.txt", "#space#", " ") + newStr() +
				 GetConvertStrWithReplace("Variable_ColonyCapture_10", "Interface.txt", "#space#", " ") + newStr() +
				 GetConvertStrWithReplace("Variable_ColonyCapture_11", "Interface.txt", "#space#", " ") + newStr() +
				 GetConvertStrWithReplace("Variable_ColonyCapture_12", "Interface.txt", "#space#", " ");
...
void SetVariable()
{
	...
	{
	    sMaxGoodsStore = GetConvertStrWithReplace("Variable_ColonyCapture_13", "Interface.txt", "#space#", " ");
	}
...
void ShowGoodsInfo(int iGoodIndex)
{
    ...
    goodsDescr += newStr() + XI_ConvertString("weight") + " " + Goods[iGoodIndex].weight + " " + XI_ConvertString("cwt") +
	              GetConvertStrWithReplace("Variable_ColonyCapture_14", "Interface.txt", "#space#", " ") + Goods[iGoodIndex].Units + " " + XI_ConvertString("units");
...
void ChangeQTY_EDIT()
{
	...
	else
	{
		if (sti(GameInterface.qty_edit.str) < 0 || BuyOrSell == -1)
		{
		    ...
		    // проверка на колво доступное <--
		    SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_ColonyCapture_15", "Interface.txt", "#space#", " "));
		    SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_ColonyCapture_16", "Interface.txt", "#space#", " ") + iWeight + GetConvertStrWithReplace("Variable_ColonyCapture_17", "Interface.txt", "#space#", " "));
		}
		else
		{
			BuyOrSell = 1;
			...
			SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_ColonyCapture_18", "Interface.txt", "#space#", " "));
			SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_ColonyCapture_19", "Interface.txt", "#space#", " ") + iWeight + GetConvertStrWithReplace("Variable_ColonyCapture_20", "Interface.txt", "#space#", " "));
		}
	}
	// если получили ноль
	if (sti(GameInterface.qty_edit.str) == 0)
