...
void InitInterface_R(string iniName, ref pStore)
{
	...
	GameInterface.TABLE_LIST.hr.td2.str = GetConvertStrWithReplace("Variable_Cannons_1", "Interface.txt", "#space#", " ");
	...
	GameInterface.TABLE_LIST.hr.td7.str = GetConvertStrWithReplace("Variable_Cannons_2", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td7.scale = 0.9;

	GameInterface.TABLE_LIST.hr.td8.str = GetConvertStrWithReplace("Variable_Cannons_3", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td8.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td9.str = GetConvertStrWithReplace("Variable_Cannons_4", "Interface.txt", "#space#", " ");
	GameInterface.TABLE_LIST.hr.td9.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td10.str = GetConvertStrWithReplace("Variable_Cannons_5", "Interface.txt", "#space#", " ");
	...
	if(refStore.Colony == "none")
	{
		SetFormatedText("STORECAPTION1", GetConvertStrWithReplace("Variable_Cannons_6", "Interface.txt", "#space#", " ") + refShipChar.ship.name + "'");
	}
	else
	{
...
void ShowHelpHint()
{
	...
	{// покажем помощь по работе с формой
        sHeader = GetConvertStrWithReplace("Variable_Cannons_7", "Interface.txt", "#space#", " ");
		sText1 = GetConvertStr("Variable_guide_trade", "Interface.txt");

        sText2 = GetConvertStr("Variable_quick_sell", "Interface.txt");

        sText3 = GetConvertStrWithReplace("Variable_Cannons_9", "Interface.txt", "#space#", " ");
...
void SetVariable()
{
	...
	switch (iCannonType)
		{
		case CANNON_NAME_CANNON			: sText = sText + GetConvertStrWithReplace("Variable_Cannons_10", "Interface.txt", "#space#", " "); break;
		case CANNON_NAME_CULVERINE		: sText = sText + GetConvertStrWithReplace("Variable_Cannons_11", "Interface.txt", "#space#", " "); break;
		case CANNON_NAME_SPECIAL_CANNON	: sText = sText + GetConvertStrWithReplace("Variable_Cannons_12", "Interface.txt", "#space#", " "); break;
		}
	sText = sText + GetCannonCaliber(sti(refCharacter.Ship.Cannons.Type)) + GetConvertStrWithReplace("Variable_Cannons_13", "Interface.txt", "#space#", " ");
	sText = sText + "\n";
...
void ShowGoodsInfo(int iGoodIndex)
{
    ...
    goodsDescr += newStr() + XI_ConvertString("weight") + " " + Goods[iGoodIndex].weight + " " + XI_ConvertString("cwt") +
	              GetConvertStrWithReplace("Variable_Cannons_14", "Interface.txt", "#space#", " ") + Goods[iGoodIndex].Units + " " + XI_ConvertString("units");
...
void ChangeQTY_EDIT()
{
	...
	else
	{
		if (sti(GameInterface.qty_edit.str) < 0 || BuyOrSell == -1)
		{
		    ...
		    // проверка на колво доступное <--
		    SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_Cannons_15", "Interface.txt", "#space#", " "));
		    SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_Cannons_16", "Interface.txt", "#space#", " ") + makeint(iStorePrice*stf(GameInterface.qty_edit.str) / iUnits + 0.5) +
			                GetConvertStrWithReplace("Variable_Cannons_17", "Interface.txt", "#space#", " ") + iWeight + GetConvertStrWithReplace("Variable_Cannons_18", "Interface.txt", "#space#", " "));
		}
		else
		{
			...
			SetFormatedText("QTY_TypeOperation", GetConvertStrWithReplace("Variable_Cannons_19", "Interface.txt", "#space#", " "));
			SetFormatedText("QTY_Result", GetConvertStrWithReplace("Variable_Cannons_20", "Interface.txt", "#space#", " ") + makeint(iShipPrice*stf(GameInterface.qty_edit.str) / iUnits + 0.5) +
			                GetConvertStrWithReplace("Variable_Cannons_21", "Interface.txt", "#space#", " ") + iWeight + GetConvertStrWithReplace("Variable_Cannons_22", "Interface.txt", "#space#", " "));
		}
	}
	// если получили ноль
	if (sti(GameInterface.qty_edit.str) == 0)
